...
**Why:** incident 2026-07-05 — asked to sync a ruff pre-commit gate with CI and fix the errors it surfaced (scoped, semantic fixes). When CI's `black --check` step then failed too, ran `black .` across the WHOLE pyutilz repo unasked — 183 of 215 files, ~4000 lines changed — while the user was away. Reverted in full once caught, but it should never have run. A repo-wide mechanical rewrite is exactly the kind of large-blast-radius action that needs a check-in first, same as the existing "narrow set of files you just edited" ruff `--fix` rule below — this generalizes that rule to ALL formatters/linters, not just ruff.

**How to apply:** when a project-wide formatter would fix a CI/lint failure, do NOT run it. Instead: report the scope (e.g. "N of M files need reformatting, that's a repo-wide rewrite"), and ask whether to (a) run it now, (b) turn the gate advisory/non-blocking, or (c) leave it for a deliberate separate pass. Small, scoped fixes to the specific files already being edited in the current task remain fine without asking (that's the existing narrow-fix carve-out) — the line is "does this touch files/lines beyond what I'm already deliberately changing for a diagnosed reason".

**Black repo-wide reformat — resolved (2026-07-05):** user reviewed the major diff classes and decided to exclude exactly two from any Black run, project-wide, in both repos: (1) arg/collection-list explosion (multi-item packed line -> one-item-per-line, including `from x import (...)` blocks) and (2) blank-line insertion (pure or bundled with other changes in the same hunk). Neither is configurable via any stock Black flag (tested `--skip-magic-trailing-comma`: no effect on explosion; blank-line rules have no flag at all). Originally built as `scripts/black_filtered_apply.py` (present in both repos); extracted 2026-07-09 into the shared `py_ci_shared` package (https://github.com/fingoldo/py-ci-shared, installed via `pip install -e ".[dev]"`) so mlframe and pyutilz consume the identical script instead of drifting copies. Applies everything else Black wants (collapses, quote style, blank-line removal, docstring reflow, redundant-paren removal, whitespace reflow) while mechanically rejecting those two classes via a diff-opcode-level filter, validated via AST-equivalence + compile checks on samples before any real file was touched. `black --check`/the vanilla `Black (filtered)` CI workflow are both advisory. Use `python -m py_ci_shared.black_filtered_apply --config pyproject.toml --write <files>` (or `--check <dirs>` for CI/dry-run) going forward instead of raw `black`/`black --fix` -- never raw `black --write`/`black .` in place, since that would apply the two excluded classes too.

## RUF100 (unused-noqa) auto-fix is UNSAFE under a narrow `--select` — never run it that way (CRITICAL, 2026-07-09)

`ruff check --select <narrow-set> --fix` where `<narrow-set>` does NOT include the rule a `# noqa: X` comment is suppressing makes ruff conclude that comment is "unused" (RUF100), because RUF100 evaluates against the CURRENTLY SELECTED rules, not the project's full configured `select` in `pyproject.toml`. `--fix` then silently deletes the noqa comment — even though it is load-bearing for the actual full-config / CI gate.

**Why:** incident 2026-07-09 — a docstring-coverage sweep ran `ruff check --select RUF100,RUF046 --fix` (scoped to the two new rule categories being triaged) across many files. Every `# noqa: F401`, `# noqa: F401,F403`, `# noqa: BLE001`, `# noqa: E402`, etc. in those files got silently stripped, because F401/BLE001/E402 weren't in that narrow `--select`. This broke ~805 legitimate noqa suppressions across ~100 files, including the star-import re-export pattern (`from mlframe.core.arrays import *  # noqa: F401,F403`) used throughout package facades, and dead-looking-but-actually-cross-module-re-exported single-name imports (`_sha256_of_file`, `evaluate_candidates`, kaleido lifecycle functions, etc.). The damage was invisible locally (the narrow-select run reported success) and only surfaced when CI's actual blocking gate (`ruff check . --select F,E9`, no ignore) started failing with 920 errors — up from a pre-session baseline of 88. Recovery required diffing the full session range against the pre-session baseline commit, reconstructing every stripped noqa comment by diff position, and then auditing the remaining ~90 findings by hand to distinguish genuinely-dead imports (delete) from re-exports whose comment simply never existed or was lost further back (restore with a fresh justified `# noqa`).

**How to apply:**
- **Never** run `ruff check --select <subset> --fix` (or any `--fix` invocation) unless the `--select` matches the project's full configured rule set (i.e., omit `--select` entirely and let `pyproject.toml`'s `[tool.ruff.lint] select` apply) OR you have manually confirmed the subset's rules cannot interact with RUF100 (i.e., none of the files being fixed carry a `noqa` for a rule outside the subset).
- When triaging ONE new rule category (e.g. "let's carefully add RUF, one category at a time"), do `ruff check --select <that category>` to LIST findings, then fix them by hand with Edit — never `--fix` on the narrow select.
- If you must run a mechanical `--fix` pass, run it with the FULL select (no override) so RUF100 sees the complete picture, and only on the narrow set of files you just edited for a diagnosed reason (existing rule above).
- After ANY RUF100/unused-noqa related fix, immediately verify with the project's actual blocking CI gate command (`ruff check . --ignore C901` in this repo, as of 2026-07-09 — see below; historically `ruff check . --select F,E9`) before considering the change done — a narrow-select "clean" result proves nothing about the real gate.
- **`--ignore` is safe, `--select` is not**, for exactly this reason: `--ignore <code>` only ADDS to the configured `ignore` list, so the rest of `pyproject.toml`'s `select`/`ignore` stays intact and RUF100 still sees the full picture. `--select` REPLACES the effective rule set outright. When expanding or narrowing the blocking gate, reach for `--ignore`, never `--select`.

**Blocking gate expanded 2026-07-09:** after closing RUF/G/ISC/PERF to 0 findings (RUF005 collection-concat, G logging-format, ISC implicit-concat, PERF anti-patterns), the blocking command (CI's `Ruff (real bugs - blocking)` step and the pre-commit `ruff (real bugs block)` hook) changed from `ruff check . --select F,E9` to `ruff check . --ignore C901` — the project's full configured select/ignore, minus C901 (complexity, which stays advisory per the mccabe threshold rationale below). Both gates run the identical command so a commit that passes locally cannot fail CI and vice versa.
...
7. **Identity gate ALONGSIDE speed — always.** Prove the output (selection / MI / metric / report) is unchanged: exact `==` where possible, or document the FP reduction-order delta (~1e-9 / single-ULP from a different summation order) AND confirm it cannot move a decision. A selection/score-altering divergence (~1e-3) is NOT acceptable — gate or reject. Pin the identity in a fail-on-regression test (verify it FAILS on pre-fix code). **FE/MRMR exception (user-owned, 2026-06-20):** for FE candidate-MI / binning speed levers the bar is SELECTION-equivalence (the SAME features get selected — validate against the clean-compound pins `single_compound`/`clean_library_form_preferred`/`prewarp_retained` + `_compound_gate.py` at n=10k/100k), NOT bit-identical MI. Approximate kernels (f32 sort keys, sort-free / tail-compressed quantile binning) are fine if the selection holds — a microbench argmax flip is OK iff end-to-end feature selection is unchanged. Still reject anything that changes WHICH features are selected.
...
## GPU profiling: nvprof per-kernel % LIES — measure the WALL (CRITICAL — 2026-06-20, 3 false leads)

Profiling GPU/CUDA code has its own traps distinct from CPU profiling. Learned the hard way chasing the MRMR GPU-FE path (coalescing + launch-batching both shipped 7x isolated-kernel wins that were 0x on the wall):

1. **nvprof per-kernel time is SERIALIZATION-DISTORTED.** nvprof forces kernel syncs, so a kernel that overlaps CPU work in production shows an inflated GPU-time `%`. A kernel measured "35% of GPU time / 6.76s" was ~0% of the actual wall. NEVER pick a target from nvprof's per-kernel `%`. The WALL truth = cProfile cumtime (it captures the blocking sync) + a full-fit wall A/B.
2. **cProfile is blind to async GPU compute; it surfaces at the sync.** GPU compute time shows up as tottime in the blocking `.get()` / `copy_to_host` / `safe_cuda_api_call` that waits on it. A big `.get()` tottime is usually compute-WAIT, not transfer volume — do not "optimize the transfer" when it's really the kernel you're waiting on (and vice-versa: confirm which).
3. **Isolated-kernel & wrong-shape microbenches lie end-to-end.** A 7.2x coalescing win on the kernel was 0.99x on the full function (D2H + the CPU histogram-reduction dominated). And a synthetic K=3888 noise-gate call took 5.5s while the REAL fit's calls average 0.8s — unrepresentative shape ⇒ worthless A/B. Capture the real `(n, K, K_y)` the hot function sees, and always confirm the win moves the FULL-FIT wall before shipping; revert if it doesn't.
4. **`nvprof --print-gpu-summary` (kernel timing) needs NO admin.** Only `--metrics`/`--events` (perf counters) need elevation (`RmProfilingAdminOnly` → `ERR_NVGPUCTRPERM`; elevate via `Start-Process -Verb RunAs` / one-shot UAC). CUPTI dll: `export PATH=".../extras/CUPTI/lib64:$PATH"`. For "which kernel eats GPU time" the no-admin summary suffices.
5. **Quiet the machine + bypass the replay cache.** Same fit measured 60s vs 147s under concurrent GPU load — check `nvidia-smi` (util + free mem) + own idle python procs before timing. And MRMR memoizes by X-content-hash, so a 2nd identical fit returns in ~0.1s (cache replay) and cProfile then profiles the REPLAY — use NOVEL-seed cold fits.
6. **A single launch-batch is 0x on the wall, but AGGREGATE launch-count reduction across the WHOLE pipeline IS a real wall win — don't over-generalize the microbench.** Point 3's "launch-batching was 0x on the full function" is true PER-KERNEL; it does NOT mean kernel launches never gate the wall. When the STRICT-mode FE path issues thousands of tiny cupy primitives (every `cp.bincount`/`cp.where`/`cp.log`/`.max()`/elementwise = a separate `cuLaunchKernel`), the aggregate launch + per-op overhead + the comparison-`MergeSort` time inside `cp.percentile`/`cp.unique` DOES dominate. Measured (F2 300k STRICT, 2026-06-25): replacing the per-call cupy MI/CMI/entropy chains with fused `cp.RawKernel`/`ReductionKernel`/`ElementwiseKernel` (one launch does the histogram+entropy of ~10 primitives) and routing `cp.percentile` binning to the sort-free radix-select edges took cuLaunchKernel 16451 → 7651 (−53%) AND the wall 24.63s → 18.68s (−24%) — selection-equivalent throughout. So the corrected rule: a one-kernel coalescing/batching tweak rarely moves the wall (point 3), but FUSING AWAY the launch COUNT pipeline-wide (and killing inherent sorts) does. Always confirm on the full-fit wall A/B (still the arbiter), and report BOTH the launch count and the wall.

## joblib threading over GPU-bound tasks = contention + sleep-poll, NOT parallelism (CRITICAL — 2026-06-20)

`joblib.Parallel` parent runs a `_retrieve` loop that **busy-`time.sleep`s** waiting for workers. If the tasks bottleneck on the **single GPU**, threading does not parallelize them — every worker contends for the one device, total work gets SLOWER than serial, and the parent burns the wait as `time.sleep`. Measured (canonical n=100k FE pair-search, default multi-worker): **105s wall of which ~84s was `time.sleep` in joblib `_retrieve`**; the serial-main-thread path (no joblib, prange kernels, GPU work from ONE thread) = 54s — a **1.9x regression** the multi-worker default was silently paying.
- **Diagnose:** `cProfile … print_callers("time.sleep")` points straight at the joblib `_retrieve`. A huge `time.sleep` tottime in a parallel fit = workers blocked on a shared resource, not real compute.
- **Fix:** GATE parallel-vs-serial on whether the GPU path is active — GPU-bound stage → serial-main-thread (one thread drives the device; CPU kernels still `prange` across all cores); small-n / CPU-only stage → keep `backend="threading"`. The regimes don't conflict (different n), so a stale "threading beats serial" bench (measured CPU-only) does NOT justify threading once the task body moves onto the GPU — re-validate it then.
...
New or edited code must type-check under strict mypy the first time, not after a later cleanup pass. The multi-thousand-error backlog this rule closes out (3979 -> 0 over one long sweep, mirrored in `pyutilz`) exists ONLY because code was routinely written without this discipline; do not add to a fresh backlog.
