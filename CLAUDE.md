...
**Black repo-wide reformat — resolved (2026-07-05):** user reviewed the major diff classes and decided to exclude exactly two from any Black run, project-wide, in both repos: (1) arg/collection-list explosion (multi-item packed line -> one-item-per-line, including `from x import (...)` blocks) and (2) blank-line insertion (pure or bundled with other changes in the same hunk). Neither is configurable via any stock Black flag (tested `--skip-magic-trailing-comma`: no effect on explosion; blank-line rules have no flag at all). Built `scripts/black_filtered_apply.py` (present in both repos) to apply everything else Black wants (collapses, quote style, blank-line removal, docstring reflow, redundant-paren removal, whitespace reflow) while mechanically rejecting those two classes via a diff-opcode-level filter, validated via AST-equivalence + compile checks on samples before any real file was touched. `black --check`/the vanilla `Black (filtered)` CI workflow are both advisory. Use `python scripts/black_filtered_apply.py --config pyproject.toml --write <files>` (or `--check <dirs>` for CI/dry-run) going forward instead of raw `black`/`black --fix` -- never raw `black --write`/`black .` in place, since that would apply the two excluded classes too.
