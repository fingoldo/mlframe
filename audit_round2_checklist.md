...
[x] SA27 `training/composite/discovery/_stability.py`+screen winner's-curse, no honest holdout — RESOLVED (index-carved honest holdout default 0.2, re-scores winner on fresh rows; biz_value shows in-screen gain inflated vs ~0 honest on pure noise; leakage test)
...
[x] CPX12 `_orthogonal_scorer_auto_fe.py:199`/`_orthogonal_three_gate_mi_fe.py:315` discard batched kernels — RESOLVED (batch separable scorers, 2x exact); three_gate RESOLVED (leakage-preserving batched train-edge binner, 1.2-2.2x <=16k rows, bit-identical OOF MI + leakage sensor)
[x] CPX13 `_stability_cluster.py:112-117` O(p²) Python double-loop — RESOLVED (triu+union-find, 9-15x identical clusters)
[x] CPX14 `inference/explainability.py:139,156` per-fold SHAP on entire dataset — RESOLVED (opt-in shap_oof per-fold X_test, bit-identical OOF matrix ~kx cheaper; default contract preserved)
