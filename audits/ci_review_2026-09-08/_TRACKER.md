...
| L1.5 | `detect-secrets` / `shellcheck` run only in pre-commit | **FUTURE** | Real for a public repo taking outside PRs. The cheap fix the agent proposes (one `pre-commit run --all-files <hooks>` job) needs its own scope check — `detect-secrets` against `.secrets.baseline` in CI has a different false-positive profile than locally. **Next action:** add the job with those hooks only, non-blocking for one week, then flip. |
...
| L1.12 | Baselines drain-warn but never require draining | **FUTURE** | The mechanism change is right, but the agent's own caveat is the blocker: the large baselines are `file:line`-keyed, so making `fixed` a failure turns every unrelated line-drift into a red gate. **Next action:** apply the strict form to the nine zero-entry baselines first (where it is free), measure the churn on one large one over a week, then decide. |
...
| L1.14 | `test_precommit_ci_scope_parity.py` covers 4 paired hooks, not the ruff pair | **FUTURE** | Correct, and the agent identifies the real obstacle: both hooks share the id `ruff`, so the pairing has to key on `(repo, files/exclude)` instead. That is a rewrite of the test's matching logic. **Next action:** re-key `PAIRED_HOOKS` on the tuple and add the ruff pair. |
...
| L3.2 | `mypy: strict_equality = true` | **FUTURE** | Genuinely valuable (always-False comparisons are a silent-correctness class nothing here covers), but the agent did not run whole-project mypy and neither has this round with the flag on — turning it on blind would land an unknown number of errors on a blocking zero-error gate. **Next action:** run mypy once with `strict_equality` on, triage the count, then enable. `warn_unreachable` stays off: this codebase's optional-dep `try/except ImportError` gates are exactly what it over-reports on. |
| L3.3 | `pip-audit` blocking rather than advisory | **FUTURE** | The reasoning is sound — the CVE floors are hand-maintained and go stale silently — but a new advisory with no patched release yet hard-blocks every push. It needs the `--ignore-vuln` list built first, which is a triage pass over the current findings. **Next action:** run `pip-audit -r pyproject.toml`, build the ignore list, then flip. |
