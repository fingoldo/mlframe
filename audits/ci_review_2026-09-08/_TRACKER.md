...
| L4.2 | `[tool.importlinter]`: 2 contracts, advisory, and its own config says so | **PARTIALLY RESOLVED** | Contract #1 ("kernel-sibling modules must not import back their facade") ported to `test_no_sibling_backimport_to_facade.py`, mirroring `test_no_inbound_edge_to_benchmarking.py`'s already-established pattern for contract #2 exactly (same top-level-only AST scan, same worktree-identity guard). Both contracts now have a real blocking gate; `[tool.importlinter]` itself is NOT yet deleted -- its own comments on contract #2 already say the section is documentation-only once the meta-test exists, so removing the section and the `import-linter` dep is a small mechanical follow-up, deliberately left for a separate commit so this one stays reviewable as "add the gate" rather than "add the gate and touch pyproject/pre-commit/dev deps in the same diff". **Next action:** delete `[tool.importlinter]`, its pre-commit hook, and the `import-linter` dev dependency. |
