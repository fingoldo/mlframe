...
| Disposition | # | Finding | Notes | Note |
|---|---|---|---|---|
| **RESOLVED** | D1 | Shard durations artifact name omits `matrix.os`, so three OS legs collide | Name now carries the OS. Latent only because macOS shards died earlier and Windows was cancelled. | `1121abf65` |
| **RESOLVED** | D1b | Durations merge is last-write-wins across platforms | The download pattern is narrowed to `COVERAGE_OS`; balancing on a blend of three platforms is not reproducible. | `191466f53` |
| **RESOLVED** | D2 | `ci-required` passes vacuously when `matrix-setup` fails or a gate is skipped | Added `matrix-setup` to `needs` and made `skipped` a failure. Severity was higher than the agent could know: master's branch protection **does** require "CI required checks" (verified via the API), so this was a green required check over zero tests. | `1121abf65` |
| **RESOLVED** | D3 | `numba-coverage.yml`'s serial job lacks the `workflow_run` gate | Copied the sharded job's condition verbatim. | `191466f53` |
| **REJECTED** | D4 | `dependabot-auto-merge` cannot merge (inferred read-only token) | Disproven by evidence. Run 34230983189's log shows `Contents: write`, `PullRequests: write` — no downgrade — and `gh pr merge --auto` exits 0. PRs #23 and #26 merged through this workflow. The unmerged github-actions-group PRs are superseded ones dependabot closed itself. No change. |  |
| **RESOLVED** | D5 | `release.yml` publishes to PyPI from a dispatch on any ref | `publish` is now `if: github.event_name == 'release'`. | `191466f53` |
| **RESOLVED** | D6 | Reusable workflows on the movable tag `@v1` while every action is SHA-pinned | All nine pinned to `915217a4a8cd…` (the commit `v1` dereferences to). The composite actions were already SHA-pinned. | `191466f53` |
| **RESOLVED** | D7 | Three pushers to master with no rebase and no shared lock | Two remain after W4; both rebase-and-retry and share `concurrency: test-durations-commit`. | `191466f53` |
| **RESOLVED** | D8 | Durations committed from failed/cancelled shards | Commit step requires a green test job; uploads stay unconditional. | `191466f53` |
| **RESOLVED** | D9 | `main` is a trigger branch several jobs ignore | `git ls-remote --heads origin main` is empty; dropped from all seven workflows. | `191466f53` |

### Gaps

| Disposition | # | Finding | Notes | Note |
|---|---|---|---|---|
| **RESOLVED** | G1 | No lockfile / no pinned resolution | `uv.lock` committed, 594 packages over Python 3.9.2-3.14. Every blocker it hit was a real defect in the declared metadata, not an artefact of locking — four of them, itemised in Round 2 below. Gated by a new `lockfile` CI job (`uv lock --check` plus an actual `uv sync --frozen`), and dependabot moved from the `pip` ecosystem to `uv` so a bumped dependency and its lock arrive in one PR. |  |
| **RESOLVED** | G2 | No lowest-supported-version resolution job | New `dep-floors.yml`: weekly + on `pyproject.toml` PRs, `--resolution=lowest-direct` on Python 3.9, install + import smoke + a scoped test slice. Non-blocking until the failure count is known. | `191466f53` |
| **RESOLVED** | G3 | No aggregated test-failure report | Every shard writes JUnit XML; a non-gating `report-tests` job renders one table into the run summary. The renderer was run against a synthetic report first. | `191466f53` |
| **RESOLVED** | G4 | Build/install smoke is Linux-only | New `install-smoke` job installs the built wheel on all three platforms and gates. | `191466f53` |
| **RESOLVED** | G5 | No release provenance / SBOM | `actions/attest-build-provenance` added to `release.yml`. SBOM rejected for now: nothing consumes one here, and it is ceremony until the distribution can actually be published (X1). | (provenance) `191466f53`; **REJECTED (SBOM)** |
| **REJECTED** | G6 | Nothing verifies the workflows themselves | Premise false. `actionlint` runs as a blocking pre-commit hook (`.pre-commit-config.yaml:149-152`), alongside yamllint and zizmor. The agent checked only `.github/workflows/`. Ran it against every change in this round: clean. |  |
| **FUTURE** | G7 | `deep-nightly` durations are never consumed | The workflow's own header already records this as deliberate and says to wire it once the run is green. It is not green yet, and balancing 20 shards on timings from a never-completed suite is guesswork. **Next action:** add the merge-and-commit job after the first fully green deep run. |  |

### Waste

| Disposition | # | Finding | Notes | Note |
|---|---|---|---|---|
| **RESOLVED** | W1 | `deep-nightly` has no uv cache: 20 cold multi-GB resolves nightly | `enable-cache` + `cache-dependency-glob`, matching every other uv call site. | `191466f53` |
| **RESOLVED** | W2 | `cache: pip` on setup-python while installing with uv | Dropped from `deep-nightly`; kept in `numba-coverage`, where the installs really are pip. | `191466f53` |
| **RESOLVED** | W3 | No `paths-ignore`: a README edit runs 30 shards | Applied to `push`, deliberately **not** to `pull_request`. The agent justified it with "this repo has no required checks" — it has three, so a required check that never runs leaves a docs-only PR permanently pending and unmergeable. Push is where the spend is anyway. | (push only) `191466f53` |
| **RESOLVED** | W4 | `update-test-durations.yml` is superseded and cannot finish | Deleted. Its own header records the supersession; `.test_durations` has 30 131 entries, all from the ci.yml path. | `191466f53` |
| **SUPERSEDED** | W5 | Four near-identical pyutilz install stanzas | Originally dispositioned as "extend `install-pyutilz` to cover the CUDA cases, then convert all of them" — which was wrong, and the user caught it. The seven hand-synced SHA copies exist because there was no lock; adding machinery to keep copies in step treats the symptom. With `uv.lock` the resolved commit is recorded once, machine-side. **Next action:** move the CI installs to `uv sync --frozen` and retire the `pyutilz-ref` inputs entirely. See M1. | by G1 |
| **RESOLVED** | W6 | `codecov-full.yml` has no `concurrency` group | Added, `cancel-in-progress: true`. | `191466f53` |

### Tools table

| Disposition | Tool | Note |
|---|---|---|
| **FUTURE** | `uv lock` + `uv sync --frozen` | see G1. |
| **REJECTED** | `actionlint` | already running, see G6. |
| **RESOLVED** | `--junitxml` + summary | see G3. |
| **RESOLVED** | `attest-build-provenance` | see G5. |
| **RESOLVED** | `--resolution=lowest-direct` | see G2. |
| **REJECTED** | `step-security/harden-runner` | audit mode produces an egress log nobody is assigned to read, on a repo whose dependency intake is already gated by dependabot, dependency-review and pip-audit. Revisit if a supply-chain incident gives it a reader. |
| **RESOLVED** | `pytest-timeout method=signal` | `191466f53` — `signal` on POSIX, `thread` on Windows (no SIGALRM). |
| **REJECTED** | `--max-worker-restart` | it converts a native crash into a silently restarted worker. This repo's own convention treats a crash as a bug detector; masking it is the opposite of the fix. |
...
| Disposition | # | Finding | Notes | Note |
|---|---|---|---|---|
| **DOC** | F1 | The three codecov flag badges are well-formed | Verified clean by the agent; no action. Residual risk it names (a flag with no data renders `unknown`) is real and unaddressed — recorded, not fixed, because the fix is "keep the nightlies green". |  |
| **RESOLVED** | F2 | Four uninterpretable coverage badges in the header | One headline badge in the header; all four moved under Testing with a line each. Descriptions checked against the workflows, not inferred from flag names. | `e441b93fc` |
| **REJECTED** | F3 | The `docs` badge may link to a 404 | Fetched `https://fingoldo.github.io/mlframe/`: live, renders "mlframe documentation index". No change needed. |  |
| **RESOLVED** | F4 | Three workflow badges are branch- and event-agnostic | Modern path form, `branch=master`, links filtered to master. `event=push` added only after checking each workflow's `on:` block; omitted for sklearn-matrix, whose push trigger is paths-filtered. | `e441b93fc` |
| **RESOLVED** | F5 | All three flag badges link to the same undifferentiated page | Superseded by the F2 restructure, as the agent expected. | `e441b93fc` |
| **RESOLVED** | a–e | Missing badges: Python versions, Ruff, CodeQL, pre-commit, py.typed | All five added; each verified against what the repo actually declares. | `e441b93fc` |
| **DOC** | — | Badges NOT to add (PyPI, conda, Scorecard, Codacy, Contributor Covenant, decoration) | Agreed, and X1 makes the PyPI ones actively harmful: `pypi/v/mlframe` would render a stranger's 0.1.15. |  |
| **RESOLVED** | S1 | The install section recommends a PyPI install of a package not on PyPI | Now the editable local-path form every sibling line uses. | `1121abf65` |
| **RESOLVED** | S2 | No "why", no runnable example above the fold | Tagline, Why (five substantiated claims), Quickstart, Documentation section. The Quickstart is the existing worked example trimmed — a first draft written from memory had the wrong import path, arguments and return shape. | `e441b93fc` |
| **RESOLVED** | S3 | No table of contents in a 715-line file | Added after the badges. | `e441b93fc` |
| **RESOLVED** | S4 | `docs/README.md` mixes user guides with research notes | Research block behind a `<details>` fold. Also fixed NESTED_PARALLEL.md, which was sitting above that section's intro as a bare bullet among tables. | `e441b93fc` |
| **RESOLVED** | S5 | CHANGELOG `[Unreleased]` has two `### Fixed` headings | Two `### Fixed` **and** two `### Added`. Merged into Keep a Changelog's order; all 103 entries preserved, counted before and after. `## [0.9.0]` does exist, so the agent's second worry is clear. | `e441b93fc` |
| **FUTURE** | P4-a | README's relative links and images break on the PyPI page | Correct, and all six PNG paths verified to exist. Not done now for two reasons: the distribution cannot be published under this name at all (X1), and absolutising every cross-file link degrades the mkdocs site, which consumes this same file. **Next action:** do it in the same change that settles the distribution name, and check the rendered mkdocs output. |  |
| **DOC** | P4-b | `pyutilz>=1.0.0` is a hard runtime dep not on PyPI | Confirmed: `pypi.org/pypi/pyutilz/json` returns 404. Nothing to fix in the metadata; it gates X1. |  |
| **RESOLVED** | P4-c | `description` is 178 chars, PyPI truncates it | Now 130, distinguishing part first. | `e441b93fc` |
| **REJECTED** | P4-d | No MIT license classifier, and there should not be | The agent's own recommendation is "do not fix"; recorded so it is not re-flagged. | (correctly) |
| **RESOLVED** | — | Incidental: `codecov.yml` comments name a python-3.11 leg that no longer exists | Comments now name `REPRESENTATIVE_PYTHON` / `COVERAGE_OS`. | `1121abf65` |
