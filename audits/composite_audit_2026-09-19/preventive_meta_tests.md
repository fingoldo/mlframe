...
- **Repo**: mlframe.
- **Disposition**: RESOLVED. (c) "the gate can fire" and (b) the cap keeping the blend's level are pinned in tests/training/composite/ensemble/test_combiner_invariants.py; they fixed EST-05 and EST-06. (a) the failing-component mean is already covered by EST-03's resolved tests. Not built: the strategy-registry parametrisation over every constructor, the meta-guard, and the (d) row-disjointness spy. COMPLETED (2026-09-24): registry parametrisation built in test_combiner_invariants.py over STRATEGIES (mean, oof_weighted, nnls_stack, linear_stack and the five build_meta_stack_ensemble learners): (a) one component raising at predict keeps the served mean within 5% for all nine; (b) capping to two components (stacks refit as the suite does) keeps it within 2% for the four weight-based strategies (the meta-stackers' fixed K-column input makes capping undefined, stated in the test); (c) the gate value is never better than the best single on 1 good + 20 noise components for the four suite strategies (oof_weighted already falls back and ties); (d) through the PMT-29 ledger, every cross-fitted fold of the two stacks reports on OOF rows disjoint from its weight-fit rows, each row once (mean has no fitted weights; oof_weighted's are K RMSE statistics). A meta-guard requires every from_* constructor, every config strategy and every META_STACKER_KINDS learner to be in the registry.
...
- **Disposition**: RESOLVED. Leg (a) built: test_frame_carrier_parity.py checks OOF holdout alignment with an identity component (pandas vs polars; monotone / reversed / shuffled time; k=1 and 3) and three row slicers against pandas under all three index orders. It found and fixed EST-07 plus the same order-losing mask in `feature_stacking`, `_slice_frame_rows`, `_row_select` and `_subset_rows`. Not built: the meta-guard registering every `(X, idx)` function that branches on polars, and leg (b), the shared `order_losing_filters` scanner. A grep for mask-from-index next to `.iloc` found 7 functions, and the 5 with a real order dependence are fixed. COMPLETED (2026-09-24): the meta-guard and leg (b) are built. test_frame_carrier_parity.py registers every function under training/ whose first two parameters are a frame and an index and whose body branches on polars (ten row slicers, each run for pandas/polars parity under monotone, reversed and shuffled indices; four non-slicers with the reason), failing on unregistered and stale entries. Leg (b) is py_ci_shared.order_losing_filters (e59cdec, pinned): it flags a function that selects rows positionally (.iloc/.take/.gather) with an index and also by a boolean mask built from that index; on the commit before the carrier fix it flags all five helpers repaired then, and on today's tree it found fit_stacked and fit_stacked_on_residual pairing y[train_idx] with a frame-order polars filter (fixed, with a regression test that fails by 23.8 pre-fix); the wired test requires zero hits.
...
- **Repo**: mlframe.
- **Disposition**: RESOLVED. New tests/training/composite/transforms/test_transform_canonical_dgp.py. Every base transform declares a canonical DGP family in `_CANONICAL_DGP` (additive, linear, second difference, product, multiplicative, geometric or saturating), and a meta-guard keeps the keys equal to the base transforms in the registry. Absorption is measured scale-free, at n in {300, 2000} and, for the additive-type families, with the base offset by 1e4: inverting the constant median T at each row's base must explain at least 90% of y's variance (80% for the product family). The test found a TRF-03-class defect in `quantile_residual`, now fixed. At n=300 its ten bins of 30 rows sat under the 50-row minimum, so every bin fell back to the global median and T ignored the base (R2 0.00). The bin count is now `min(n_bins, n // min_bin_n)`, which gives R2 0.96. Not built: the smoother-improves-with-n leg and the grouped-level leg. COMPLETED (2026-09-24): the smoother and grouped legs are built in test_transform_canonical_dgp.py. Smoothers (NW, spline, monotonic, quantile, median, rank, rank-ECDF): the curve error against the noise-free truth at n=20k must be at most 0.7x its n=2k value; polynomial_residual_deg2 and gaussian_copula_residual are excluded with the reason (fixed functional form, bias flat in n), and a meta-check classes every saturating transform. The leg found quantile_residual and median_residual flat (ratio 0.97-0.98: fixed 10-20 bins); their bin count now grows like n^0.3 (bins_for_rows, bounded so params stay within 2x per 10x rows), ratios 0.51, with downstream composite RMSE unchanged at 2k and 1.4-3% better at 20k-100k. Grouped level leg: twenty identical lognormal groups must sit within 0.05 sd of the ungrouped twin's level for ewma, linear, monotonic, quantile and rolling_quantile_ratio grouped (frac_diff_grouped excluded: a recursion, not a level). It found monotonic_residual_grouped at +0.075 sd from the cumulative-max monotonisation of noisy knot medians; weighted isotonic regression gives 0.025.
...
- **Repo**: mlframe.
- **Disposition**: RESOLVED. New tests/training/composite/test_composite_suite_contracts.py runs ONE module-scoped suite (TVT fixture, linear + lgb, `transforms=['linear_residual','additive_residual']`, `max_total_composite_targets=1`, 49 s for all seven tests). Built legs: (a) every trained slot reloads via `load_mlframe_suite` with the same model types, and `predict_from_models` on the reloaded suite equals the in-memory one key by key; (b) the saved metadata covers every returned key, and the CT-ensemble metrics match; (c) exported spec names equal the trained composite keys, and the capped spec is a failure with the cap reason; (e) every spec's transform is in the whitelist; (f) `predict_mlframe_models_suite` composite predictions are finite and on the y scale; (g) the targets-table composite rows carry `scale='y'` and the recorded y-scale test RMSE; (j) no composite logger emits a failure at WARNING. Proof: deleting `persist_after_composite_post(ctx)` fails (a) with `_CT_ENSEMBLE__target` missing on disk. Fixed to make the legs hold: INT-07, INT-08, INT-11, EST-13. Not built: (d) the precomputed-bundle rerun; the additive `RMSE_y == RMSE_T` identity in (f); (h) and (i) as suite variants (unit-level in test_extreme_ar_skip_decision.py and test_composite_post_moe_value_report.py); and the TST-17 rewrite of test_composite_integration.py onto this fixture. COMPLETED (2026-09-24): remaining legs built in test_composite_suite_contracts.py. (d) a module-scoped rerun hands the first run's specs back as TrainMlframeSuitePrecomputed(composite_target_specs=...) and must train exactly the same composite keys. (f) the additive identity: the per-model hook's val/test frames are captured, and for every additive composite y_hat - y must equal the inner's stored T_hat - T row by row (1e-9) on every row the out-of-range base soft-shrink leaves alone (the one test row it shrank is the only row that differs; clip counters are zero). (h) and (i) run in a grouped variant with the MoE gate on: a spy on the dummy-baseline tag site must classify every composite key MTRESID and the raw target MTTR, and cross_target_ensemble_metrics val/test RMSE must equal the RMSE of the model the returned suite ships. Building (i) found the MoE gate a silent no-op whenever the group column is bookkeeping only (an extractor's group_field is dropped from the features); it now says so once at INFO. TST-17's rewrite of test_composite_integration.py onto a shared fixture shipped earlier; both files now run 20 tests in 79 s.
...
- **Disposition**: RESOLVED - (a) ships as tests/training/composite/discovery/test_report_reasons_from_ledger.py, parametrised over the RejectStage vocabulary so a new gate is covered as soon as it writes a ledger row, and (c) as tests/training/core/test_env_signature_drift.py. (b), the printed-advice scanner, belongs in py-ci-shared and is still owed; the two advice strings this audit named (DSC-26, INT-19) are fixed, so it has no failing subject left here COMPLETED (2026-09-24): (b) built: py_ci_shared.printed_advice (94b1c0d, pinned) finds every literal passed to a logging call, warnings.warn, print or an exception that advises an action, keyed <path>::<enclosing function>#<n>. tests/training/composite/test_printed_advice.py follows each of the 21 advising messages in composite/ and observes the promised effect, and tests/test_meta/test_printed_advice_wired.py fails on an unregistered or stale key. Following the advice found a live defect: the leak-corr filter ran in float32, so a near-copy of y (true |corr| 1 - 1.3e-9) scored 1.00000018 and raising forbidden_base_corr_threshold, as its log advises, could never keep it; near-threshold columns are now recomputed in float64 and |corr| is capped at 1.
