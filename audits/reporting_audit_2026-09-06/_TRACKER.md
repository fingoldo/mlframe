...
| `visual_walkthrough.md` | 11 | 3 | 3 | 5 | 0 | 0 |
| **Total** | **75** | **63** | **4** | **5** | **2** | **1** |
...
| **TODO** | P2 | `WALK-05` | statistic-bearing panel titles wrap to two or three lines | Titles are still built by concatenation (e.g. `charts/quantile.py` interval coverage); what exists is measured wrapping and a reserved title band. Remains: a length budget for statistic-bearing titles, or moving the statistics to the caption. |
| **RESOLVED** | P2 | `WALK-06` | Spearman heatmap reads as anti-correlation | Same defect as VIS-01. `test_heatmap_scale_and_notice.py`. |
| **TODO** | P2 | `WALK-07` | an empty panel is captioned for a metric never chosen | `metric` defaults to None now, but `_headline_metric` (`charts/model_comparison.py`) still returns `""` when any model has no `metrics`, and `_leaderboard_panel` then spends a panel on "metric '' missing on all models". Remains: drop the panel or say that the models carry no metrics. |
| **TODO** | P2 | `WALK-08` | unequal grid cells and a ragged last row | `pack_panels` (`charts/_layout.py`) still pads the last row with `None`, leaving a lone left-aligned panel. Remains: centre or span the last row, and re-check the narrower regression scatter. |
| **RESOLVED** | P2 | `WALK-09` | heatmap axis labels full-length on both axes | Same defect as LBL-03. `test_heatmap_tick_budget.py`. |
| **TODO** | P3 | `WALK-10` | red/green is the only channel on the WoE bars | Never dispositioned. `charts/category_discriminability.py` says the bar direction either side of WoE=0 already encodes the sign, which would make colour not the only channel. Remains: confirm by render and record the disposition. |
| **TODO** | P3 | `WALK-11` | suptitle and single panel title repeat each other | Still reproduces: suptitle `Category discriminability (\|WoE\|)` and panel title `Category discriminability (signed \|WoE\|; ...)` in `charts/category_discriminability.py`. Remains: drop the repeated lead from the panel title. |
