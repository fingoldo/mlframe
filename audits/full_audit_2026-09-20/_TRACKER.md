...
| `_nondiscriminating_assert_baseline.json` carries 8 entries only py-ci-shared v1.17.0 reports (six baseline-write-on-first-run skips and GPU/feature-probe skips, one median canary) | pass under the pinned v1.17.0 | OPEN with the pin bump: py-ci-shared 9b48122..HEAD fixes those scanner false positives (baseline-`exists()` and GPU-capability probes are environment probes; a median next to `assert_allclose` is a canary); after the pin moves, refresh this baseline so they leave it |
