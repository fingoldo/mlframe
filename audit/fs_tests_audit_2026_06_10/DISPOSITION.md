...
### Improvement-loop round 4: PB-5 (RFECV pure-noise FP control) -- ATTEMPTED + REJECTED (measured tradeoff)
A rule-resolution-layer fix (in 'auto', reject all features when the best evaluated subset cannot beat the no-features N=0 dummy) was implemented + measured across pure-noise / strong-signal / weak-signal fixtures (RF, n=400 p=15, 3 seeds). PURE NOISE flipped 15 -> 0 (correct) with strong/weak DETECTABLE-signal recall bit-identical, BUT it also rejected RECOVERABLE signal on two real-signal fixtures (6-informative min-aggregation 12->0; recency sample-weighted 2-feature 2->0). Root cause: RFECV's "all-features can't beat the dummy" early-exit stops the search at {N=0, N=full} on BOTH pure noise AND noise-diluted-but-recoverable signal -- indistinguishable at the rule-resolution layer. REVERTED (shipped nothing); the real fix requires an OUTER-LOOP search change (explore smaller subsets before concluding). Bench: wrappers/_benchmarks/bench_auto_rule_noise_fp.py; in-code note at _rfecv_stability_select.py:select_optimal_nfeatures_; gap pinned in test_biz_val_wrappers_rfecv.py + the PB-5 xfail left in place.

### Core accuracy: monotone-warp tie-break -- ATTEMPTED, two flaws diagnosed, REVERTED (needs downstream-gate re-validation)
Implemented a tie-break that, on an exact MI tie between a selected raw column and a non-selected strictly-monotone twin (e.g. f vs g=exp(4f)), prefers the more linearly-usable member (|corr(col, rank(col))| proxy). The warp-survivor win measured correctly (monotone-warp sensor 29 passed: f survives). REVERTED because it broke selection on quantization_nbins=5 (empty support_, the PB-6 class). Two distinct flaws found: (1) twin-detection on the COARSE-binned data codes false-fires (unrelated columns share a bin sequence at small nbins -> rank_corr>=0.99); fixed by detecting twins on RAW rank-corr instead -- but (2) the deeper flaw remains: the swap substitutes a NON-selected twin that downstream selection gates then reject, collapsing support. A correct fix must re-validate the swapped-in twin against the same downstream gates (relevance/redundancy/never-empty) the original passed, then verify against the FULL (slow) MRMR suite. Bench kept at filters/_benchmarks/bench_warp_linear_tiebreak.py; the monotone-warp xfail stays open.

### Core accuracy: partial-TS-leak audit -- ATTEMPTED, REJECTED (measured structural boundary)
Goal: make the gain-ratio leak audit (top-leak/top-legit gain >= 2x, fires on a direct corr~1 leak) also flag a PARTIAL look-ahead leak (lead1=s_{t+1}, corr~0.7). Measured proof (bench_mrmr_partial_leak_gain_ratio.py): NO gain-based threshold separates the partial leak from a strong LEGIT feature -- both sit at corr~0.7 / high gain, and MRMR is time-agnostic (no temporal order to exploit), so any threshold low enough to flag lead1 (<0.7) equally false-flags every corr~0.7 strong legit feature. The gain-ratio audit is structurally a DIRECT-leak detector; correlation-route partial leaks are the domain of RFECV's existing leakage_corr_threshold (which the sensor already exercises on the same fixture). Shipped nothing; the partial-TS-leak xfail stays open as a documented structural boundary, not a fixable bug.
