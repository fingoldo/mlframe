...
- **PB-6 (P1): MRMR returns EMPTY support_ (0-width transform) despite min_features_fallback>=1.** Reproduced on make_signal_plus_noise (linear, no redundancy), FE on: seeds 2 (0-width) & 4 (support_=0 but 1 engineered col) of 6. Violates the documented never-empty guarantee. Sensor: test_biz_val_h2h_external.py xfail. Investigated: the never-empty rescue at _mrmr_fit_impl.py:7185 doesn't fire for the 0-width seed — the speculative "include redundancy-dropped raw when n_engineered_out==0" fix did NOT resolve it (seed 2's empty result returns from a different branch than the 7045 rescue block), so it was reverted. FUTURE fix needs tracing the actual return path for the genuinely-empty (n_engineered_out==0) case; the xfail pins the regression meanwhile.
