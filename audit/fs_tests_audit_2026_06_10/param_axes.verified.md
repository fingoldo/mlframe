...
[Verifier note on the sampling statement: tests/feature_selection/ does contain 468 files and 97 test_biz_value_mrmr_layer*.py files — counts independently confirmed. However the FE-layer sample omitted layers 24/28/29/83, which are FE layers carrying regression + multiclass content; this omission materially affects finding 01 (see its verdict).]
...
VERDICT: CORRECTED — the cited file:line refs, source quotes (_mrmr_fit_impl.py:3024-3025/:3698/:757, _target_encoding_fe.py:156-157), the layer15/16 pins, and the 0-hit greps over the 18 listed FE layers all verify exactly. BUT the headline "ZERO regression-target and ZERO multiclass coverage" for the modern families is REFUTED for the largest modern family, fe_hybrid_orth: test_biz_value_mrmr_layer29.py fits MRMR(fe_hybrid_orth_enable=True, basis="hermite", top_k=5) on load_diabetes (continuous-y regression, TestDiabetesHybrid with R2 floors, :212-235) and on load_iris/load_wine (3-class multiclass); test_biz_value_mrmr_layer83.py runs ALL 10 fe_hybrid_orth scorer variants (plug_in/ksg/copula/dcor/hsic/jmim/tc/cmim/lasso/elasticnet via `_mechanism_kwargs`, :142-172) on 7 datasets including diabetes + fetch_california_housing (regression) and load_digits (10-class). Additionally tests/training/_fuzz_combo/axes.py crosses mrmr_fe_hybrid_orth_enable_cfg/mrmr_fe_kfold_te_enable_cfg/mrmr_fe_mi_greedy_enable_cfg (:1378,:1463,:1494) with target_type ∈ {regression, multiclass_classification, multilabel_classification} (:46-48) at smoke level. The finder's grep keywords (make_regression / "regression" / n_classes=) missed the load_diabetes/load_iris/load_wine/load_digits dataset loaders, and layers 24/28/29/83 were absent from its sample. Also minor: the "only 4 files touch make_regression" list is wrong on one member — test_biz_val_filters_mrmr.py contains 0 make_regression hits, while test_mrmr_classification.py and test_wrappers_rfecv_fi_semantics.py (both FE-flag-setting) DO contain make_regression. SURVIVING CORE (verified): the non-orth modern families — fe_kfold_te, fe_grouped_agg/quantile, fe_cat_num_interaction, fe_rankgauss, fe_rare_category, fe_temporal_agg, fe_composite_group_agg, fe_conditional_residual, fe_mi_greedy — have no regression-target or multiclass biz-value coverage anywhere in tests/ (independent grep: no file enabling these flags loads a regression/3+-class dataset). Scope the proposal to those families; severity P1→P2.
...
VERDICT: CONFIRMED — every factual claim independently verified: _SELECTOR_FACTORIES = [RFECV, MRMR] at test_selectors_shared.py:57-60 with the append-here docstring at :14; the 3-selector list at test_fs_selector_contract.py:71-75; registry.py registers all 4 at :146-151; _instantiate_rfecv (:75-105) AND _instantiate_boruta_shap (:106+) both wrap in GroupAwareMRMR with cluster_reduce=True default; both contract suites construct bare classes via direct imports (_make_rfecv at shared:37-43, _rfecv_factory at contract:43-49); BorutaShap/HybridSelector appear in neither suite (grep: 0 functional hits; the single "registry" hit in shared:34 is a comment word); production suite goes through registry.get (_setup_helpers_pre_pipelines.py:112,:138). Minor notes that do not change the verdict: hybrid_selector.py get_feature_names_out is at :532 (:523 is transform; class :73, fit :357 correct); the shared suite has 18 Test groups, not 17; the GroupAwareMRMR-wrapped RFECV/BorutaShap DO have dedicated drop-in tests outside the contract suites (test_groupaware_real_rfecv_dropin.py, test_groupaware_wraps_sklearn_selector.py, test_cluster_reduce_kwargs_reachable.py) — but none of those run the pickle/clone/refit/drift contract groups, so the finding's specific claim stands. Proposal coherent: registry exposes get() (:58) and available() (:64).
...
VERDICT: CORRECTED — the contract-level gap is real and verified: TestSharedInputTypes (:168-189) has only pandas+numpy methods, and grep for polars/pl. over BOTH contract suites returns zero hits; mrmr.py:2823-2826 quote verbatim-accurate; test_boruta_shap_polars_input.py and tests/training/test_mrmr_polars_fe.py exist; CLAUDE.md quote verbatim. Correction: the closing example overstates the blast radius — both named selectors' polars paths ARE witnessed outside the contract suites: RFECV polars transform is pinned in test_regression_w2b_rfecv_arrow_bridge.py:15 (test_rfecv_transform_polars_input_with_arrow_bridge) plus tests/training/test_feature_selection.py:530 (test_rfecv_with_polars), and ShapProxiedFS polars input is pinned in test_shap_proxied_fs_contract.py:68-69 (test_polars_input_supported). So a polars regression in those selectors would NOT go fully undetected; what is missing is the uniform selector-agnostic axis (and the pandas-vs-polars selected-set parity assertion, which no existing test performs). Finding stands with that narrowed motivation.
...
VERDICT: CONFIRMED — independently re-verified with multiple grep variants (`score_pair_mi`, `_mi_dispatch`, `mi_dispatch` over tests/): zero functional test calls; the only test reference to the module at all is a module-NAME string in test_biz_value_mrmr_layer52.py:69 (`(16, "mlframe.feature_selection.filters._mi_dispatch")` — a structural/import-budget check, not a call). Source verified: def at _mi_dispatch.py:61-66, docstring "Single entry point to all MI estimators." at :67, exactly 9 routed ids (plug_in :88, mixed_ksg/ksg_lnc :92, mine/infonet/mist :96, fastmi :100, median/genie :103), float64 coercion at :85-86, ValueError raise at :107 (finder said :108 — off by one, immaterial) with message "score_pair_mi: unknown estimator {estimator!r}" which matches the proposal's `match="unknown estimator"`. test_biz_val_mi_estimators.py:197 is indeed `test_alternative_estimators_via_standalone_modules` importing from `._ksg` directly.
...
VERDICT: CORRECTED — all cited facts verify exactly: mrmr.py:937 `dcd_enable: bool = True` and :891 `cluster_aggregate_enable: bool = True`; layer35:106-121 / layer101:205-209 / layer70:159-162 pins verbatim; 62 `dcd_enable=False` occurrences across layer files (exact count reproduced); `def _make_mrmr` in exactly 48 layer files; layer35:331-334 documents the L91-gate-vs-L35-AUC interaction; both headline tests exist (layer35:328, :354); layer64 (the other kitchen-sink, :232-233) also pins both off. Correction: the TITLE ("Entire 97-layer FE biz suite ... pins the production-DEFAULT redundancy mechanisms OFF") is overstated — independent grep shows at least 12 FE layer files (87, 88, 89, 90, 92, 93, 94, 95, 99, 104, plus 52, 55) construct `MRMR(fe_<mech>_enable=True, ...)` WITHOUT pinning dcd/cluster_aggregate, i.e. single-mechanism FE × production-default redundancy IS witnessed for the grouped/temporal/composite/rare/residual/rankgauss families; layers 15/16 additionally run the full default surface (FE off). The surviving (and verified) core: every ALL-FE-ON kitchen-sink/state-of-the-union contract (L35, L64, L70, L101) pins dcd/cluster_aggregate/friend-graph OFF, so the all-FE × prod-defaults interaction the proposal targets is genuinely unwitnessed. Retitle to "all-FE-on headline contracts" scope; proposal unchanged and coherent.
...
VERDICT: CONFIRMED — all claims independently reproduced: fit signature with groups=None, sample_weight=None at mrmr.py:2897; `sample_weight` appears in exactly 1 of 97 layer files (layer53) and `groups=` in 0; test_mrmr_sample_weight_unit.py:29+ covers none-vs-omitted/uniform/shape only with no FE flags (layer53 explicitly sets fe_hybrid_orth/mi_greedy/kfold_te = False at :92-94); _target_encoding_fe.py has 0 sample_weight hits, so MRMR-level weights cannot reach per-cell TE means — the "unpinned either way" characterization is accurate. Two contextual notes (do not change the verdict): MRMR-level groups handling IS tested outside the layer files but never with FE (strict_groups raise/warn-and-stamp in test_fs_groups_recipe_seed_fixes.py:32-42, test_regression_S30_mrmr_strict_groups.py, test_fs_pre_screen_group_seed.py); the fuzz suite axis `use_sample_weights_in_fs_cfg` (tests/training/_fuzz_combo/axes.py:478) can cross weighted FS with MRMR FE flags at crash-smoke level only — no equivalence/lift assertion anywhere. The proposed groups-smoke should account for strict_groups semantics (default path warns and stamps groups_ignored_ rather than "forwarding" — assert on that documented contract).
...
VERDICT: CORRECTED — the substantive coverage claim is fully verified: all 4 call sites (lines 26, 47, 50, 63 — exact) pass classification=True; `classification=False` = 0 hits anywhere in tests/; test_hetero_vote.py is the only test file referencing heterogeneous_relevance_vote; src refs verified (_default_panel regressor branch at :43-49, R2-skill in _cv_skill docstring/body at :62-68, public default classification: bool = True at :84). Exact correction: the file contains 3 test functions (test_hetero_vote_keeps_signal_drops_noise :21, test_skill_weighting_off_is_exactly_equal_weight :40, test_skill_weighting_on_runs_and_reports_weights :57), not "4 tests" — 4 call sites across 3 tests (one test makes two calls). The proposal text itself already says "the existing three behavioral tests", consistent with the corrected count.
...
VERDICT: CONFIRMED — every count and line ref independently reproduced exactly: `clone_preserves` in 46 of the 97 layer files; `def test_default_ctor_values` in exactly the 12 listed layers (62, 63, 65, 66, 67, 68, 69, 71, 72, 73, 74, 76 — list matches verbatim); test_default_off_no_ksg_columns at layer65:366 + test_pickle_roundtrip_preserves_ksg_recipes at :437, copula pair at layer66:348/:385, hsic pair at layer71:426/:466; layer104:434-554 holds the default_off (:434) / enabled_adds (:450/:474/:486) / pickle (:511/:526/:541) / clone (:554) family blocks; 62 distinct fe_*_enable flags in mrmr.py (exact). The "~150-180 near-identical functions" figure is an estimate consistent with 46 files x 3-4 shapes; not exhaustively recounted but plausible. Proposal coherent with source (flags exist as ctor params reachable via get_params()).
...
VERDICT: CONFIRMED — line refs exact (test_rare_category_no_y_leak :382, test_conditional_residual_no_y_leak :400, test_rankgauss_no_y_leak :417 in layer104); "leak" mentioned in exactly 42 of the 97 layer files (case-insensitive grep reproduced the count); independent grep for permuted/shuffled-y patterns (`y_perm|permut.*y|shuffled_y|y_shuf`) hits only 16 layer files, each with its own local fixture/threshold — no shared helper or uniform parametrized gate exists anywhere in tests/feature_selection (no file parametrizes a leak check over multiple FE-flag dicts). The "heterogeneous per-family recipes" characterization is consistent with what I sampled (layer104 uses per-family AUC-on-permuted-y; other layers use in-fold-vs-OOF comparisons).
...
VERDICT: CONFIRMED — fixture verified at conftest.py:219-240 (n_samples=50, 3 informative + 100 noise = p=103); within tests/feature_selection/ its only consumer besides conftest is test_wrappers.py (RFECV family). Note: tests/training/ has same-named tests (test_core.py:1108, test_integration.py:479, synthetic.py:192) but neither fits MRMR — test_core's runs the suite with ridge only (no use_mrmr_fs), test_integration's uses RidgeCV directly — so they do not refute the claim. layer20:410-411 verified: test_p500_walltime_budget at N_ROWS=1500, p=500 (n>p). Active refutation attempts found no p>n MRMR fit anywhere: small-n candidates checked individually — test_dcd_adversarial_mrmr.py:79 is n=40/p=3, layer25:208 n=50/p=2, layer53 n=50 is a partial_fit batch — all n>p. Claim survives.
...
VERDICT: CONFIRMED — body verified verbatim at tests/training/test_feature_selection.py:137-159: `for method in ["quantile", "uniform"]:`, MRMR ctor kwargs as quoted, `try: selector.fit(...); assert hasattr(selector, "n_features_in_") except Exception as e: warnings.warn(f"Quantization method {method} failed: {e}")`. The zero-power characterization is exact — since AssertionError is an Exception subclass, even the lone assert is swallowed by the except; the test literally cannot fail. The rfecv quartet verified at :171 (basic_regression), :191 (basic_classification), :211 (catboost_regressor), :235 (catboost_classifier, ending ~:249).
...
VERDICT: CORRECTED — line refs verified (MECHANISMS :97-110, LINEAR_DATASETS :111, matrix fixture build :310-326, test_full_matrix_populated_no_nans :335-344, per-dataset loops :444-468). Exact correction to evidence (a): test_full_matrix_populated_no_nans does NOT abort at the first broken cell — it accumulates ALL non-finite (ds, mech) pairs into a `bad` list and asserts once with the full list (`bad.append(...)` then `assert not bad`, :338-344); test_best_mechanism_score_geq_median uses the same accumulate-then-assert pattern (:453+). Only some checks assert inside the loop (e.g. test_best_mechanism_is_in_roster :444-447 fails on the first dataset). Evidence (b) stands in full: no per-cell test ids exist, so selective rerun of one (dataset, mechanism) cell via `-k` is impossible and a red run re-executes the whole 70-cell matrix. The proposal remains worthwhile for (b) alone; drop claim (a) for the headline test.
...
VERDICT: CORRECTED — verified: float32 appears in exactly 2 of 97 layer files (layer15 — test_float32_y_matches_float64_support at :469, y-dtype only — and layer16, same shape); TestSharedDtypeVariety at test_selectors_shared.py:560-582 uses int32/float32 X with only `n_features_ >= 1` asserts; test_fe_narrow_code_dtype.py exists and targets internal bin-code dtype. Exact correction: "engineered-recipe REPLAY numerics under float32 inputs are never pinned" is too broad — test_mrmr_engineered_replay.py fits, replays on a disjoint test set, and pickle-round-trips LEGACY `fe_max_steps` recipes on float32 input frames (fixture builds all columns as np.float32 at :38-45; recipes carry quantization_dtype=np.float32 at :283/:322), and test_numerical_stability.py pins discretize_array float32-vs-float64 bin-assignment equivalence. What is genuinely unwitnessed (verified by grep over the FE-mechanism test files): float32 fit+replay for the MODERN mechanism recipes (orth/Hermite bases, rankgauss erfinv, mi-greedy ratio/log families) and any f32-vs-f64 selected-name-set or engineered-value parity assertion. Scope the finding to modern mechanisms + the parity axis; proposal unchanged.
...
VERDICT: CONFIRMED — read in full at layer35:395-453: `_run_one_mech` helper at :395-401, then exactly the 4 named methods (:403, :415, :429, :443), each body verbatim `auc = self._run_one_mech(fixture, **kwargs)` + `assert auc - baseline_auc >= 0.02` differing only in mech kwargs and message text. Layer104:434-554 family blocks verified under finding 08 (default_off :434 / enabled_adds :450,:474,:486 / pickle :511,:526,:541 / clone :554). All proposal kwargs match the actual ctor params used in the existing methods.
...
VERDICT: CONFIRMED — all line refs exact: miller_madow trio at :989/:1001/:1012 with bodies identical modulo the flag value; unseen-cell strategy methods at :882-932 (test_clip_strategy :882, test_sentinel_strategy_no_unseen :891, test_clip_with_unseen_cells :901, test_sentinel_with_unseen_cells :912, test_raise_strategy_leaves_negative_one :923, test_unknown_strategy_raises :932 — exactly 6) plus the second-class pair at :1294/:1305; existing parametrization at :1086-1087 (correction over bonferroni/bh_fdr) and :593-594 (select_on). One proposal nit: the "auto" mode is spelled `use_miller_madow=None` in the actual ctor (verified :1016), so the parametrize values must be [True, False, None], not the string "auto".
...
VERDICT: CORRECTED — structure verified: TestFourier/TestRBF/TestSigmoid/TestPade each carry roundtrip + coef_size + empty-coefs shapes at the cited ranges, and TestRegistryMetadata.test_all_four_families_registered at :293 iterates ("fourier", "rbf", "sigmoid", "pade") against the registry, which IS already imported (test_bases.py:9). Two exact corrections: (1) the registry constant is named `EXTRA_BASES` (from mlframe.feature_selection.filters.bases import EXTRA_BASES), not `BASIS_REGISTRY` as the proposal writes; (2) the trio is not perfectly "structurally identical" across families — RBF's size test is behavior-specific `test_coef_size_saturates_at_9` and Pade's empty-coefs is `test_empty_too_short_returns_zeros` (different contract: too-short vs empty), so the lifted parametrized trio needs a small per-family expected-size/min-len table rather than a uniform body. Core point (registration should imply contract coverage) stands.
...
VERDICT: CONFIRMED — all three refs verbatim-exact: test_cluster_aggregate.py:57 loops over the 3-method subset ("mean_inv_var", "pca_pc1", "factor_score"); layer50:317-318 loops over a 5-method subset ("mean_z", "mean_inv_var", "pca_pc1", "pca_pc2", "factor_score"); CLUSTER_AGGREGATE_METHODS is a module-level tuple constant at _cluster_aggregate.py:33-45 with exactly the 9 listed methods. Both loops are strict subsets of the roster (missing median/median_z/signed_max_abs/signed_l2_sum resp. median/median_z/signed_max_abs/signed_l2_sum), confirming the silent-skip claim; the proposal's import is valid.
...
[Verifier note on the closing verdict: "essentially everything FE-related is binary-classification ... + non-default DCD config" inherits the overstatements corrected in findings 01 and 05 — the fe_hybrid_orth family is biz-value-tested on regression and multiclass real datasets (layers 29, 83), and 12 newer FE layers run with production-default DCD/cluster-aggregate. The narrowed-but-real gaps: non-orth target-aware FE families lack any regression/multiclass coverage, and the all-FE-on kitchen-sink contracts never run with default redundancy mechanisms.]

---

## Verification summary

- CONFIRMED: 10 (02, 04, 06, 08, 09, 10, 11, 14, 15, 17)
- CORRECTED: 7 (01, 03, 05, 07, 12, 13, 16)
- REFUTED: 0

All counts in the verified findings (468 files, 97 layers, 62 dcd pins, 62 fe_*_enable flags, 48 _make_mrmr copies, 46 clone_preserves files, 42 leak files, the 12-layer ctor list) were independently reproduced exactly. Every quoted source line was opened and checked verbatim.

Three most load-bearing corrections:

1. **[param_axes-01]** "ZERO regression-target and ZERO multiclass coverage" for the modern FE families is false for the largest family: fe_hybrid_orth (univariate + all 9 scorer variants) is quantitatively biz-value-tested on load_diabetes + fetch_california_housing (regression) and iris/wine/digits (3- and 10-class) in test_biz_value_mrmr_layer29.py and test_biz_value_mrmr_layer83.py — both absent from the finder's sample. The real gap is the NON-orth target-aware families (kfold_te, grouped_*, cat_num_interaction, rankgauss, rare_category, temporal_agg, conditional_residual, mi_greedy); severity P1→P2.
2. **[param_axes-05]** "Entire 97-layer FE biz suite pins production-default redundancy OFF" is overstated: 12 FE layer files (87-95, 99, 104, 52, 55) fit FE-enabled MRMR with dcd/cluster_aggregate constructor defaults ON. The surviving (verified) core is narrower: every ALL-FE-ON kitchen-sink contract (L35, L64, L70, L101) pins them off, so only the all-on × prod-defaults interaction is unwitnessed.
3. **[param_axes-13]** "Engineered-recipe replay under float32 is never pinned" is refuted for the legacy recipe family: test_mrmr_engineered_replay.py fits + replays + pickle-round-trips fe_max_steps recipes on all-float32 frames. The unwitnessed surface is modern-mechanism recipes (orth bases / rankgauss / mi-greedy) under f32 and any f32-vs-f64 parity assertion. (Runner-up: [param_axes-12] — layer83's headline matrix test accumulates all bad cells before asserting, so the "first broken cell masks siblings" half of the evidence is wrong; only the missing per-cell ids half stands.)
