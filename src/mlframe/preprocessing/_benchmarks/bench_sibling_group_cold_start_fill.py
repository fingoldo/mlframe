...
def _run(n_groups: int, rows_per_group: int, n_calls: int) -> None:
    df = _make_frame(n_groups, rows_per_group)
    for _ in range(n_calls):
        sibling_group_cold_start_fill(df, "group", "order", "value")


if __name__ == "__main__":
    for n_groups, rows_per_group, n_calls in [(2000, 5, 20), (50000, 5, 20), (50000, 20, 20)]:
        t0 = time.perf_counter()
        _run(n_groups, rows_per_group, n_calls)
        wall = time.perf_counter() - t0
        print(f"n_groups={n_groups:>7} rows_per_group={rows_per_group:>3} n_calls={n_calls:>4} -> {wall * 1000:9.2f} ms")
