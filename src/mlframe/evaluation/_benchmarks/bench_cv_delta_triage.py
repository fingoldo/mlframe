...
Also profiles the opt-in ``CVDeltaHistory`` accumulator path (``history=`` kwarg), which adds an O(n_folds)
``np.var`` call per invocation on top of the plain path.
"""
...
def _run_with_history(n_calls: int = 20_000, n_folds: int = 5) -> None:
    rng = np.random.default_rng(0)
    history = CVDeltaHistory()
    for _ in range(n_calls):
        baseline = 0.85 + 0.01 * rng.standard_normal(n_folds)
        candidate = baseline + 0.003
        triage_cv_delta(baseline, candidate, change_source="feature_engineering", history=history)
        triage_cv_delta(baseline, candidate, change_source="hyperparameter", history=history)
...
if __name__ == "__main__":
    ...
    print(f"plain wall for {n_calls} paired calls: {wall * 1000:.2f} ms ({wall / n_calls * 1e6:.2f} us/call pair)")

    t0 = time.perf_counter()
    _run_with_history(n_calls=n_calls)
    wall_history = time.perf_counter() - t0
    print(
        f"history wall for {n_calls} paired calls: {wall_history * 1000:.2f} ms "
        f"({wall_history / n_calls * 1e6:.2f} us/call pair)"
    )
