"""Rule-based hyperparameter-candidate filtering DSL, sampling helpers, and ``ParamsOptimizer``
(the base ML-guided trial suggestion class). Carved out of ``tuning.py`` (monolith split, CLAUDE.md
"sibling re-export" convention) to keep that module under the 1000 LOC budget -- re-exported from
``tuning.py`` unchanged.

Imports only from ``._tuning_types`` (never from ``.tuning`` or ``.tuning_catboost``), so both
``tuning.py`` and ``tuning_catboost.py`` can depend on this module without an import cycle: this
module needs ``ParamsOptimizer`` to exist alongside the rule functions it calls, and
``CatboostParamsOptimizer`` needs ``ParamsOptimizer`` as a base class, so a ``tuning_rules -> tuning``
or ``tuning_rules -> tuning_catboost`` back-edge would cycle either sibling straight back here.
"""
