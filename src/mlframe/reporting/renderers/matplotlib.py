...
from mlframe.reporting.colors import BAR_PRIMARY, resolve_heatmap_cmap
...
class MatplotlibRenderer:
    ...
    # Bound at the bottom of this module from ``._matplotlib_heatmap`` (carved out for the LOC ceiling);
    # declared here so ``_render_panel``'s dispatch and mypy both still see them on the class.
    _heatmap: ClassVar[Any]
    _confusion_margins: ClassVar[Any]
    # Bound at the bottom of this module from ``._matplotlib_network``.
...
# ``_heatmap`` and ``_confusion_margins`` live in ``._matplotlib_heatmap``: they are the two largest panel
# builders here and together they took this file past the 1000-LOC ceiling. Bound back onto the class so the
# ``_render_panel`` dispatch and any external ``MatplotlibRenderer._heatmap`` reference keep resolving.
from ._matplotlib_heatmap import _confusion_margins as _confusion_margins_impl
from ._matplotlib_heatmap import _heatmap as _heatmap_impl

MatplotlibRenderer._heatmap = _heatmap_impl
MatplotlibRenderer._confusion_margins = _confusion_margins_impl

# ``_network`` lives in ``._matplotlib_network`` for the same LOC ceiling; bound back onto the class like the two above.
from ._matplotlib_network import _network as _network_impl

MatplotlibRenderer._network = _network_impl
