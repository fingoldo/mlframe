"""Bench: pseudo_smote._smote_synthesize_intra row-loop vs draw-then-vectorise gather+lerp.

The intra-positive SMOTE interpolation drew (src, nbr, alpha) per iteration AND wrote one output row per
iteration. The per-row numpy fancy-index + lerp dominates; the RNG draws must stay in the exact interleaved
per-iteration order to keep the PCG64 stream (hence the synthetic cloud) bit-identical, so only the gather+lerp
is hoisted out of the loop.

Measured (py3.14.3, n_min=500, d=30, k_smote=5, best-of-15):
  OLD (row loop)            ~48 ms
  NEW (draws + vec lerp)    ~31 ms   => ~35% faster, BIT-IDENTICAL.

Run: python -m mlframe.feature_engineering._benchmarks.bench_pseudo_smote_synthesize_vectorized
"""
