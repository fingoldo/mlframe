...
def _run_dae(n: int) -> None:
    X = _make_dataset(n, d=12, seed=0)
    y = np.zeros(n)  # unused, API symmetry only
    splitter = KFold(n_splits=5, shuffle=True, random_state=0)
    compute_denoising_autoencoder_features(X, y, X_query=None, splitter=splitter, seed=0, hidden_size=16, bottleneck_dim=4, max_iter=150)
...
if __name__ == "__main__":
    ...
    for n in [500, 2_000, 10_000]:
        ...
        _run_dae(n)
        wall = time.perf_counter() - t0
        print(f"compute_denoising_autoencoder_features n={n:>7,} -> {wall * 1000:9.2f} ms")
