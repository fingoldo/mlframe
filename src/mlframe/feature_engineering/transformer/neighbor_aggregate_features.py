...
def _aggregate_one(
    ...
    _, neighbor_idx = knn_search(X_index, X_query, k_max)  # (n_q, k_max)
...
def compute_neighbor_aggregate_features(
    ...
    *,
    ...
    pl.DataFrame
        ...
        ``{column_prefix}_{agg_col}_k{k}_{stat}``.
    """
    ...
    if X_query is not None:
        ...
        cols = _aggregate_one(Xt_s, Xq_s, agg_values, k_list, stats, dtype, column_prefix)
        return pl.DataFrame(cols)

    if splitter is None:
        raise ValueError("Mode A (X_query=None) requires a splitter.")
    n_features = len(agg_values) * len(k_list) * len(stats)
    col_order: list[str] = []
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splits):
        ...
        fold_cols = _aggregate_one(Xt_s, Xq_s, fold_agg_values, k_list, stats, dtype, column_prefix)
        if not col_order:
