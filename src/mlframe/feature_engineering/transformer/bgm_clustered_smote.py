...
def compute_bgm_clustered_smote_features(
    ...
    *,
    ...
    def _process(Xt: np.ndarray, Xq: np.ndarray, y_t: np.ndarray, fold_seed: int) -> np.ndarray:
        """Core per-fold pipeline: scale, slice into pos/neg cores, synthesize BGM-clustered SMOTE virtual points, then compute each query row's k-th-neighbour distance to (real-positives + virtuals) and to real-negatives, plus the signed log-gap between them. Returns an all-zero block if either core has <2 rows (too few for a meaningful boundary)."""
        if standardize:
