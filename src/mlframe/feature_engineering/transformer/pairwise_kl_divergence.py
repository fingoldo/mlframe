...
def _fit_3baselines_with_sigma(Xt: np.ndarray, y_t: np.ndarray, Xq: np.ndarray, task: str, seed: int):
    ...
    if task == "binary":
        ...
        p1_train = m1.predict_proba(Xt)[:, 1].astype(np.float32)
        p1_query = m1.predict_proba(Xq)[:, 1].astype(np.float32)
        ...
        p2_train = m2.predict_proba(Xt)[:, 1].astype(np.float32)
        p2_query = m2.predict_proba(Xq)[:, 1].astype(np.float32)
        try:
        ...
        m1.fit(Xt, y_t)
        p1_train = m1.predict(Xt).astype(np.float32)
        p1_query = m1.predict(Xq).astype(np.float32)
        m2 = lgb.LGBMRegressor(n_estimators=50, max_depth=5, learning_rate=0.1, random_state=int(seed) + 1, verbose=-1, n_jobs=-1)
        m2.fit(Xt, y_t)
        p2_train = m2.predict(Xt).astype(np.float32)
        p2_query = m2.predict(Xq).astype(np.float32)
