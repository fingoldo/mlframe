"""The report's blocks: each renders one section of the pre-registered decision tables as text lines.

Carved out of `analyze`, which had become a list of section renderers with the report's assembly at the
bottom. The blocks are independent -- each takes the cell records and returns its own lines -- so they live
together here and `analyze.format_report` is left doing only what its name says: choosing the order.
"""
...
def headline_block(verdicts: Sequence[ScenarioVerdict]) -> List[str]:
...
def interaction_block(verdicts: Sequence[ScenarioVerdict]) -> List[str]:
...
def reliability_block(records: Sequence[Dict[str, Any]]) -> List[str]:
...
def control_adjusted_block(records: Sequence[Dict[str, Any]], models: Sequence[str], k_labels: Sequence[str]) -> List[str]:
...
def cost_block(records: Sequence[Dict[str, Any]]) -> List[str]:
...
def declaration_block(records: Sequence[Dict[str, Any]]) -> List[str]:
...
def pooled_block(records: Sequence[Dict[str, Any]], models: Sequence[str], k_labels: Sequence[str]) -> List[str]:
...
def variance_block(records: Sequence[Dict[str, Any]], models: Sequence[str], k_labels: Sequence[str]) -> List[str]:
