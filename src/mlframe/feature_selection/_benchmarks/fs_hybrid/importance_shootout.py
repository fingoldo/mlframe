...
from .run_experiment import compute_auc_mean
...
def downstream(Xtr, Xte, ytr, yte, cols):
    ...
    for name, mk in {
        ...
        try:
            m = mk()
            m.fit(Xtr[cols], ytr)
            out[name] = round(float(roc_auc_score(yte, m.predict_proba(Xte[cols])[:, 1])), 4)
...
def main():
    ...
    for i, (sc, sd, ig) in enumerate(cells, 1):
        ...
        try:
            ...
            r = RFECV(
                estimator=lgb.LGBMClassifier(n_estimators=120, num_leaves=31, learning_rate=0.06, n_jobs=-1, verbose=-1), cv=3, scoring=None, verbose=0, fi_config=fi, search_config=sgc, random_state=0
            )
            t0 = time.time()
            r.fit(Xtr, ytr)
            ...
            base = set(t["base"])
            noise = set(t["noise"])
            row.update(n_feat=len(cols), base_recall=round(len(set(cols) & base) / max(1, len(base)), 3), noise_sel=len(set(cols) & noise), auc=downstream(Xtr, Xte, ytr, yte, cols))
            row["auc_mean"] = compute_auc_mean(row["auc"])
            log(f"[{i}/{len(cells)}] {sc}/{sd}/{ig}: n={row['n_feat']} rec={row['base_recall']} noise={row['noise_sel']} " f"fit={row['fit_s']}s auc={row['auc']}")
        except Exception as e:
...
def summarize():
    ...
    if not ok:
        log("no successful cells")
        return
    df = pd.DataFrame(ok)
    log("\n=== mean over (scenario,seed) by importance ===")
    g = df.groupby("importance").agg(
        lgbm=("auc", lambda s: round(np.mean([a.get("lgbm") for a in s if a.get("lgbm")]), 4)),
