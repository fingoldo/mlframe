...
- **FOLLOW-UP WAVE (4 parallel agents, 2026-06-04):**
  - **SHIPPED: RFECV madelon-timeout FIX** — wide-data perm-FI cost guard (fall back to gain-importance ranking
    when p>200; CV score unaffected). madelon now yields a real curve + support<500 within budget; 169 RFECV tests
    pass. New params wide_data_fi_fallback/threshold/n_repeats. (test_rfecv_wide_data_fi_guard.py)
  - **SHIPPED: MRMR synergy-bootstrap n-aware cost gate** (fe_synergy_max_sweep_cost=5e8) — fixes the cap=250
    large-n blow-up (n=20k p=200: 180s→58s; n=100k was >24min). Fires on wins (n*p²≤5e8), skips blow-ups.
  - **FE (agent): rich-ops stay in `medium`** (no quality gain in default `minimal`, +50-80% cost — measured);
    **median-gated operators** a measured win (+0.04 on skewed) but need a `prewarp`-style stateful pseudo-unary
...
|A1-7|`operand_mi_preserve_binning`|MDLP collapses pure-interaction operands to 1 bin (no marginal split) → zeroes them before FE. Fall back to quantile binning for 1-bin-collapsed cols in the JOINT sweep only.|**MEASURED (diagnostic) — CONFIRMED mechanism, but ALREADY PATCHED in prod → no action.** Raw MDLP DOES collapse pure-interaction operands to 1 bin (ia/ib on hard_synth, a/b on y=sign(a*b); 487/500 madelon cols incl. 7/20 informative). BUT production `per_feature_edges` (`_adaptive_nbins.py:620-688`) already falls back to quantile binning for collapsed cols → 0/500 madelon collapse in the real path. So binning is NOT the madelon-collapse cause (corroborates: it's the SELECTION gate — greedy under-selects on ~0 marginal MI even with live 5-bin operands). No fix needed. (round4_mdlp_collapse_diag.py)|
...
|A2-4|`prior_protected_rfecv`|warm-start RFECV's elimination with must_include = high-confidence core from Boruta/MRMR; RFECV only trims the tail.|**MEASURED — NEGATIVE (must beat plain RFECV; does not).** hard_synth cleaner support (noise 13→3) but AUC −0.0006 (~core_only); synth −0.0075 (pins in extras plain RFECV correctly trimmed). Cleaner support ≠ AUC win; confirms H3-4. (round4_prior_protected_rfecv_bench.py)|
...
|A2-7|`tentative_to_cmi`|route Boruta `tentative` set to MRMR conditional-MI given the accepted core; admit if it adds new info.|**MEASURED — NEGATIVE.** hard_synth: CMI-gate admits 2/8 tentative (1 weak/1 noise), weak 6→7, AUC +0.0027 (noise); un-gated all-tentative ctrl 0.768 > gate 0.760 (LGBM tolerates the noise; gate drops a useful weak). Buried-weak CMI barely clears the perm null (same SNR floor as A4-4). (round4_tentative_to_cmi_bench.py)|
...
|A3-3 / A3-6|`fe_holdout_greedy_pairs` / `fe_residual_target`|iterative greedy FE accept on HELD-OUT gain, and rank FE pairs by MI with the linear-removal RESIDUAL.|**MEASURED — REJECTED (parsimony-only, hurts madelon).** vs keep-all: synth/hard_synth keep FEWER products (1-10 vs 30) at ~equal AUC (+0.003, a parsimony win); but madelon keep_all 0.8555 BEATS holdout_greedy 0.8369 (−0.019), resid_mi/rawy_mi −0.065 to −0.077 — the acceptance gate drops useful products, trading recall for parsimony, and the recall loss hurts the hardest bed. Not an AUC win. (round4_fe_accept_frugal_bench.py)|
...
|A4-4|`su_seeded_interactions` (Shap)|cheap pairwise-SU screen → top-K synergistic pairs → run interaction objective on ONLY those K (avoids O(P²) tensor).|**MEASURED — MIXED (opt-in for clear-SNR, no-op on noise-buried) → recommend to Shap owner.** Synergy-SU screen ranks the TRUE operand pair #0 on pure-interaction + synth; seeding lifts ShapSel op-recall +2/+1, AUC **+0.388 / +0.072** — recovers pure interactions the main-effect proxy is blind to, NO O(P²) cost. hard_synth: ia*ib synergy below the noise-pair floor → screen MISSES, −0.010. Opt-in SNR-gated sparse-interaction path ungating interaction_aware past phi=16. (round4_su_seeded_interactions_bench.py)|
|A4-5|`adaptive_n_trials` (Boruta)|stop trials by binomial-confidence convergence; reallocate budget to borderline-tentative features.|**MEASURED — speed WIN, but only with a SAFE gate (naive is a correctness trap) → recommend to Boruta owner.** Prod already stops at zero-tentative, but a tentative TAIL (synth3/hard5) never resolves → full 120-cap burned. Naive W=10 acc-stable: synth jac 1.0 but **hard_synth jac 0.0 (locks WRONG set)**. Margin-gated SAFE stop: synth jac 1.0 save ~44s (72%), hard_synth jac 0.842 save ~68s (63%). Add a margin-gated tentative-tail convergence stop ONLY. (round4_adaptive_n_trials_bench.py)|
...
|A5-5|`conf_rep`|cluster rep chosen by cross-member confidence rank-agg instead of perm-FI alone.|**MEASURED — NEGATIVE (AUC-neutral, low ceiling).** Borda rank-agg of [votes, perm-FI] changed 3/1 cluster reps; AUC synth +0.0009 / hard_synth −0.0012 (both noise); redundant-cluster copies are interchangeable downstream. Matches the round-3 FE-protective-rep rejection. (round4_conf_rep_bench.py)|
|A5-6|`union_backward`|one honest RFECV `one_se_min` backward pass over the small UNION of member picks (prune, not re-discover).|**MEASURED — REJECTED.** hard_synth 2-seed: `union_all` (union, no RFECV) ≈ baseline (28→subsumes; H3-4 confirmed), and RFECV-over-the-union PRUNES hard (28→14, 30→11) but LOSES AUC both seeds (sd0 0.817→0.812 −0.005; sd1 0.7659→0.7583 −0.008) — RFECV one_se_min over-prunes the clean union. Not a win. (round4_union_backward_bench.py)|
