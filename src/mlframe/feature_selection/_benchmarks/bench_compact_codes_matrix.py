...
def _bench(n_rows: int = 200_000, n_cols: int = 400, n_bins: int = 10, cap: int = 127, reps: int = 5) -> None:
    ...
    print(f"  codes matrix bytes : int32={int32_mb:8.1f} MB  compact int8={int8_mb:8.1f} MB  "
          f"({int32_mb / max(int8_mb, 1e-9):.1f}x smaller)")
    print(f"  downcast wall      : min={min(t) * 1e3:7.2f} ms  median={np.median(t) * 1e3:7.2f} ms  "
          f"({data.nbytes / max(min(t), 1e-9) / 2**30:.1f} GB/s)")
