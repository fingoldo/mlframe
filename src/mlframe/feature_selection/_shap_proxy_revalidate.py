...
def proxy_trust_guard(
    ...
    *, classification, metric=None, n_anchors=30, rng=None, min_card=1, max_card=None,
    spearman_floor=0.6, n_jobs=-1, unit_to_members=None,
):
    ...
    honest_losses = _parallel_honest_losses(
        ...
        X_holdout, y_holdout, classification, metric, n_jobs)
...
def within_cluster_refine(
    ...
    *, classification, metric=None, parsimony_tol=0.02, n_jobs=-1, max_drop_rounds=None,
    ...
    if len(current) <= 1:
        return current
    base = _honest_loss(model_template, X_search, y_search, X_holdout, y_holdout, current, classification, metric)
...
def importance_topk_ablation(
    ...
    *, classification, metric=None, unit_to_members=None,
):
