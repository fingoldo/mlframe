...
| ND-3 | REGRESSION-CONFIRMED (attribution corrected) | I4/I4b/I5[subsumed_plus_private-heavytail-regression-s312] fail: FE uplift delta -0.064 (< -0.05 bar). A/B PROVES campaign-introduced: PASSES at pre-campaign cbb16aca (6 passed / I4 hits heavytail skip-guard), FAILS at HEAD (3 failed). Mechanism is NOT the sync-free binning (CPU bin codes are BYTE-IDENTICAL across the campaign) -- it is one of the campaign's CPU-path CMI-cardinality/renumber changes (6f1b402b thread kx/kz cardinalities / 53ce3c56 dense renumber for CMI conditioning join / e0853961 drop redundant max reads / 2bd146d0 known histogram width) that altered a CMI VALUE and flipped FE gain ranking. | CMI-cardinality path (bisecting) | IN-PROGRESS -- agent bisecting cbb16aca..HEAD to the culprit CMI commit, then minimal correctness fix restoring pre-campaign CPU CMI. NOT a stale-test re-frame. |
