...
|----|-----|---------|-----------|-------------|--------|
| S-F1 | P1 | GPU relevance null uses 2 perms vs CPU 32 → p-value floor 0.333 ≥ alpha always → subtracts null_mean from every feature incl. strong signal; hardware-dependent selection | gpu.py:974 / evaluation.py:530 | FIX (bump GPU null budget to max(nperm,32)) | TODO |
...
| S-F5 | Low | redundancy_aggregator typos silently degrade to Fleuret | _mrmr_class.py:~3355 | FIX (validate) | TODO |
...
| FE-F2 | P2 | temporal entity key raw `.astype(str)` not canonical token → int/float drift routes all test rows to global prior | _temporal_agg_fe.py:92 | FIX (canonical_group_token) | TODO |
| FE-F3 | P2 | target_aware_group_bin small-group OOF fallback uses all-rows edges → y-leak into MI gate | _grouped_quantile_fe.py:413,442 | FIX (per-fold train-only edges) | TODO |
| FE-F4 | P2 | `_compute_target_encoding` naive path (n_oof_folds<=0) emits row's own y | _cat_target_encoding_and_weighted.py:81 | FIX (guard K>=2) | TODO |
| FE-F5 | Low | rolling replay casts ns timestamps to float64 (precision loss at window boundary) | _temporal_agg_fe.py:731 | FIX (keep int64) — expanding/lag done in d804cdf4 | TODO (rolling) |
| FE-F6 | Low | grouped_quantile pct_rank self-inclusion fit vs replay (≈1/m offset) | _grouped_quantile_fe.py:206,309 | DOC | TODO |
| FE-F7 | Low | temporal winner name collision → duplicate columns | _fe_stage_temporal_agg.py:101 | FIX (dedup on concat) | TODO |
...
| EN-2 | Low | polars branch crashes when pandas absent | _encoding_recipes.py:73 etc | FIX (guard pd) | TODO |
...
| ID | Sev | Finding | file:line | Disposition | Status |
|----|-----|---------|-----------|-------------|--------|
| EX-1 | P2 | `_is_argmax_eligible` finiteness guard is a tautology → NaN columns not excluded | _conditional_gate_fe.py:357 | FIX (isfinite(a).all()) | TODO |
...
| ST-3 | Low | fallback support_ omits dtype=int64 → int32 on Windows | _finalise.py:281 | FIX | TODO |
