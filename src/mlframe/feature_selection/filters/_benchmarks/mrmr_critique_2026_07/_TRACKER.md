...
| N-F2 | P1 | Relevance is debiased (MM when active + null-mean significance debias) but the Fleuret redundancy (conditional_mi) is raw plug-in -> the relevance-minus-redundancy objective is biased vs high-cardinality candidates. | evaluation.py (redundancy call) ; info_theory/_entropy_kernels.py:308 (conditional_mi) ; _fe_cmi_redundancy_null.py:195-205 | FUTURE-bench (fully scoped) — DECOMPOSED into: (1) MM-consistency (opt-in, default-unchanged, N-F1 pattern): when use_mi_miller_madow() is active, subtract the analytic MM CMI bias (k_xyz + k_z - k_xz - k_yz)/(2n) from conditional_mi -- the exact formula _cmi_from_binned already uses. IMPLEMENTATION NOTE: conditional_mi's entropy_cache stores only the entropy scalar, so the occupied-cell counts (k_z/k_xz/k_yz/k_xyz) must be computed AND cached alongside; a subtle bug here silently shifts every selection, so it needs the full MM/CMI regression + a bit-identity check on the MM-off default. (2) default-path null-mean redundancy debias (mirror the relevance null-mean subtraction on the redundancy using the null_mean from _fe_cmi_redundancy_null.py) -- this CHANGES the default core objective, so it stays gated behind a flag (REJECTED!=DELETED convention) until a wide multi-seed biz-value bench (high-card vs low-card fixtures, honest OOF) proves selection equal-or-better. Do in a dedicated session with fresh context. |
...
| ND-2 | P2? | test_biz_val_mrmr_sample_weight_fe [mi_greedy]: weight-vs-duplication RAW-set equivalence dropped to 1/3 seeds. NOT OURS — traces to the parallel session's GPU-resident-FE mi_greedy quantile-binning rewrites (51f7ad5c/7d783ac1/53ce3c56); our evaluation.py/permutation.py changes are provable no-ops on this default path. | tests/.../test_biz_val_mrmr_sample_weight_fe.py | PARALLEL-SESSION (for that session to reconcile) |
