...
| ND-3 | REGRESSION-CONFIRMED | I4/I4b/I5[subsumed_plus_private-heavytail-regression-s312] fail: FE uplift delta -0.064 (< -0.05 bar), fe R2 0.857 vs raw-only 0.921. This case was GREEN at the pinned _DEFAULT_CASE_N=25000 (delta +0.015, documented at test lines 84-85) BEFORE the parallel session's sync-free quantile binning (commit 7d783ac1 'branchless dedup + manual percentile' + the bin-code cache chain 51f7ad5c/6f1b402b/53ce3c56). The manual-percentile edges diverge on heavytail extreme values enough to flip FE selection to add(a,sin(d))/sub(cbrt(a),log(b)), which underperform raw-only downstream. Violates the binning's own SELECTION-EQUIVALENCE acceptance bar. | src/mlframe/feature_selection/filters/_mi_greedy_cmi_fe.py | IN-PROGRESS -- root-cause+fix agent dispatched (isolated worktree, full A/B + selection-equivalence). NOT a stale-test re-frame: n=25k already escapes finite-sample noise; this is a genuine downstream regression to fix in the binning, not the test. |
