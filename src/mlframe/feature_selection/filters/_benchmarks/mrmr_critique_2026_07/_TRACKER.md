...
| ND-5 | OPEN | 2 selection-equiv pins fail at HEAD, byte-identical pre/post the ND-3 fix (NOT caused by it): test_feature_engineering_example_single_compound + test_clean_library_form_preferred_over_monotone_prewarp. Both emit ['c', 'sub(log(div(sqr(a),neg(b))),prewarp(mul(prewarp(c),sin(d))))'] -- a prewarp-form + 'c' fragmentation from the separate prewarp/fragmentation campaign, distinct root cause from the tail-subsume gate. | prewarp form selection / c fragmentation | OPEN -- investigate whether the prewarp double-nesting + standalone 'c' is a real regression or a stale over-specified pin; separate from ND-2 (mi_greedy sample_weight) though both are campaign fallout. |
