"""GPU-resident FE: radix-select CUDA kernels + dispatch helpers (carved sibling of ``_gpu_resident_select.py``).

Carved VERBATIM out of ``_gpu_resident_select.py`` (sibling re-export pattern) to bring that module under
the 1k-LOC ceiling. Holds every RawKernel source string, its lazy-compiled module-level-singleton getter,
the coalesced-tiled-transpose kernels, and the per-host KTC dispatch helpers (``_resolve_radix_threads``,
``_resolve_radix_f32_variant``, ``_get_radix_select_f32_dispatch``, ``fe_gpu_radix_edges_enabled``). The
higher-level order-statistic API (``_radix_select_interior_edges`` / ``_radix_quantiles``) stays in the
parent and calls back into this module; the parent re-exports every name here via
``from ._gpu_resident_select_kernels import *`` so every existing ``from ._gpu_resident_select import X`` /
``from .._gpu_resident_fe import X`` path still resolves byte-for-byte. No kernel-source, dispatch-
threshold, residency, or selection behavior changed.
"""
...
# 20B) v2 doesn't have (confirmed v3's real static-shared use is
...
# Lever B: threads/block is now KTC-TUNED per host (radix_select_f32 was the biggest kernel,
...
def _resolve_radix_threads(n: int) -> int:
    ...
    try:
        ...
        logging.getLogger(__name__).debug("radix-select threads KTC lookup failed; using fixed default", exc_info=True)
...
# Lever C variant select: which f32 radix-select kernel to launch - the base linear-scan
...
def _resolve_radix_f32_variant(n: int) -> str:
    ...
    try:
        ...
        logging.getLogger(__name__).debug("radix-select f32 variant KTC lookup failed; using fixed default 'v3'", exc_info=True)
        return "v3"
