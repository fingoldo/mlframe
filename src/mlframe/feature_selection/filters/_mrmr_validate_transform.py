...
# Carved into ``_mrmr_validate_strings`` to keep this module under its LOC ceiling; re-exported so existing import sites keep working.
from ._mrmr_validate_strings import (  # noqa: F401
    _UNIMPLEMENTED_REDUNDANCY_ALGOS,
    _validate_hybrid_orth_string_params,
...
def _identity_fastpath_is_safe(self, X) -> bool:
    ...
    try:
        ...
        log_throttle(
            logger, "mrmr_transform_identity_probe_failed", logging.WARNING,
            "MRMR.transform: identity fast-path safety probe failed on a %s; using the fully-validated path.", type(X).__name__, exc_info=True,
        )
        return False
