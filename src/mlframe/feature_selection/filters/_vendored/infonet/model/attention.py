...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x_q: torch.Tensor, x_kv: torch.Tensor, attention_mask: Optional[torch.Tensor] = None):
        ...
        out = torch.einsum("nhql,nlhd->nqhd", [attention, values]).reshape(batch, query_len, self.heads * self.v_head_dim)
        # attention shape: (N, heads, query_len, key_len)
...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        self.attention = MultiHeadAttention(
            q_dim=q_dim, kv_dim=kv_dim, qk_out_dim=qk_out_dim, v_out_dim=v_out_dim, output_dim=q_dim, heads=heads, dropout=dropout
        )
