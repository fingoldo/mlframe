...
class FeatureMatrix:
    ...
    def numeric_column(self, name: str) -> np.ndarray:
        ...
        # columns.index(name) returns only the FIRST index
...
def from_feature_matrix(fm: FeatureMatrix):
    ...
    # The per-name dicts (col_objs/data) built below are
    ...
    if fm.framework == "numpy":
        # numpy input is all-numeric; round-trip through the SAME null/float64-restored columns the
        # pandas/polars branches use so missingness is restored identically, then honor the intended
        # float32 P0 footprint. The old ``fm.numeric.copy()`` skipped the null-mask restore - an asymmetry.
        return np.column_stack([data[name] for name in fm.columns]).astype(fm.dtype, copy=False)
