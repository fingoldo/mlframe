...
# LARGE-N PEAK-MEMORY FIX. The ``_FE_BUFFER_RAM_BUDGET_RATIO`` budget was
...
# HOIST-GATE RE-CALIBRATION. The conservative ``_fe_effective_buffer_budget_bytes``
...
# ABSOLUTE FREE-RAM FLOOR. The ``_FE_BUFFER_RAM_BUDGET_RATIO`` (0.3) cap is RELATIVE: on
...
# ABSOLUTE BUFFER CEILING. The RELATIVE budget above (``0.3*(available-reserve)/(3.0*nw)``)
...
# UNIFIED FE/screen subsample. ONE knob for the whole FE block - the relevance SCREEN, the FE
...
# OPT5: short-TTL cache for ``psutil.virtual_memory().available``.
...
def _can_hoist_shared_buffer(
    ...
    LARGE-N FIX: with ``overhead_aware=True`` (default) the fit check uses the
    ...
    if buffer_bytes > _ceiling:
        # ABSOLUTE CEILING: refuse regardless of how much RAM is free - the anti-sawtooth
    ...
    # ABSOLUTE-HEADROOM ACCEPTANCE: the conservative relative budget over-declines a
