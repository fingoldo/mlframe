...
def _fit_impl(self, X: pd.DataFrame | np.ndarray, y: pd.DataFrame | pd.Series | np.ndarray, groups: pd.Series | np.ndarray = None, **fit_params):
    ...
    _fe_live = set(locals())
    # X_h/X_e (hinge / extra-basis, Layer 23), X_t/X_q/X_aa/X_ad/X_rt/X_df/X_cb/X_boot/X_tg/X_ksg/
    # X_copula/X_dcor/X_hsic/X_jmim/X_tc/X_cmim/X_auto/X_ens/X_meta (the 19 hybrid_orth
    # family-variant temp frames) and X_mg/X_cmi/X_te/X_ba/X_c/X_f/X_cn/X_i/X_p/X_r/X_lr/X_gd/X_ld
    # (Layers 23/26/33/34/37/38's own temp frames) no longer exist as _fit_impl locals at all --
    # all these blocks were carved into siblings (_fe_stage_cascade_early_a.py,
    # _fe_stage_cascade_early_b.py, _hybrid_orth_family_variants.py),
    # where each temp frame is now genuinely function-local and reclaimed automatically on that
    # function's return; the manual del here would have been dead (all these `if "X_t" in _fe_live`
    # guards evaluate False now, and ruff's F821 on the bare `del X_t` naming a name unbound
    # anywhere in this scope confirmed it) rather than a memory-safety gap.
