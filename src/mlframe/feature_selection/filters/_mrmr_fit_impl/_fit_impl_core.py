...
from ._helpers import _dispatch_default_scorer, _mrmr_cache_bytes_total, _orth_fe_numeric_cols, fe_decide_on_subsample
...
def _fit_impl(self, X: pd.DataFrame | np.ndarray, y: pd.DataFrame | pd.Series | np.ndarray, groups: pd.Series | np.ndarray = None, **fit_params):
    ...
    selected_vars = _friend_graph_and_redundancy_passes(
        self,
        X=X,
        classes_y=classes_y,
        cols=cols,
