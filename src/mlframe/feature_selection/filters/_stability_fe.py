...
def _run_bootstraps(
    ...
    for _b in range(n_bootstraps):
        ...
        try:
            ...
            log_throttle(logger, "stability_fe_bootstrap_failed", logging.WARNING, "stability_select_fe: bootstrap %d/%d failed (%s: %s); excluded from frequencies.", _b + 1, n_bootstraps, type(exc).__name__, exc)
            n_failed += 1
            continue
