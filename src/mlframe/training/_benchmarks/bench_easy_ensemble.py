...
def _run(n_pos: int, n_neg: int, n_bags: int) -> None:
    X, y = _make_data(n_pos, n_neg, seed=0)
    easy_ensemble_fit_predict(X, y, X[:200], model_factory=lambda: LogisticRegression(max_iter=200), n_bags=n_bags, random_state=0)
