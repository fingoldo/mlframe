...
def _run_fuzzy(n_entities: int, n_replicates: int, n_features: int, n_calls: int, radius: float) -> None:
    rng = np.random.default_rng(0)
    centers = rng.standard_normal((n_entities, n_features)) * 5.0
    entity_id = np.repeat(np.arange(n_entities), n_replicates)
    n = entity_id.shape[0]
    noisy = centers[entity_id] + 0.3 * rng.standard_normal((n, n_features))
    X = pd.DataFrame(noisy, columns=[f"f{i}" for i in range(n_features)])
    for _ in range(n_calls):
        reconstruct_pseudo_group_ids(X, fuzzy_radius=radius)
...
if __name__ == "__main__":
    ...
    # fuzzy_radius path -- NearestNeighbors radius query + connected-components, meaningfully more expensive
    # than the exact hash path, so profiled separately at a smaller n_calls to keep the harness fast.
    for n_entities, n_replicates, n_features, n_calls in [(1_000, 5, 10, 10), (20_000, 10, 15, 2)]:
        t0 = time.perf_counter()
        _run_fuzzy(n_entities, n_replicates, n_features, n_calls, radius=1.5)
        wall = time.perf_counter() - t0
        n_rows = n_entities * n_replicates
    ...
    profiler = cProfile.Profile()
    profiler.enable()
    _run_fuzzy(20_000, 10, 15, 2, radius=1.5)
    profiler.disable()
    buf = StringIO()
    stats = pstats.Stats(profiler, stream=buf).sort_stats("cumulative")
    stats.print_stats(15)
