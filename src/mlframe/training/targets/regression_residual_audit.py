...
def _diagnose(
    *,
    ...
    if excess_kurt > EXCESS_KURT_EXTREME:
        ...
        # Above the ceiling the suite's own loss recommender REFUSES Huber, because its gradient
        # ``delta * sign(r)`` vanishes when most rows sit near zero and the booster stops at iter 0-1. Advising it
        # here anyway put the two components in direct contradiction on the same statistic -- a production run
        # printed "suggested: Huber" 25 times on targets whose kurtosis the recommender was simultaneously using to
        # reject it. One ladder, read the same way in both places.
        if excess_kurt > EXCESS_KURT_HUBER_FAILS:
            ...
            # The HYPOTHESIS is unchanged -- the residuals are still contaminated, which is a statement about the
            # distribution. Only the remedy differs, because at this kurtosis the bounded-influence loss that
            # normally answers contamination is itself the failure mode.
            return (
