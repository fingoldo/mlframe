...
def _unstable_rate_warnings(segments: list, mean_rates: list[float], target_type: str, drift_warn_threshold: float) -> list[str]:
    """The "target rate is NOT stable" warning plus one line per segment, or ``[]`` when the spread is within threshold."""
    spread_abs = max(mean_rates) - min(mean_rates)
...
def _dropped_bins_warning(bins: list, kept_bins: list, threshold_n: float, min_bin_fraction: float) -> tuple[int, int, str | None]:
    """``(n_dropped, n_trailing_dropped, warning)`` for the bins the sparse-bin filter removed; warning is None if none were.

    Names the dropped range explicitly. A stability verdict is normally read against the evaluation split, and when the
    thin bins are the RECENT ones the audit covers a window that ends before that split does -- a production run
    dropped 5 trailing bins and reported stability over a window ending five weeks before the test period ended. A
    target still accruing at extract time produces exactly that bin profile.
    """
    ...
    _coverage = f"the audit therefore covers {kept_labels[0]}..{kept_labels[-1]} only" if kept_labels else "no bins remain"
    _recency_note = (
    ...
    return len(dropped_bins), _trailing, (
        f"{len(dropped_bins)} bin(s) dropped from the audit (n_obs < "
        f"{int(threshold_n):_} = {min_bin_fraction}x median bin size): {', '.join(b.bin_label for b in dropped_bins)}; "
        f"{_coverage}.{_recency_note} "
        "If this number is large, consider a wider granularity."
    )


def _maturity_caveat_for_recommendation(maturity: Any) -> str:
    """Caution appended to the "restrict to the most-recent segment" advice when the target looks still-accruing."""
...
def _audit_from_agg(
    *,
    ...
    if len(segments) >= 2:
        ...
        if mean_rates:
            warnings.extend(_unstable_rate_warnings(segments, mean_rates, target_type, drift_warn_threshold))

    n_dropped, _trailing, _dropped_warning = _dropped_bins_warning(bins, kept_bins, threshold_n, min_bin_fraction)
    if _dropped_warning:
        warnings.append(_dropped_warning)
    ...
    if most_recent_stable is not None and len(segments) >= 2:
        _maturity_caveat = _maturity_caveat_for_recommendation(maturity)
