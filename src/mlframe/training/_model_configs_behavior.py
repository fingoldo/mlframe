...
class TrainingBehaviorConfig(BaseConfig):
    ...
    # When True, the simple-ensembling blend weights are fit by Caruana greedy selection (metric-direct, hill-climbs AUC on the OOF member preds) instead of the default NNLS squared-error fit -- fed into ``score_ensemble`` via ``use_caruana_weights``. OFF by default (NNLS stays the default weight fit): the multi-seed honest-holdout A/B (``_benchmarks/bench_caruana_vs_nnls.py``, 5 scenarios x 25 seeds) measured caruana at only +0.0012 mean AUC (driven by the one-strong-many-weak case) but +0.020 mean log-loss -- it optimizes ranking at the cost of calibration, so it stays an opt-in win for AUC-primary tasks rather than a default flip.
    use_caruana_weights_in_ensemble: bool = False
    # Extra ensemble flavours appended to the default ``SIMPLE_ENSEMBLING_METHODS`` set for ``score_ensemble`` (e.g. ``("rrf",)`` for reciprocal-rank fusion). Defaults to the scale-invariant ``rank_average`` rank-fusion blend. Rank-fusion flavours (``rank_average`` / ``rrf``) output a normalised rank in [0, 1], which is neither a value on a regression target's scale nor a probability, so ``score_ensemble`` builds them only for learning-to-rank targets and drops them (INFO log) for every other target type; see ``mlframe.models.ensembling.flavour_policy``. De-duplicated; unknown names are rejected by ``ensemble_probabilistic_predictions``. Set ``()`` for the legacy magnitude-only flavour set.
