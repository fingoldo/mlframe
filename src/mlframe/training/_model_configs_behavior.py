...
class TrainingBehaviorConfig(BaseConfig):
    ...
    # When True, the simple-ensembling blend weights are fit by Caruana greedy selection (metric-direct, hill-climbs AUC on the OOF member preds) instead of the default NNLS squared-error fit -- fed into ``score_ensemble`` via ``use_caruana_weights``. OFF by default (NNLS stays the default weight fit).
    use_caruana_weights_in_ensemble: bool = False
    # Extra ensemble flavours appended to the default ``SIMPLE_ENSEMBLING_METHODS`` set for ``score_ensemble`` (e.g. ``("rank_average",)`` to add the scale-invariant rank-fusion blend, or ``("rrf",)``). Empty = the default flavour set only. De-duplicated; unknown names are rejected by ``ensemble_probabilistic_predictions``.
    extra_ensembling_methods: tuple = ()
