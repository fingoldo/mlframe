...
# PySR lives in a sibling module (this file was over the 1000-LOC house limit); re-imported here so the
# historical ``from ._pipeline_extensions import _apply_pysr_fe`` path and the call site below both work.
from ._pipeline_extensions_pysr import _apply_pysr_fe
from ._pipeline_extensions_row_wise import apply_row_wise_steps
...
def _filter_to_numeric(_df, keep_cols=None):
    ...
    if _df is None:
        return _df, []
    # Pre-fix this silently passed through polars DataFrames; downstream ``_df.select_dtypes(...)`` then raised AttributeError with no diagnostic naming the type. Coerce polars -> pandas explicitly; raise on truly unsupported types so the upstream caller sees the boundary.
    ...
    # The CRITICAL no-df.copy() rule (CLAUDE.md "Memory / RAM constraints") forbids a full-frame clone here -- on a 100+ GB frame that would OOM the host. An earlier form did ``_df = _df.copy()``; replaced with a per-column in-place dtype mutation. Single-column assignment is a block-level dtype swap, so the unchanged numeric columns keep their original buffers (the regression sensor asserts ``np.shares_memory`` on the float columns).
...
def apply_preprocessing_extensions(
    ...
    out_row_wise_replay: Optional[dict] = None,
):
    ...
    ``out_row_wise_replay``, when provided, receives the pinned row-wise column list and the fitted extremality
    reference predict needs to reproduce the row-wise columns; see ``apply_row_wise_steps``.
