...
def _regression_shape_warnings(
    *,
    split_name: str,
    ...
    """Relative level / dispersion / upper-tail shift warnings for one regression split, recording every ratio into ``drifts``.
    ...
    * regression — mean / std / median / p01 / p99. Warns when val or
    ...
    if is_multilabel:
        ...
        for split_name in ("val", "test"):
            ...
            warnings.extend(
                _regression_shape_warnings(
                    split_name=split_name,
                    train_summary=splits["train"],
                    split_summary=splits[split_name],
                    drifts=drifts,
                    rel_threshold=regression_rel_shift_threshold,
                )
            )
        ...
        for split_name in ("val", "test"):
            ...
            if abs(delta_pp) > warn_threshold_pp:
                warnings.append(
                    ...
                    f"{split_name}. The remedy is a held-out calibration split "
                    f"(TrainingSplitConfig.calib_size); without one the suite can measure the "
                    f"miscalibration but cannot correct it."
                )
