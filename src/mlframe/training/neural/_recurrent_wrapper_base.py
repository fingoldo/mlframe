"""``_RecurrentWrapperBase`` (+ the ``_monitor_mode`` EarlyStopping helper and the constants both
it and the base class use), carved out of ``_recurrent_wrappers.py`` to keep that module under the
repo's enforced hard 1000-LOC CI gate (it hit 1049 lines once ``RecurrentClassifierWrapper`` /
``RecurrentRegressorWrapper`` moved in from ``recurrent_dataset_helpers.py``). Behaviour preserved
bit-for-bit; ``_recurrent_wrappers.py`` imports everything back from here so its own re-export of
these same names (for ``recurrent_dataset_helpers.py``'s facade) keeps working unchanged.
"""
