...
def _maybe_refit_on_collapsed_predictions(
    *,
    ...
    try:
        import numpy as _np
        _y = _np.asarray(train_target)
    except Exception:
        return False
    if _y.ndim != 1 or _y.size < 10:
        return False
    _y_finite = _y[_np.isfinite(_y)]
    ...
    try:
        _preds = _np.asarray(model.predict(train_df)).reshape(-1)
    except Exception:
        return False
    if _preds.size != _y.size:
        return False
    _preds_finite = _preds[_np.isfinite(_preds)]
    ...
    try:
        _preds2 = _np.asarray(model.predict(train_df)).reshape(-1)
        _preds2_finite = _preds2[_np.isfinite(_preds2)]
