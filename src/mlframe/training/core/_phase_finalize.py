...
def finalize_suite(ctx: TrainingContext) -> dict:
    ...
    # Fit an optimized binary decision threshold (+ optional per-cohort / cv report) on the calib slice. ON by
    # default (auto_optimize_threshold).
    try:
    ...
    # Every step above that fits on the calib slice reports only what it produced, so with no slice they all fell
    # silent together. Say once which enabled steps that cost, and the one knob that restores them.
    try:
        report_calib_dependent_steps_skipped(ctx)
        report_oof_dependent_steps_inert(ctx)
    except Exception as _skip_err:
        logger.debug("[calib] skipped-steps report failed: %s", _skip_err)
