...
__all__ = [
    ...
    "report_skipped_and_inert_steps",
]
...
def report_skipped_and_inert_steps(ctx: "TrainingContext") -> None:
    """Run both reports for ``finalize_suite``; a failure here is logged at DEBUG and never breaks finalization.

    Every step that fits on the calib slice reports only what it produced, so with no slice they all fell silent
    together. Say once which enabled steps that cost, and the one knob that restores them; likewise for OOF.
    """
    try:
