...
# Per-fold FIT-row budget for the adversarial fold-selection diagnostic. It 5-fold cross-validates a LightGBM
# train-vs-test classifier; five full-size fits cost about two minutes on a 559k-row union, and a production run
# materialised 10.26M rows for a fit whose model list was a single CatBoost. The budget used to SKIP the diagnostic above
# 500k union rows, which disabled it on exactly the data sizes it matters for. Each fold now fits on at most this many
# rows (stratified subsample of its training part) while still predicting every held-out row, so every train row keeps
# an honest OOF score and the cost stays bounded at any size. ``max_union_rows=0`` restores unbounded fits.
ADVERSARIAL_FOLD_MAX_UNION_ROWS = 400_000
...
def adapt_adversarial_fold_selection(train_df, val_df, test_df, target_col, cat_features, group_ids, y, **kwargs) -> dict:
    ...
    try:
        ...
        _max_fit = int(kwargs.pop("max_union_rows", ADVERSARIAL_FOLD_MAX_UNION_ROWS) or 0)
        kwargs.setdefault("max_fit_rows", _max_fit)
