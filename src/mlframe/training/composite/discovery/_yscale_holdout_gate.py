...
def apply_structural_fragility_gate(
    ...
    for spec in kept_specs:
        ...
        except Exception as e:  # the same base is read at predict: a base the gate cannot read cannot be served
            # Logged and ledgered by the helper; kept out of ``rejected``, whose summary line formats fragility numbers.
            _gate_error_reject(self, spec, [], RejectStage.STRUCTURAL_FRAGILITY, f"base column {base_col!r} could not be read ({type(e).__name__}: {e})")
            continue
...
def _val_group_labels(self, val_df: Any, eval_idx: np.ndarray) -> np.ndarray | None:
    ...
    if not col or val_df is None:
        return None
    try:
        out: np.ndarray = np.asarray(_extract_column_array(val_df, col))[np.asarray(eval_idx)]
        return out
...
class _SharedTinyLgb:
    ...
    def __init__(self, params: dict, n_estimators: int) -> None:
        self.params, self.n_estimators = params, int(n_estimators)
...
def _fold_local_params(transform, y_fit: np.ndarray, base_fit: np.ndarray, valid: np.ndarray, groups_fit: np.ndarray | None) -> tuple[dict, np.ndarray] | None:
