...
def _rescore_spec_gain(
    ...
    valid = np.asarray(transform.domain_check(y, base), dtype=bool)  # type: ignore[type-var,arg-type]  # np.asarray(x, dtype=bool) is valid at runtime (numpy-stub quirk); base is None only for requires_base=False transforms, whose domain_check tolerates it
