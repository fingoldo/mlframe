...
def compute_candidates_exploration_scores(search_space:Sequence,known_candidates:Sequence):
    ...
    l = None
    for r in sorted(known_candidates):
        if l is None:
...
class MBHOptimizer():
    ...
    def __init__(self,
        ...
        if not isinstance(known_candidates,np.ndarray): known_candidates = np.array(known_candidates)
        if not isinstance(known_evaluations,np.ndarray): known_evaluations = np.array(known_evaluations)
        ...
        self.suggested_candidates=ExpiringDict(max_len=1e6,max_age_seconds=suggestions_cache_max_age_sec,)
        self.evaluated_candidates=[]
        self.last_retrain_ninputs=0        
        self.additional_info=""
        self.mode=""        
                        
        self.expected_fitness=None;self.y_pred=None;self.y_std=None
        ...
        if self.model_name=="CBQ":
            ...
            self.model = CatBoostRegressor(**model_params,loss_function=loss_function,verbose=0,)
    ...
    def submit_evaluations(self,candidates:Sequence,evaluations:Sequence,durations:Sequence):
        ...
        for next_candidate,next_evaluation,next_duration in zip(candidates,evaluations,durations):
            ...
            if (self.n_noimproving_iters==0 and self.plotting==OptimizationProgressPlotting.OnScoreImprovement) or self.plotting==OptimizationProgressPlotting.Regular:
                plot_search_state(
                    search_space=self.search_space,next_cand=next_candidate, new_y=next_evaluation,best_candidate=self.best_candidate, best_evaluation=self.best_evaluation,nsteps=self.nsteps,
                    expected_fitness=self.expected_fitness,y_pred=self.y_pred,y_std=self.y_std,ground_truth=self.ground_truth,
                    known_candidates=self.known_candidates, known_evaluations=self.known_evaluations,
...
def optimize_finite_onedimensional_search_space(search_space: Sequence,  # search space, all possible input combinations to check
    ...
    optimizer=MBHOptimizer(search_space=search_space,    
    ...
    while True:
        ...
        if next_candidate is None:
            if verbose: logger.info("Search space fully checked, quitting")
            break
        ...
        if max_noimproving_iters and optimizer.n_noimproving_iters>=max_noimproving_iters:
            if verbose: logger.info(f"Max # of noimproved iters reached: {optimizer.n_noimproving_iters}")   
            break 

    return (optimizer.best_candidate,optimizer.best_evaluation),optimizer.evaluated_candidates

def plot_search_state(
    search_space,next_cand:int, new_y:float,best_candidate:int, best_evaluation:float,nsteps:int,
    expected_fitness:np.ndarray,y_pred:np.ndarray,y_std:np.ndarray,ground_truth:np.ndarray,
    known_candidates:np.ndarray, known_evaluations:np.ndarray,
    ...
    if ground_truth is not None: axMain.plot(search_space, ground_truth, color="black", label="Ground truth")
