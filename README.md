...
| `tests/feature_selection/test_biz_val_filters_permutation.py` | Permutation MI: Besag-Clifford early-stop ≥3x faster than full budget on strong signal, BC stops before full budget, BC and full agree on no-signal verdict, prange reproducibility across runs, npermutations=0 guard (5 tests) |
| `tests/feature_selection/test_biz_val_filters_gpu.py` | GPU MI (CuPy RawKernel): `mi_direct_gpu_batched` ≥1.5x faster than CPU at n=10k, scales to n=200k under 30s, OOM-safe fallback at small batch_size (4 tests; skipped if cupy missing) |
| `tests/training/test_biz_val_training_composite_discovery.py` | CompositeTargetDiscovery: pipeline runs clean on logratio / diff / linear targets, spec schema validates on linear, `screening='hybrid'` runs tiny rerank, `mi_estimator='bin'` default, `fail_on_no_gain='fallback_raw'` graceful (6 tests) |
| `tests/training/test_biz_val_training_baseline_diagnostics.py` | BaselineDiagnostics: ablation rank-1 is the dominant feature, dominant_features list rank-aligned, `sample_n` caps runtime <30s on n=50k, `enabled=False` short-circuits in <1s, report has init-score field (5 tests) |
