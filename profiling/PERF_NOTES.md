...
| **10** | **CRITICAL DEADLOCK** -- win #5 (persistent kaleido server) regression: a single figure that triggers a JS error inside kaleido (`KaleidoError: Error 525...`) cancels the asyncio task chain and every subsequent `write_fig_sync` hangs forever in `await asyncio.gather`. Reproduced empirically: c0031 hung 2+ hours on `multiclass + recency + ensembling` combo. | **v1**: catch all exceptions, restart server, retry oneshot. **v2**: hard 30s threading.join timeout to handle the case where the call doesn't even raise. **v3**: when burned, write HTML directly instead of falling through to oneshot (which uses the SAME broken kaleido server). **v4**: threshold=2 consecutive failures before burning (cap waste at 60s instead of 6min). | **c0031: 2+h hang -> 113s bounded**. v2 (84s when kaleido happened not to crash) -> v3 hangs again -> v4 (this) at 113s with full reliability guarantee. |
| 11 | `strategies.build_polars_enum_map` did N collects per cat col across 2 frames (HGB + XGB share the same loop body) | batch via `select([pl.col(c).cast(pl.String).drop_nulls().unique().implode().alias(c) for c in cols]).collect()` | bench (warmed, 80k rows x 15 cat cols, train+val): 16.5ms (was ~250-300ms estimated based on 60 collects x ~5ms in c0031 profile). Equivalence: identical Enum levels + null handling. 93 strategy tests still pass. |
| 12 | `pipeline._select_scalable_numeric_columns` did 1+2 collects per numeric col (n_non_null check + 2 stat queries for the chosen scaler method) | one lazy select with all per-col stats expressions, fallback to per-col loop on any error | bench (warmed, 80k rows x 15 cols): robust 6.6ms / standard 2.3ms / min_max 3.0ms (was ~135ms cumulative for robust on c0031). Equivalence: identical scalable list across good/all-null/constant/zero-std/nans/inf inputs. 13 pipeline tests still pass. |
