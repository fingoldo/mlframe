...
def test_biz_val_geometric_weight_blend_hybrid_alpha_recovers_from_near_zero_glitch():
    ...
    assert improvement > 0.03, f"expected hybrid blend to beat pure geometric blend by >3% log-loss on the glitch-prone dataset, got improvement={improvement:.4f} (pure_geo={pure_geo['loss']:.4f}, hybrid={hybrid['loss']:.4f})"
