...
def test_report_cindex_byte_identical_to_standalone_concordance(seed: int, n: int):
    ...
    report_regression_model_perf(
        targets=yt, columns=["a"], model_name="m", model=None,
        preds=yp, metrics=m, print_report=False, show_perf_chart=False,
    )

    ref = fast_concordance_index(yt, yp)
    assert m["ConcordanceIndex"] == ref, (
        f"ConcordanceIndex {m['ConcordanceIndex']!r} != standalone "
        f"fast_concordance_index {ref!r} (seed={seed}, n={n})"
    )
