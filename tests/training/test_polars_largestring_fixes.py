        ...
        df = pl.DataFrame(
            {
                ...
                "category_grp": pl.Series("category_grp", ([None] * 2 + ["x"] * (n - 2)), dtype=pl.String).cast(pl.Categorical),
                "hourly_budget": pl.Series("hourly_budget", [None] * n, dtype=pl.String).cast(pl.Categorical),
            }
        )
        assert _polars_df_has_null_in_categorical(df, cat_features=["job_type", "category_grp", "hourly_budget"])
...
class TestPolarsNullableCategoricalColsDetector:
    ...
    def test_returns_only_nullable_cat_cols(self):
        ...
        assert out == ["dirty"], "must return ONLY the Categorical column with nulls — not clean cats, not non-cat columns even if they have nulls"
    ...
    def test_empty_input_returns_empty_list(self):
        ...
        assert _polars_nullable_categorical_cols(pl.DataFrame({"n": [1.0, 2.0]})) == []
