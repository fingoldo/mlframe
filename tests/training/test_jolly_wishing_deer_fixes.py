...
# ---------------------------------------------------------------------------
# encode_cats_as_codes — the LGB Polars→pandas categorical crash fix
# ---------------------------------------------------------------------------

def test_encode_cats_as_codes_converts_categorical_to_int():
    """pd.Categorical columns in cat_features must be encoded as int16 codes.

    Root cause: LGB's _LGBMValidateData → sklearn check_array converts pd.Categorical
    to numpy object array of strings, then LGB's Dataset.__init_from_np2d raises
    ValueError: could not convert string to float: 'HOURLY'.
    encode_cats_as_codes must replace those columns with integer codes before LGB sees them.
    """
    from mlframe.training.utils import encode_cats_as_codes

    df = pd.DataFrame({
        "cat_col": pd.Categorical(["HOURLY", "FIXED", "HOURLY", "FIXED"]),
        "obj_col": pd.array(["A", "B", "A", "C"], dtype=object),
        "num_col": [1.0, 2.0, 3.0, 4.0],
    })
    result = encode_cats_as_codes(df, cat_features=["cat_col", "obj_col"])

    assert result["cat_col"].dtype == np.int16, "cat column should be int16 codes"
    assert result["num_col"].dtype == float, "numeric column must be untouched"
    assert result["obj_col"].dtype == np.int16, "object column should be int16 codes"
    # Codes are non-negative integers
    assert (result["cat_col"] >= 0).all()
    # Original df not mutated
    assert df["cat_col"].dtype.name == "category"


def test_encode_cats_as_codes_polars_enum_roundtrip():
    """Full path: Polars Enum → pandas → encode_cats_as_codes → LGB-safe integers."""
    pytest.importorskip("lightgbm")
    from mlframe.training.utils import encode_cats_as_codes, get_pandas_view_of_polars_df

    rng = np.random.default_rng(42)
    n = 200
    categories = ["HOURLY", "FIXED", "MILESTONE"]
    enum_type = pl.Enum(categories)
    pl_df = pl.DataFrame({
        "budget_type": pl.Series([categories[i % 3] for i in range(n)]).cast(enum_type),
        "num_feat": rng.standard_normal(n).astype(np.float32),
    })

    pd_df = get_pandas_view_of_polars_df(pl_df)
    encoded = encode_cats_as_codes(pd_df, cat_features=["budget_type"])

    assert encoded["budget_type"].dtype == np.int16
    assert encoded["num_feat"].dtype == np.float32

    # LGB must be able to fit on this without ValueError
    import lightgbm as lgb
    y = rng.integers(0, 2, size=n)
    model = lgb.LGBMClassifier(n_estimators=5, verbose=-1)
    model.fit(encoded, y, categorical_feature=["budget_type"])
    preds = model.predict_proba(encoded)
