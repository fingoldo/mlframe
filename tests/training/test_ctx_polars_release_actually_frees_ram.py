...
def test_release_no_warning_when_rss_drops_enough(caplog):
    ...
    expected_mb = sum(
        float(getattr(ctx, a).estimated_size("mb")) for a in ("train_df_polars", "val_df_polars", "test_df_polars")
    )
    ...
    with mock.patch(
        ...
        "mlframe.training.core._phase_train_one_target.maybe_clean_ram_and_gpu",
        return_value=0.0,
    ), caplog.at_level(logging.WARNING, logger="mlframe.training.core._phase_train_one_target"):
