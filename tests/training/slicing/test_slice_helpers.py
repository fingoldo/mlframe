...
def test_random_shards_stratified_classification(clf_val_df: tuple[pd.DataFrame, np.ndarray]) -> None:
    ...
    for s in shards:
        ...
        assert abs(pos_rate - overall_pos_rate) <= 0.10, f"stratified shard pos rate {pos_rate} too far from overall {overall_pos_rate}"
...
def test_random_shards_with_group_ids_switches_to_groupkfold(
    ...
    with caplog.at_level(logging.WARNING):
        shards = build_slice_eval_sets(X, y, source="random", k=4, min_rows_per_shard=20, random_state=42, group_ids=group_ids)
...
def test_temporal_shards_preserve_order(reg_val_df: tuple[pd.DataFrame, np.ndarray]) -> None:
    ...
    shards = build_slice_eval_sets(X, y, source="temporal", k=4, min_rows_per_shard=10, random_state=42, time_values=time_values)
    assert len(shards) == 4
    ...
    for i, s in enumerate(shards):
        ...
        assert np.array_equal(np.sort(s.row_indices), np.sort(expected)), "temporal shard rows must match the contiguous time-window indices"
...
def test_fairness_shards_from_indexed_subgroups(reg_val_df: tuple[pd.DataFrame, np.ndarray]) -> None:
    ...
    shards = build_slice_eval_sets(X, y, source="fairness", k=5, min_rows_per_shard=30, random_state=42, indexed_subgroups=indexed_subgroups)
    assert len(shards) == 2
...
def test_sample_weight_aligned(reg_val_df: tuple[pd.DataFrame, np.ndarray]) -> None:
    ...
    shards = build_slice_eval_sets(X, y, source="random", k=4, min_rows_per_shard=10, random_state=42, sample_weight=sample_weight)
    for s in shards:
