...
def test_soft_winsorize_bad_distribution_raises_value_error():
    ...
    with pytest.raises(ValueError, match="distribution must be"):
        soft_winsorize(
            ...
            rel_lower_limit=0.1,
            rel_upper_limit=0.1,
            abs_lower_threshold=1.0,
            abs_upper_threshold=10.0,
            distribution="banana",
        )
...
def test_mrmr_select_features_input_checks_raise_value_error():
    ...
    with pytest.raises(ValueError, match="mrmr_relevance_algo must be"):
        screen_predictors(factors_data=fd, factors_nbins=[3, 3, 3], targets_data=td, targets_nbins=[2], y=[0], mrmr_relevance_algo="banana")
    ...
    with pytest.raises(ValueError, match="at least 10 rows"):
        screen_predictors(factors_data=fd[:5], factors_nbins=[3, 3, 3], targets_data=td[:5], targets_nbins=[2], y=[0])
    with pytest.raises(ValueError, match=r"must equal len\(targets_nbins\)"):
        screen_predictors(factors_data=fd, factors_nbins=[3, 3, 3], targets_data=td, targets_nbins=[2, 2], y=[0])
...
def test_input_validation_raises_in_targeted_modules():
    ...
    with pytest.raises(ValueError, match="distribution must be"):
        soft_winsorize(
            np.array([1.0, 2.0, 3.0, 100.0]), rel_lower_limit=0.1, rel_upper_limit=0.1, abs_lower_threshold=1.0, abs_upper_threshold=10.0, distribution="banana"
        )
    with pytest.raises(ValueError, match="method must be"):
        clip_to_quantiles(np.array([1.0, 2.0, 3.0]), method="banana")
