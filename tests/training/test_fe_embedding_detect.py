...
def test_pl_list_int8_detected_as_embedding():
    ...
    text_features, embedding_features, _ = _auto_detect_feature_types(
        df, feature_types_config=cfg, cat_features=[], verbose=False,
    )
    assert "quant_emb" in embedding_features, (
        f"List(Int8) column not detected as embedding; "
        f"text={text_features}, emb={embedding_features}"
    )
...
def test_pl_array_float32_detected_as_embedding():
    ...
    assert "fixed_emb" in embedding_features, (
        f"pl.Array(Float32, 4) column not detected as embedding; "
        f"text={text_features}, emb={embedding_features}"
    )
