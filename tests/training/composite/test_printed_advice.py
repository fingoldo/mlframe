...
def test_kfold_one_uses_the_external_holdout(caplog):
    ...
    kw = dict(component_models=[model], component_names=["m"], component_specs=[None], train_X=X, y_train_full=y,
              base_train_full_per_spec={}, holdout_frac=0.2, random_state=0, external_holdout_X=Xv, external_holdout_y=yv)
    with caplog.at_level(logging.WARNING):
...
def test_the_ensemble_builders_accept_what_their_errors_ask_for():
    ...
    for kw in ({"component_oof_rmse": [1.0, 2.0]}, {"component_train_rmse": [1.0, 2.0]}):
        CompositeCrossTargetEnsemble.from_train_metrics(component_models=[m, m], component_names=["a", "b"], baseline_oof_rmse=3.0, baseline_train_rmse=3.0, **kw)
...
def test_an_oof_baseline_is_used_and_silences_the_scale_warning(caplog):
    ...
    caplog.clear()
    with caplog.at_level(logging.WARNING):
        ens = CompositeCrossTargetEnsemble.from_train_metrics(component_models=m, component_names=["a", "b"], component_oof_rmse=[1.0, 1.5], baseline_oof_rmse=2.0)
...
def test_oof_rmses_replace_the_train_rmse_ranking(caplog):
    ...
    m = [LinearRegression(), LinearRegression()]
    with caplog.at_level(logging.WARNING):
        CompositeCrossTargetEnsemble.from_train_metrics(component_models=m, component_names=["a", "b"], component_train_rmse=[1.0, 1.5], baseline_train_rmse=2.0)
    assert any("biased optimistic" in m_ for m_ in _warnings(caplog))
    caplog.clear()
    with caplog.at_level(logging.WARNING):
        ens = CompositeCrossTargetEnsemble.from_train_metrics(component_models=m, component_names=["a", "b"], component_train_rmse=[1.0, 1.5],
                                                              component_oof_rmse=[1.5, 1.0], baseline_oof_rmse=2.0)
    assert not _warnings(caplog)
    w = np.asarray(ens.weights, dtype=float)
...
def test_the_estimators_accept_what_their_errors_ask_for():
    ...
    CompositeMultiOutputEstimator(base_estimator=LinearRegression(), column_specs={"transform_name": "diff", "base_column": "b"}, skip_failed_columns=False).fit(X, Y)
    y = Y[:, 0]
...
def test_the_feature_generator_accepts_a_wrapper_factory():
    ...
    gen = CompositeFeatureGenerator(wrapper_factory=lambda: CompositeTargetEstimator(base_estimator=LinearRegression(), transform_name="diff", base_column="b"),
                                    column_name="f", n_splits=3)
