...
def test_biz_val_residualise_removes_linear_component():
    ...
    assert abs(corr) < 1e-6, (
        f"residual must have zero correlation with x; got {corr:.6f}"
    )
    assert abs(float(np.mean(resid))) < 1e-12, (
        f"mean residual must be zero; got {np.mean(resid):.6e}"
    )
...
def test_biz_val_diff_transform_roundtrip_identity():
    ...
    assert max_err < 1e-12, (
        f"diff roundtrip must be exact; got max|error|={max_err:.2e}"
    )
...
def test_biz_val_linear_residual_transform_roundtrip():
    ...
    assert max_err < 1e-10, (
        f"linear_residual roundtrip must be nearly exact; "
        f"got max|error|={max_err:.2e}"
    )
...
def test_biz_val_linear_residual_fit_recovers_true_coefficients():
    ...
    assert slope is not None and abs(slope - 2.5) < 0.1, (
        f"slope must be ~2.5; got {slope:.4f}"
    )
    assert abs(intercept - 1.0) < 0.1, (
        f"intercept must be ~1.0; got {intercept:.4f}"
    )
...
def test_biz_val_ratio_transform_finite_roundtrip():
    ...
    max_err = float(np.max(np.abs(y_roundtrip - y)))
    assert max_err < 1e-12, (
        f"ratio roundtrip must be exact; got max|error|={max_err:.2e}"
    )
