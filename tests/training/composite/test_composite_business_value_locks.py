...
class TestLockRegimeGate:
    ...
    def test_per_bin_gate_rejects_wrong_logratio(self) -> None:
        ...
        assert len(d_off.specs_) >= 1, "regression: OFF should have kept the bad logratio spec (global gate too lenient)"
        assert len(d_on.specs_) == 0, f"regression: per-bin gate should have rejected the bad logratio spec; got {[(s.name) for s in d_on.specs_]}"
...
class TestLockPermutationNull:
    ...
    def test_null_filter_reduces_pure_noise_false_positives(self) -> None:
        ...
        for rep in range(n_reps):
            ...
            n = 250
            data = {f"noise_{c}": rng.normal(size=n) for c in ["a", "b", "c", "d", "e", "f", "g", "h"]}
            ...
            cfg_on = _disc_with_kwargs(
                auto_base_null_perms=30,
                auto_base_null_z_threshold=2.0,
                auto_base_top_k=1,
                transforms=["linear_residual"],
    ...
    def test_null_centred_gain_separates_noise_from_signal(self) -> None:
        ...
        def gain(x, y, n_perm=20):
            ...
            for _ in range(n_perm):
                ...
                null_vals.append(float(np.mean([_plugin_mi_regression_njit(x[:, j].copy(), yp, 16) for j in range(x.shape[1])])))
        ...
        assert abs(gain_noise) < 0.05, f"regression: noise gain {gain_noise:+.4f} should be ~0 (null filter calibrated correctly)"
        # Signal gain clearly above noise gain.
        assert gain_signal > 0.10, f"regression: signal gain {gain_signal:+.4f} should be >= 0.10 to clearly separate signal from noise"
        assert gain_signal > gain_noise + 0.10, f"regression: gain separation {gain_signal - gain_noise:+.4f} should be >= 0.10"
