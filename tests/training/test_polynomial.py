...
def test_polynomial_flips_interaction_only_when_over_byte_cap(caplog) -> None:
    ...
    assert any(
        "interaction_only=True" in rec.message and "polynomial" in rec.message for rec in caplog.records
...
def test_polynomial_skips_entirely_when_even_degree_1_too_big(caplog) -> None:
    ...
    assert any(
        "skipping" in rec.message and "polynomial" in rec.message for rec in caplog.records
...
def test_polynomial_fits_when_under_byte_cap_no_warn(caplog) -> None:
    ...
    assert not any(
        "polynomial" in m and ("flipping" in m or "decrementing" in m or "skipping" in m) for m in _warn_msgs
