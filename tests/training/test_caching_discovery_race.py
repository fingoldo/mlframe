...
def test_get_returns_default_on_missing_file_without_raising():
    with tempfile.TemporaryDirectory() as tmpdir:
        cache = DiscoveryCache(tmpdir)
        assert cache.get("deadbeef", default="fallback") == "fallback"


def test_get_treats_delete_between_check_and_open_as_miss():
...
def test_get_treats_corrupt_pickle_as_miss():
