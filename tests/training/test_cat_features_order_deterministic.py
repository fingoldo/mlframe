...
def _run(cat_features, cat_features_polars):
    ...
    out = _phase_auto_detect_feature_types(
        train_df=df, val_df=None, test_df=None,
        train_df_polars_pre=None, val_df_polars_pre=None, test_df_polars_pre=None,
        cat_features=list(cat_features), cat_features_polars=list(cat_features_polars),
        was_polars_input=False, all_models_polars_native=False,
        pipeline_config=None, feature_types_config=None,
        metadata=metadata, verbose=False, train_df_pandas_pre_meta=None,
    )
