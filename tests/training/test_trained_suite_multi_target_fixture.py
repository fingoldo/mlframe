...
def test_multi_target_fixture_has_two_regression_targets(trained_suite_multi_target):
    ...
    assert set(reg_targets) >= {"target", "target_extra"}, (
        f"expected both regression targets trained in one fit; got {list(reg_targets)}"
    )
    ...
    for tname, ns_list in reg_targets.items():
        ...
        assert any(getattr(ns, "model", None) is not None for ns in ns_list), (
            f"target {tname!r} has no estimator on any namespace"
        )
