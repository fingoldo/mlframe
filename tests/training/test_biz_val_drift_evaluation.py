...
def test_biz_val_drift_compute_label_binary_detects_shift():
    ...
    assert max_pp > 10.0, f"50/50 train vs 100% positive val must have >10pp max drift; got {max_pp:.1f}"
...
def test_biz_val_eval_rmse_constant_offset():
    ...
    assert abs(rmse - c) < 1e-12, f"RMSE of y vs y+{c} must be {c}; got {rmse:.6f}"
