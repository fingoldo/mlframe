...
def test_target_encoder_cache_does_not_collide_across_targets():
    ...
    assert stats["n_keys"] == 2, (
        f"distinct targets must occupy distinct cache slots; cache has {stats['n_keys']} entries"
    )
    ...
    assert not np.allclose(a_arr, b_arr), (
        "target-encoder OOF must differ across distinct targets; pre-fix the second call replayed the first"
    )
