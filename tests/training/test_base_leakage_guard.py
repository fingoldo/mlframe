...
def test_leakage_guard_drops_monotone_reencoding_keeps_lag():
    ...
    assert disc._base_leakage_guard_ran_ is True, "the guard never scanned, so the drop below would prove nothing"
    dropped = {name for name, _reason in disc._leaky_bases_dropped_}
...
def test_leakage_guard_noop_without_time_ordering():
    ...
    # Read the attributes directly rather than through a getattr default: "the guard ran and dropped nothing"
    # and "fit never reached the guard at all" are both an absent attribute, and only the first is the contract.
    assert disc._base_leakage_guard_ran_ is False, "the guard scanned despite no time_ordering; autocorrelation can be read as leakage"
    assert disc._leaky_bases_dropped_ == [], f"an inert guard dropped bases: {disc._leaky_bases_dropped_}"
