...
from mlframe.training.core._volatility_lag_router import (
    VolatilityLagRouter,
    _group_local_volatility,
    build_volatility_lag_router,
)
...
class TestBuilderValGate:
    ...
    def test_builder_measures_via_ctx_group_ids_but_declines_when_group_column_dropped(self):
        ...
        for w in range(30):
            ...
            for i in range(n):
                rows.append((md[i], lag[i]))
                raw_pred.append(model[i])
                lag_pred.append(lag[i])
                y.append(truth[i])
                gids.append(w)
    ...
    def test_biz_val_router_beats_both_all_raw_and_all_lag(self):
        ...
        for w in range(40):
            ...
            lag = np.empty(n)
            lag[0] = truth[0]
            lag[1:] = truth[:-1]  # exact previous value
