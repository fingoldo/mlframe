...
def test_main_module_imports_reset_session_from_fingerprint():
    ...
    assert hasattr(core_main, "reset_fh_session"), (
        "core.main must import reset_session from feature_handling.fingerprint so suite entry "
        "can rotate the FH cache namespace"
    )
...
def test_suite_entry_rotates_fh_session_token():
    ...
    assert token_after != token_before, (
        "suite entry must rotate the FH session token; without it consecutive suites can collide "
        "on recycled id(train_df) for in-memory cache entries"
    )
...
def test_two_consecutive_suite_calls_produce_distinct_session_tokens():
    ...
    def _capture(*args, **kwargs):
        ...
        with pytest.raises(TypeError):
            core_main.train_mlframe_models_suite(
                df=999, target_name="y", model_name="m",
    ...
    assert seen[0] != seen[1], (
        "consecutive suite calls must produce distinct FH session tokens"
    )
