...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("reporting/charts/multiclass.py", "argmax_classes_safe"),
        # reporting/charts/ltr.py uses inline nanargmax pattern:
        ("reporting/charts/ltr.py", "np.nanargmax(scores_q)"),
        ...
        ("metrics/_fairness_metrics.py", "np.nanquantile(performances"),
        # ``compute_member_quality_gate`` lives in ``models/ensembling/quality_gate.py``.
        ("models/ensembling/quality_gate.py", "np.nanmedian(per_member_mae)"),
        ("models/ensembling/quality_gate.py", "np.nanmedian(per_member_std)"),
        # feature_selection/general.py:200/202 -> nanmax/nanquantile:
        ("feature_selection/general.py", "np.nanquantile(all_permuted_mis"),
...
def test_wave21_production_site_migrated(rel, must_contain):
    ...
    if rel == "training/core/predict.py":
        ...
        for _nm in ("_predict_main.py", "_predict_main_from_models.py", "_predict_main_suite.py", "_predict_pre_pipeline.py"):
            _p = _dir / _nm
            if _p.exists():
