...
def test_sgd_regressor_wraps_via_partial_fit(reg_data) -> None:
    ...
    model = SGDRegressor(max_iter=1, tol=None, random_state=0,
                          learning_rate="constant", eta0=0.01)
    wrapped, did = maybe_wrap_for_partial_fit_es(model, model_category="sgd",
...
def test_sgd_classifier_wraps_with_classification_flag(clf_data) -> None:
    ...
    model = SGDClassifier(loss="log_loss", max_iter=1, tol=None, random_state=0,
                           learning_rate="constant", eta0=0.01)
    wrapped, did = maybe_wrap_for_partial_fit_es(model, model_category="sgd",
                                                   X_val=X_val, y_val=y_val,
...
def test_ridge_wraps_via_dichotomic(reg_data) -> None:
    ...
    wrapped, did = maybe_wrap_for_partial_fit_es(model, model_category="ridge",
                                                   X_val=X_val, y_val=y_val,
                                                   is_classification=False)
    assert did is True
    wrapped.fit(X_tr, y_tr)
...
def test_linear_regression_passes_through(reg_data) -> None:
    ...
    wrapped, did = maybe_wrap_for_partial_fit_es(model, model_category="linear",
                                                   X_val=X_val, y_val=y_val,
                                                   is_classification=False)
    assert did is False
...
def test_native_es_categories_pass_through(reg_data) -> None:
    ...
    for cat in ["lgb", "xgb", "cb", "hgb", "ngb", "mlp", "tabnet"]:
        ...
        wrapped, did = maybe_wrap_for_partial_fit_es(model, model_category=cat,
                                                       X_val=X_val, y_val=y_val,
                                                       is_classification=False)
...
def test_already_wrapped_idempotent(reg_data) -> None:
    ...
    once, did1 = maybe_wrap_for_partial_fit_es(m, model_category="sgd",
                                                  X_val=X_val, y_val=y_val,
                                                  is_classification=False)
    twice, did2 = maybe_wrap_for_partial_fit_es(once, model_category="sgd",
                                                  X_val=X_val, y_val=y_val,
                                                  is_classification=False)
    assert did1 is True
...
def test_getattr_passes_coef_after_fit(reg_data) -> None:
    ...
    wrapped = PartialFitESWrapper(
        SGDRegressor(max_iter=1, tol=None, random_state=0,
                      learning_rate="constant", eta0=0.01),
        metric="rmse", patience=10, max_iter=20,
        external_X_val=X_val, external_y_val=y_val,
    )
    wrapped.fit(X_tr, y_tr)
    coef = wrapped.coef_
    assert coef.shape == (4,)
...
def test_getattr_falls_back_to_estimator_for_unknown_attrs() -> None:
    ...
    wrapper = PartialFitESWrapper(rf, metric="rmse", budget_param="n_estimators",
                                    budget_min=2, budget_max=10)
...
def test_external_val_set_used_when_fit_called_without_x_val(reg_data) -> None:
    ...
    wrapper = PartialFitESWrapper(
        SGDRegressor(max_iter=1, tol=None, random_state=0,
                      learning_rate="constant", eta0=0.01),
        metric="rmse", patience=5, max_iter=20,
        external_X_val=X_val, external_y_val=y_val,
    )
...
def test_external_val_overridden_by_explicit_fit_kwargs(reg_data) -> None:
    ...
    wrapper = PartialFitESWrapper(
        SGDRegressor(max_iter=1, tol=None, random_state=0,
                      learning_rate="constant", eta0=0.01),
        metric="rmse", patience=5, max_iter=10,
        external_X_val=X_val, external_y_val=y_val,
    )
