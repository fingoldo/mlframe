...
def small_text_df():
    ...
    texts = [
        f"alpha beta gamma {i} foo bar baz qux quux corge grault garply"
        for i in range(60)
    ]
    return pd.DataFrame({
        "text_col": texts,
...
def test_tfidf_sparse_dataframe_has_sparsedtype_columns(small_text_df):
    cfg = PreprocessingExtensionsConfig(
        tfidf_columns=["text_col"], tfidf_max_features=50, tfidf_keep_sparse=True,
    )
    ...
    for c in tfidf_cols:
        assert isinstance(out[c].dtype, pd.SparseDtype), (
            f"Expected SparseDtype for column {c}, got {out[c].dtype}"
        )


def test_tfidf_dense_path_when_keep_sparse_false(small_text_df):
    cfg = PreprocessingExtensionsConfig(
        tfidf_columns=["text_col"], tfidf_max_features=50, tfidf_keep_sparse=False,
    )
...
def test_sparse_uses_less_memory(small_text_df):
    ...
    cfg_sparse = PreprocessingExtensionsConfig(
        tfidf_columns=["text_col"], tfidf_max_features=200, tfidf_keep_sparse=True,
    )
    out_sparse, _, _, _ = apply_preprocessing_extensions(
        small_text_df.copy(), None, None, cfg_sparse, verbose=0,
    )
    cfg_dense = PreprocessingExtensionsConfig(
        tfidf_columns=["text_col"], tfidf_max_features=200, tfidf_keep_sparse=False,
    )
    out_dense, _, _, _ = apply_preprocessing_extensions(
        small_text_df.copy(), None, None, cfg_dense, verbose=0,
    )
    ...
    assert mem_sparse < mem_dense, (
        f"Expected sparse to be smaller; got sparse={mem_sparse} bytes vs dense={mem_dense} bytes"
    )
...
def test_sparse_and_dense_produce_equivalent_values(small_text_df):
    ...
    cfg_s = PreprocessingExtensionsConfig(
        tfidf_columns=["text_col"], tfidf_max_features=80, tfidf_keep_sparse=True,
    )
    ...
    cfg_d = PreprocessingExtensionsConfig(
        tfidf_columns=["text_col"], tfidf_max_features=80, tfidf_keep_sparse=False,
    )
...
def test_sparse_train_val_test_alignment(small_text_df):
    ...
    cfg = PreprocessingExtensionsConfig(
        tfidf_columns=["text_col"], tfidf_max_features=40, tfidf_keep_sparse=True,
    )
    tr, va, te, _ = apply_preprocessing_extensions(
        small_text_df, val_df, test_df, cfg, verbose=0,
    )
