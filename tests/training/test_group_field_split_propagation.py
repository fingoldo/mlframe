...
class TestGroupFieldEndToEnd:
    ...
    def _build_problem(self, n_groups: int = 20, rows_per_group: int = 100, seed: int = 42):
        ...
        for g in range(n_groups):
            ...
            for i in range(rows_per_group):
                rows.append({
                    "x_a": float(x_a[i]), "x_b": float(x_b[i]),
                    "y": float(y[i]), "well_id": f"well_{g:03d}",
                })
    ...
    def test_groups_never_leak_across_splits(self) -> None:
        ...
        assert group_ids is not None, (
            "extractor with group_field set should produce group_ids"
        )
        assert len(group_ids) == len(df), "group_ids shape mismatch"

        cfg = TrainingSplitConfig(
            shuffle_val=True, shuffle_test=True,
        ...
        assert not cross_train_val, (
            f"well_id leakage train<->val: {cross_train_val}"
        )
        assert not cross_train_test, (
            f"well_id leakage train<->test: {cross_train_test}"
        )
        assert not cross_val_test, (
            f"well_id leakage val<->test: {cross_val_test}"
        )
    ...
    def test_use_groups_false_explicitly_disables(self) -> None:
        ...
        extractor = SimpleFeaturesAndTargetsExtractor(
            regression_targets=["y"], group_field="well_id", verbose=0,
        )
        ...
        wells_test = set(wells[test_idx])
        any_overlap = bool(
            (wells_train & wells_val)
            or (wells_train & wells_test)
            or (wells_val & wells_test)
        )
        # Probabilistically guaranteed with these sizes; if this ever
        # flakes, increase n_groups / rows_per_group.
        assert any_overlap, (
            "use_groups=False should produce well leakage on this size; "
            "got isolated wells across splits which is suspicious"
        )
...
class TestSplitterEmitsGroupAwareInfoLine:
    ...
    def test_enabled_log_when_groups_supplied(self, caplog) -> None:
        ...
        with caplog.at_level(logging.INFO, logger="mlframe.training.splitting"):
            make_train_test_split(
                df=df, timestamps=None, stratify_y=None, groups=groups,
                test_size=0.1, val_size=0.1,
                shuffle_val=True, shuffle_test=True,
    ...
    def test_disabled_log_when_groups_none(self, caplog) -> None:
        ...
        with caplog.at_level(logging.INFO, logger="mlframe.training.splitting"):
            make_train_test_split(
                df=df, timestamps=None, stratify_y=None, groups=None,
                test_size=0.1, val_size=0.1,
                shuffle_val=True, shuffle_test=True,
                val_placement="forward",
            )
        assert any(
            "Group-aware splitting: disabled" in r.message for r in caplog.records
        )
