...
def test_partial_fit_classification_sgd_uses_predict_proba() -> None:
    ...
    wrapper = PartialFitESWrapper(
        SGDClassifier(loss="log_loss", max_iter=1, tol=None, random_state=0, learning_rate="constant", eta0=0.01),
        metric="logloss",
        patience=15,
        max_iter=80,
        val_size=0.2,
...
def test_partial_fit_es_stops_on_divergence() -> None:
    ...
    X, y = _make_regression()
    wrapper = PartialFitESWrapper(
        SGDRegressor(max_iter=1, tol=None, random_state=0, learning_rate="constant", eta0=10.0),  # blow-up rate
        metric="rmse",
        patience=10,
        max_iter=80,
    )
