...
def test_maybe_run_confidence_analysis_noop_paths():
    ...
    maybe_run_confidence_analysis(
        run_test=False, confidence=SimpleNamespace(include=True), test_df=object(),
        test_target=None, test_probs=None, fit_params=None, model_type_name="X",
        figsize=None, verbose=0,
    )
    # include False -> no-op
    maybe_run_confidence_analysis(
        run_test=True, confidence=conf_off, test_df=object(), test_target=None,
        test_probs=None, fit_params=None, model_type_name="X", figsize=None, verbose=0,
    )
    # test_df None -> no-op
    maybe_run_confidence_analysis(
        run_test=True, confidence=SimpleNamespace(include=True), test_df=None,
        test_target=None, test_probs=None, fit_params=None, model_type_name="X",
        figsize=None, verbose=0,
    )
