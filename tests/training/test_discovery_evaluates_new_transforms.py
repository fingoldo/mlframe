...
class TestDiscoveryEvaluatesNewTransforms:
    def test_discovery_runs_without_crash(
        self, synthetic_heavy_tail_df: tuple[pd.DataFrame, np.ndarray, np.ndarray],
    ) -> None:
    ...
    def test_all_14_transforms_evaluated(
        ...
        evaluated = {
            ...
            if isinstance(r, dict) and r.get("transform_name")
        }
        missing = EXPECTED_TRANSFORMS - evaluated
        assert not missing, (
            f"Discovery did NOT evaluate {sorted(missing)}; got {sorted(evaluated)}"
        )
    ...
    def test_unary_dedup_runs_each_unary_once(
        ...
        for unary in ("cbrt_y", "log_y", "yeo_johnson_y", "quantile_normal_y"):
            assert counts[unary] == 1, (
                f"unary '{unary}' evaluated {counts[unary]} times -- dedup did not apply"
            )
    ...
    def test_chain_transforms_evaluated_per_base(
        ...
        for chain in (
            ...
            assert counts[chain] >= 1, (
                f"chain '{chain}' not evaluated at all"
            )
