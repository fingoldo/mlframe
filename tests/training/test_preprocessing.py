...
class TestTrainTestSplit:
    ...
    def test_basic_split(self, sample_regression_data):
        ...
        train_idx, val_idx, test_idx, _, _, _ = make_train_test_split(df, **config.model_dump(exclude={"use_groups", "calib_size"}), timestamps=None)
    ...
    def test_split_with_timestamps(self, sample_timeseries_data):
        ...
        train_idx, val_idx, test_idx, _, _, _ = make_train_test_split(df, **config.model_dump(exclude={"use_groups", "calib_size"}), timestamps=timestamps)
    ...
    def test_split_with_shuffling(self, sample_regression_data):
        ...
        train_idx1, val_idx1, test_idx1, _, _, _ = make_train_test_split(df, **config.model_dump(exclude={"use_groups", "calib_size"}), timestamps=None)

        # Run again with same seed
        train_idx2, val_idx2, test_idx2, _, _, _ = make_train_test_split(df, **config.model_dump(exclude={"use_groups", "calib_size"}), timestamps=None)
