...
def _isolated_cache(tmp_path, monkeypatch):
    ...
    eb._per_member_use_numba_2d.cache_clear()
...
def test_env_override_forces_backend(monkeypatch):
    ...
    eb._per_member_use_numba_2d.cache_clear()
    assert eb._per_member_use_numba_2d(500_000, 8) is False  # would otherwise be numba
    monkeypatch.setenv("MLFRAME_PER_MEMBER_BACKEND", "numba")
    eb._per_member_use_numba_2d.cache_clear()
    assert eb._per_member_use_numba_2d(100, 2) is True  # would otherwise be sub-floor numpy
...
def test_fallback_floor(monkeypatch):
    ...
    eb._per_member_use_numba_2d.cache_clear()
    below = eb._per_member_use_numba_2d(eb._PER_MEMBER_NUMBA_FLOOR_ELEMENTS - 1, 4)
    above = eb._per_member_use_numba_2d(eb._PER_MEMBER_NUMBA_FLOOR_ELEMENTS + 1, 4)
...
def test_ensure_tuning_populates_cache_and_dispatch_reads_it():
    ...
    eb._per_member_use_numba_2d.cache_clear()
    decision = eb._per_member_use_numba_2d(50_000, 4)
