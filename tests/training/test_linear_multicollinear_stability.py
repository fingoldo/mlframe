...
class TestMulticollinearStability:
    def test_coefficients_stay_bounded(self) -> None:
        ...
        assert max_coef < 100.0 * feat_std_max, (
            f"Coefficient blow-up: max |coef|={max_coef:.2f} vs feature std max={feat_std_max:.2f}"
        )
    ...
    def test_test_mae_within_5x_train_mae(self) -> None:
        ...
        assert test_mae < 5.0 * train_mae + 1.0, (
            f"Out-of-distribution blow-up: train MAE={train_mae:.4f}, test MAE={test_mae:.4f} "
            f"(ratio={test_mae / max(train_mae, 1e-9):.1f}x; threshold=5x)"
        )
    ...
    def test_well_conditioned_matches_ols(self) -> None:
        ...
        assert max_abs_delta < 1e-3 * y_range, (
            f"Ridge(alpha=1e-3) diverged from OLS on well-conditioned features: "
            f"max |delta|={max_abs_delta:.6f} vs y-range={y_range:.4f}"
        )
