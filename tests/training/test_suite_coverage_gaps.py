...
def test_load_raises_on_critical_schema_change(tmp_path):
    ...
    with pytest.raises(
        ValueError,
        match=r"(load-bearing|Model input-schema mismatch|critical missing|" r"Unexpected input dimension|missing.*column|cat_0)",
    ):
...
def test_constant_and_all_null_cols_dropped_by_default(tmp_path):
    ...
    assert "num_const" not in trained_cols, f"constant column survived: {trained_cols}"
    assert "num_null" not in trained_cols, f"all-null column survived: {trained_cols}"
...
def test_two_runs_same_seed_produce_identical_schema(tmp_path):
    ...
    assert (meta_a.get("columns") or []) == (
        meta_b.get("columns") or []
    ...
    if schemas_a and schemas_b:
        ...
        for k in keys:
            assert schemas_a[k].get("schema_hash") == schemas_b[k].get(
                "schema_hash"
...
def test_outlier_detector_filters_training_rows(tmp_path):
    ...
    assert size_after is not None and size_after < n, f"train_size_after_od unexpected: {size_after} vs n={n}"
...
def test_rfecv_pipeline_runs_for_each_model_family(tmp_path, caplog):
    ...
    assert any(
        s in log_text.lower() for s in ("rfecv", "feature selection", "selected features")
...
def test_predict_models_suite_loads_and_predicts(tmp_path):
    ...
    assert preds_dict or probs_dict, f"predict returned no model outputs: keys={list(result)}"
...
def test_metadata_carries_load_bearing_fields(tmp_path):
    ...
    for k in ("schema_hash", "input_schema", "mlframe_model", "weight_name"):
        assert k in entry, f"model_schemas entry missing {k!r}; entry keys={list(entry)}"
...
def test_continue_on_model_failure_skips_crashed_model(tmp_path, monkeypatch):
    ...
    assert cb_failures, f"CB crash not recorded in failed_models: {failed}"
...
def test_predictions_finite_for_regression(tmp_path):
    ...
    for name, arr in preds.items():
        ...
        assert np.isfinite(arr_np).all(), f"non-finite regression predictions from {name}"
...
def test_preprocessing_extensions_polynomial_features_run(tmp_path):
    ...
    assert len(cols) > 3, f"polynomial_degree=2 did not expand feature count: cols={cols}"
...
def test_recurrent_lstm_smoke(tmp_path):
    ...
    seq_len = 8
    n_features = 3
    sequences = [rng.standard_normal((seq_len, n_features)).astype("float32") for _ in range(n_seqs)]
...
def test_continue_on_failure_with_ensembles(tmp_path, monkeypatch, caplog):
    ...
    assert any(f.get("model") == "cb" for f in failed), f"CB crash not recorded in failed_models: {failed}"
