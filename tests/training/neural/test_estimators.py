"""
Tests for sklearn-compatible estimator classes in mlframe.training.neural

Run tests:
    pytest tests/training/neural/test_estimators.py -v
    pytest tests/training/neural/test_estimators.py --cov=mlframe.training.neural --cov-report=html
"""
...
def classification_data():
    ...
    X, y = make_classification(n_samples=200, n_features=10, n_informative=8, n_redundant=2, n_classes=3, random_state=42)
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)
    return {"X_train": X_train.astype(np.float32), "y_train": y_train.astype(np.int64), "X_test": X_test.astype(np.float32), "y_test": y_test.astype(np.int64)}


@pytest.fixture
def regression_data():
    """Generate regression dataset."""
    X, y = make_regression(n_samples=200, n_features=10, n_informative=8, noise=10.0, random_state=42)
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)
    return {
...
class TestPytorchLightningClassifier:
    ...
    def test_fit_with_validation(self, estimator_params_classifier, classification_data):
        ...
        clf.fit(classification_data["X_train"], classification_data["y_train"], eval_set=eval_set)
    ...
    def test_partial_fit(self, estimator_params_classifier, classification_data):
        ...
        # First partial fit
        clf.partial_fit(classification_data["X_train"][:50], classification_data["y_train"][:50], classes=np.array([0, 1, 2]))

        assert hasattr(clf, "model")
        assert hasattr(clf, "classes_")

        # Second partial fit (should reuse model)
        clf.partial_fit(classification_data["X_train"][50:100], classification_data["y_train"][50:100])
...
class TestPytorchLightningRegressor:
    ...
    def test_fit_with_validation(self, estimator_params_regressor, regression_data):
        ...
        reg.fit(regression_data["X_train"], regression_data["y_train"], eval_set=eval_set)
    ...
    def test_partial_fit(self, estimator_params_regressor, regression_data):
        ...
        # First partial fit
        reg.partial_fit(regression_data["X_train"][:50], regression_data["y_train"][:50])

        assert hasattr(reg, "model")

        # Second partial fit
        reg.partial_fit(regression_data["X_train"][50:100], regression_data["y_train"][50:100])
...
class TestSampleWeightSupport:
    ...
    def test_classifier_with_sample_weight(self, estimator_params_classifier, classification_data):
        ...
        clf.fit(classification_data["X_train"], classification_data["y_train"], sample_weight=sample_weight)
    ...
    def test_regressor_with_sample_weight(self, estimator_params_regressor, regression_data):
        ...
        reg.fit(regression_data["X_train"], regression_data["y_train"], sample_weight=sample_weight)
    ...
    def test_classifier_with_validation_and_sample_weight(self, estimator_params_classifier, classification_data):
        ...
        eval_set = (classification_data["X_test"], classification_data["y_test"])
        clf.fit(classification_data["X_train"], classification_data["y_train"], sample_weight=train_weights, eval_set=eval_set, eval_sample_weight=val_weights)
    ...
    def test_uniform_weights_produces_valid_predictions(self, estimator_params_classifier, classification_data):
        ...
        clf.fit(classification_data["X_train"], classification_data["y_train"], sample_weight=uniform_weights)
...
class TestNetworkResetAndClone:
    ...
    def test_sklearn_clone_after_fit_gives_unfitted_model(self, estimator_params_classifier):
        ...
        assert not hasattr(cloned, "network") or cloned.network is None, "Cloned model should be unfitted"
