...
def test_use_residual_false_default_no_residual_blocks():
    ...
    net = generate_mlp(
        num_features=8, num_classes=1, nlayers=2,
        first_layer_num_neurons=16, dropout_prob=0.0,
        use_batchnorm=False, verbose=0,
    )
...
def test_use_residual_true_inserts_blocks_constant_width():
    ...
    blocks."""
    net = generate_mlp(
        num_features=16, num_classes=1, nlayers=3,
        first_layer_num_neurons=16, dropout_prob=0.0,
        use_batchnorm=True, use_residual=True, verbose=0,
    )
...
def test_use_residual_with_spectral_norm_warns_but_works(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="mlframe.training.neural.flat"):
        net = generate_mlp(
            ...
            use_residual=True, spectral_norm=True, verbose=0,
        )
    msgs = [r.getMessage() for r in caplog.records if r.levelno == logging.WARNING]
    assert any("use_residual" in m and "spectral" in m for m in msgs), (
        f"expected use_residual+spectral_norm warning; got {msgs}"
    )
