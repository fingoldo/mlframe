...
from mlframe.training.neural._muon_optimizer import (
    Muon, MuonAdamWHybrid, _zeropower_via_newtonschulz5,
)
...
def test_newton_schulz_returns_approximate_orthogonal():
    ...
    assert torch.allclose(UUt, eye, atol=0.5), (
        f"U U^T should be near-identity; max off-diag = "
        f"{(UUt - eye).abs().max().item():.4f}"
    )
...
def test_muon_step_updates_2d_params():
    ...
    assert loss_after.item() < loss_before.item(), (
        f"Muon failed to decrease loss: before={loss_before.item():.4f}, "
