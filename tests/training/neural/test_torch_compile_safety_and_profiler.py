...
def test_compile_skipped_on_lstm_with_warning(caplog):
    ...
    assert not hasattr(module.network, "_orig_mod"), "torch.compile should have been skipped for LSTM-containing network"
    msgs = [r.getMessage() for r in caplog.records if r.levelno == logging.WARNING]
    assert any("LSTM" in m or "GRU" in m or "RNN" in m for m in msgs)
    assert any("graph-break" in m.lower() or "graph_break" in m.lower() or "intentionally" in m.lower() for m in msgs)
...
def test_predict_step_uses_inference_mode(monkeypatch):
    ...
    assert no_grad_calls["n"] == 0, f"predict_step must NOT enter torch.no_grad() (graph-break form); observed {no_grad_calls['n']} no_grad enters"
