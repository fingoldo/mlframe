"""
Multi-GPU (DDP) integration tests for mlframe.training.neural.
...
Run tests:
    ...
    pytest tests/training/neural/test_multigpu.py -v

    # Run with specific GPU count
    pytest tests/training/neural/test_multigpu.py -v --devices=2
"""
...
pytestmark = pytest.mark.skipif(torch.cuda.device_count() < 2, reason="Multi-GPU tests require at least 2 GPUs")
...
class TestMultiGPUBestEpochTracking:
    ...
    def test_best_epoch_tracking_regressor(self, multigpu_regression_data, multigpu_estimator_params_regressor):
        ...
        reg.fit(multigpu_regression_data["X_train"], multigpu_regression_data["y_train"], eval_set=eval_set)
...
class TestMultiGPUPartialFit:
    ...
    def test_partial_fit_classifier(self, multigpu_classification_data, multigpu_estimator_params_classifier):
        ...
        # Second partial fit
        clf.partial_fit(multigpu_classification_data["X_train"][batch_size:], multigpu_classification_data["y_train"][batch_size:])
...
class TestMultiGPUSampleWeights:
    ...
    def test_classifier_with_sample_weights(self, multigpu_classification_data, multigpu_estimator_params_classifier):
        ...
        eval_set = (multigpu_classification_data["X_val"], multigpu_classification_data["y_val"])
        clf.fit(multigpu_classification_data["X_train"], multigpu_classification_data["y_train"], sample_weight=sample_weight, eval_set=eval_set)
    ...
    def test_regressor_with_sample_weights(self, multigpu_regression_data, multigpu_estimator_params_regressor):
        ...
        eval_set = (multigpu_regression_data["X_val"], multigpu_regression_data["y_val"])
        reg.fit(multigpu_regression_data["X_train"], multigpu_regression_data["y_train"], sample_weight=sample_weight, eval_set=eval_set)
