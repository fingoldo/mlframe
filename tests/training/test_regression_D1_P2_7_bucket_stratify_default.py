...
class _DummySplitConfig:
    ...
    def model_dump(self, exclude=None):
        d = {
            ...
            "shuffle_val": self.shuffle_val, "shuffle_test": self.shuffle_test,
...
def test_d1_p2_7_regression_bucket_stratify_runs_on_heavy_tail_target(caplog):
    ...
    y = np.concatenate([rng.exponential(scale=1.0, size=int(n * 0.95)),
                        rng.exponential(scale=20.0, size=int(n * 0.05))])
    rng.shuffle(y)
    ...
    with caplog.at_level(logging.INFO, logger="mlframe.training.core._phase_helpers_fit_split"):
        _phase_train_val_test_split(
            df=df, target_by_type=target_by_type, timestamps=None,
            group_ids=None, group_ids_raw=None, artifacts=None, sequences=None,
            split_config=_DummySplitConfig(), behavior_config=type("B", (), {"fairness_features": None})(),
            metadata=metadata, data_dir=None, models_dir=None,
            target_name="y", model_name="m", df_size_mb=1.0, verbose=False,
        )
...
def test_d1_p2_7_regression_bucket_stratify_disabled_when_flag_false(caplog):
    ...
    with caplog.at_level(logging.INFO, logger="mlframe.training.core._phase_helpers_fit_split"):
        _phase_train_val_test_split(
            ...
            metadata={}, data_dir=None, models_dir=None,
            target_name="y", model_name="m", df_size_mb=1.0, verbose=False,
        )
    info_msgs = [r.message for r in caplog.records if r.levelno >= logging.INFO]
    assert not any("Bucket-stratify" in m for m in info_msgs), (
        f"Did not expect bucket-stratify when flag=False; got: {info_msgs}"
    )
...
def test_d1_p2_7_bucket_distribution_balanced_in_val(caplog):
    ...
    y = np.concatenate([rng.exponential(scale=1.0, size=int(n * 0.9)),
                        rng.exponential(scale=30.0, size=int(n * 0.1))])
    rng.shuffle(y)
    df = pd.DataFrame({"x1": rng.standard_normal(n)})
    target_by_type = {_DummyTargetTypeRegr(): {"y": y}}
