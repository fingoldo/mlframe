...
Pre-fix a broad ``except Exception`` swallowed ANY error from the batched-ICE kernel call (now routed through
``_ice_kernel_dispatch``, which picks the serial or parallel ``_batch_per_class_ice_kernel*`` variant by size)
and silently fell back to the slow per-class loop at DEBUG, hiding genuine numeric/logic bugs. Post-fix only
(ValueError, TypeError) fall back; any other exception type propagates.
"""
...
def test_unexpected_error_type_propagates(monkeypatch):
    ...
    monkeypatch.setattr(ice, "_ice_kernel_dispatch", _boom)
...
def test_value_error_still_falls_back(monkeypatch):
    ...
    monkeypatch.setattr(ice, "_ice_kernel_dispatch", _value_err)
