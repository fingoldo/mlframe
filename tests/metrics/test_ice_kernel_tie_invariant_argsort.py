...
_KW = dict(nbins=10, use_weights=True, mae_weight=3.0, std_weight=2.0, brier_loss_weight=0.8,
...
def test_ice_kernel_invariant_to_within_tie_order(round_dec, n, k):
    ...
    assert np.array_equal(ice_q, ice_m), (
        f"ICE differs by within-tie argsort order: quicksort={ice_q.tolist()} mergesort={ice_m.tolist()}")
