...
def test_unit_weights_equal_the_unweighted_kernel(n):
    """With every weight 1 the weighted kernel is bit-identical to the unweighted one, ties included."""
...
def test_weighted_auc_terms_match_sklearn():
    """The weighted ROC and PR AUC terms equal sklearn's weighted scores."""
...
def test_equal_weights_leave_the_metric_unchanged_and_bad_weights_raise():
    """Scaling every weight equally changes nothing; a negative weight raises."""
...
def test_weights_follow_their_rows_through_the_subgroup_and_time_split_wrappers():
    """The time-split wrapper hands each split the weights of that split's own rows."""
