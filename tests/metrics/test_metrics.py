...
class TestICENaNGuards:
    ...
    def test_nan_brier_loss_scores_the_worst_value(self):
        """A NaN LOSS term means the loss could not be computed, and dropping it from the sum makes the model look
        BETTER than one whose Brier was merely bad -- the lower-is-better direction rewards the failure. ICE returns
        the finite worst value instead (finite because CatBoost rejects a non-finite custom metric value)."""
        val = integral_calibration_error_from_metrics(
            calibration_mae=0.01,
            calibration_std=0.01,
            calibration_coverage=1.0,
    ...
    def test_all_five_inputs_nan_scores_the_worst_value(self):
        """Nothing about this model could be measured, so it must not outrank one that was measured and was bad."""
        val = integral_calibration_error_from_metrics(
            calibration_mae=float("nan"),
            calibration_std=float("nan"),
            calibration_coverage=1.0,
