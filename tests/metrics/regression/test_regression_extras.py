...
def test_wmape_mixed_dtypes_bit_equivalent():
    ...
    for y_t, y_p, atol in [
        ...
        ref = np.abs(np.asarray(y_p, np.float64) - np.asarray(y_t, np.float64)).sum() / np.abs(np.asarray(y_t, np.float64)).sum()
        assert abs(v - ref) < atol, f"dtypes ({y_t.dtype}, {y_p.dtype}): fast_wmape={v} vs ref={ref}"
...
def test_iter607_pearson_corr_mixed_dtypes_bit_equivalent():
    ...
    for y_t, y_p, atol in [
        ...
        assert abs(v - ref) < atol, f"Pearson dtypes ({y_t.dtype}, {y_p.dtype}): got={v} ref={ref}"
...
def test_iter606_explained_variance_mixed_dtypes_bit_equivalent():
    ...
    for y_t, y_p, atol in [
        ...
        v = fast_explained_variance(y_t, y_p)
        ref = float(
        ...
        assert abs(v - ref) < atol, f"EV dtypes ({y_t.dtype}, {y_p.dtype}): got={v} ref={ref}"
...
def test_iter597_mbe_nse_mixed_dtypes_bit_equivalent():
    ...
    for y_t, y_p, atol in [
        ...
        assert abs(v - ref) < atol, f"MBE dtypes ({y_t.dtype}, {y_p.dtype}): got={v} ref={ref}"
    ...
    for y_t, y_p, atol in [
        ...
        assert abs(v - ref) < atol, f"NSE dtypes ({y_t.dtype}, {y_p.dtype}): got={v} ref={ref}"
...
def test_kendall_tau_mid_range_uses_scipy_and_matches_numba_kernel():
    ...
    for N in (600, 1500, 3000):
        ...
        assert numba_val == pytest.approx(scipy_val, abs=1e-10), f"numba O(N^2) and scipy O(N log N) disagree at N={N}: numba={numba_val} scipy={scipy_val}"
