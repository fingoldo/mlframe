...
class _AlwaysFailingModel:
    ...
    def predict(self, X):
        """Always raises ``RuntimeError(self._msg)``."""
    ...
    for i in range(n_models):
        if fail:
            ...
            class _Ok:
                ...
                def predict(self, X):
                    """Returns ``np.zeros(len(X), dtype=np.float64)``."""
...
def _build_df():
    """Builds seeded synthetic test data; returns ``pd.DataFrame({'x0': rng.standard_normal(50).astype(np.float64), 'x1': rng.standard_norm...``."""
...
def test_partial_success_does_not_raise() -> None:
    ...
    class _Ok:
        ...
        def predict(self, X):
            """Returns ``np.full(len(X), 1.5, dtype=np.float64)``."""
    ...
    assert any(isinstance(_v, np.ndarray) and np.allclose(_v, 1.5) for _v in result["predictions"].values())
