...
ALLOWED = {
    f"available_not_passed:{_D}_stacked.py::fit_stacked->{_D}_fit.py::fit:val_df": "pass 2 adds OOF columns to the train frame only; the val frame lacks them, so pass 2 cannot be scored on it",
    f"available_not_passed:{_D}_stacked.py::fit_stacked->{_D}_fit.py::fit:val_y": "goes with val_df above",
    f"available_not_passed:{_D}_stacked.py::fit_stacked_on_residual->{_D}_fit.py::fit:val_df": "pass 2 fits the residual target, so the raw-target val split does not apply",
    f"available_not_passed:{_D}_stacked.py::fit_stacked_on_residual->{_D}_fit.py::fit:val_y": "goes with val_df above",
    f"available_not_passed:{_T}feature_drift_report.py::_compute_drift_invariant->{_T}feature_drift_report.py::compute_categorical_drift_psi:feature_names": "the call sits in the feature_names-is-None branch; the restricted case passes the categorical subset above it",
...
def test_wrappers_forward_what_they_wrap():
    ...
    from py_ci_shared import kwarg_forwarding as kwf
