...
try:
    trained, _ = train_mlframe_models_suite(df=df, target_name='repro', model_name='repro', features_and_targets_extractor=fte, mlframe_models=['lgb', 'xgb'], hyperparams_config={'iterations': 3, 'xgb_kwargs': {'device': 'cpu', 'verbosity': 0}, 'lgb_kwargs': {'device_type': 'cpu', 'verbose': -1}}, preprocessing_config=PreprocessingConfig(drop_columns=[]), use_ordinary_models=True, use_mlframe_ensembles=False, verbose=0, output_config=OutputConfig(data_dir=_REPRO_OUTDIR, models_dir='models'), feature_selection_config=FeatureSelectionConfig(use_mrmr_fs=True, mrmr_kwargs={'verbose': 0, 'max_runtime_mins': 1, 'n_workers': 1, 'quantization_nbins': 5, 'use_simple_mode': True, 'min_nonzero_confidence': 0.9, 'max_consec_unconfirmed': 3, 'full_npermutations': 3}))
    print("PASS")
except Exception as e:
