...
def _make_cat_series(n=200, n_unique=5, seed=42):
    ...
    return pd.Series(rng.choice([f"cat_{i}" for i in range(n_unique)],
                                   size=n), name="cat")
...
def test_biz_val_categorical_compute_countaggs_returns_fixed_length_vector():
    ...
    assert len(out_short) == len(out_long), (
        ...
        f"short={len(out_short)}, long={len(out_long)}"
    )
    # Names length must match output length (contract for downstream
    # column-naming).
    assert len(names) == len(out_short), (
        f"get_countaggs_names() must match compute_countaggs output "
        f"length; got names={len(names)}, output={len(out_short)}"
    )
...
def test_biz_val_categorical_compute_countaggs_handles_cardinality(n_unique):
    ...
    assert len(out) == len(names), (
        f"cardinality={n_unique}: output len {len(out)} != names len {len(names)}"
    )
...
def test_biz_val_categorical_compute_countaggs_top_n_changes_output_shape():
    ...
    assert len(names_n3) > len(names_n1), (
        f"counts_top_n=3 ({len(names_n3)}) must produce more names "
        f"than =1 ({len(names_n1)})"
    )
...
def test_biz_val_categorical_get_countaggs_names_no_duplicates():
    ...
    assert len(names) == len(set(names)), (
        f"duplicate names in get_countaggs_names(); "
        f"names={names}, unique count={len(set(names))}"
    )
