...
def _make_dated_df(n=200, seed=42):
    ...
    return pd.DataFrame({"timestamp": [base + pd.Timedelta(days=int(d))
                                          for d in days]})
...
def test_biz_val_basic_create_date_features_extracts_components():
    ...
    assert out.shape[1] > df.shape[1], (
        f"create_date_features must add columns; "
        f"input cols={list(df.columns)}, output cols={list(out.columns)}"
    )
    # Expanded columns must include something timestamp-derived.
    new_cols = [c for c in out.columns if c != "timestamp"]
    assert any("timestamp" in c.lower() or
                  any(k in c.lower() for k in ("year", "month", "day",
                                                 "hour", "week", "quarter"))
                  for c in new_cols), (
...
def test_biz_val_basic_create_date_features_delete_original_default():
    ...
    df = _make_dated_df(n=100)
    out = create_date_features(df, cols=["timestamp"],
                                  delete_original_cols=True)
    assert "timestamp" not in out.columns, (
        f"delete_original_cols=True must drop 'timestamp'; "
...
def test_biz_val_basic_create_date_features_keep_original_when_disabled():
    ...
    assert "timestamp" in out.columns, (
        f"delete_original_cols=False must keep 'timestamp'; "
        f"got {list(out.columns)}"
    )
...
def test_biz_val_basic_create_date_features_row_count_preserved():
    ...
    assert len(out) == len(df), (
        f"row count must be preserved; got len(df)={len(df)}, "
        f"len(out)={len(out)}"
    )
