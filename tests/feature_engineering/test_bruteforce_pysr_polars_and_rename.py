...
def test_bruteforce_polars_side_fillna_imputes_with_median(_stub_pysr, monkeypatch):
    ...
    assert abs(float(f0[4]) - expected_median) < 1e-9, f"A2#6 imputation: f0[4] should be median ({expected_median}), got {f0[4]}"
...
def test_run_pysr_fe_does_not_rename_caller_frame(_stub_pysr):
    ...
    assert list(original.columns) == cols_before, f"A2#11: caller's original.columns changed: before={cols_before} after={list(original.columns)}"
    assert list(view.columns) == view_cols_before, f"A2#11: caller's view.columns changed: before={view_cols_before} after={list(view.columns)}"
...
def test_run_pysr_fe_no_view_aliasing_with_polars(_stub_pysr):
    ...
    assert list(original.columns) == cols_before, f"A2#11 (polars branch): caller's polars columns changed: before={cols_before} after={list(original.columns)}"
...
def test_pysr_polars_native_support_documented():
    ...
    manifest_path = os.path.join(
        ...
        "critique_2026_05_24",
        "manifests",
        "DONE_w16b-bruteforce-pysr-polars.json",
    )
    ...
    if not os.path.exists(manifest_path):
        ...
        pytest.skip(f"Wave 16B manifest not present on this checkout (audit/**/manifests/DONE_*.json is gitignored): {manifest_path}")
    ...
    assert isinstance(note, str) and len(note) >= 50, "pysr_finding_note must be substantive (>=50 chars), not a stub"
