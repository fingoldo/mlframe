...
def test_biz_val_numerical_numaggs_returns_fixed_length_vector():
    ...
    assert len(out_short) == len(out_long), f"output dim must be invariant to input length; got short={len(out_short)}, long={len(out_long)}"
...
def test_biz_val_numerical_numaggs_finite_on_finite_input():
    ...
    assert n_finite >= len(out) * 0.8, f"at least 80% of aggregates must be finite; got {n_finite}/{len(out)}"
...
def test_biz_val_numerical_numaggs_directional_only_smaller_output():
    ...
    assert len(out_dir) < len(out_full), f"directional_only=True must produce smaller output; got dir={len(out_dir)}, full={len(out_full)}"
...
def test_biz_val_numerical_numaggs_return_float32_dtype():
    ...
    assert out.dtype == np.float32, f"return_float32=True must produce float32; got {out.dtype}"
