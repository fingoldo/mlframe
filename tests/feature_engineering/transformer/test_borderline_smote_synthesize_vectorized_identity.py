"""Identity-pin for the borderline_smote SMOTE-synthesize vectorization.

``_smote_synthesize_from`` originally drew (src, nbr, alpha) per iteration AND wrote one
interpolated output row per iteration inside the Python loop. The optimization keeps the
per-iteration PCG64 draw order intact (so the synthetic cloud is unchanged) but hoists the
gather + convex-interpolation out into a single vectorized pass (~35% faster on the synthesize
step; see ``feature_engineering/_benchmarks/bench_borderline_smote_synthesize_vectorized.py``).

The draw order is load-bearing: ``np.random.default_rng`` interleaves the src-int / nbr-int /
alpha-float draws, so batching them would silently change WHICH neighbours interpolate and break
...
from mlframe.feature_engineering.transformer.borderline_smote import _smote_synthesize_from
...
def test_smote_synthesize_bit_identical_to_row_loop(seed, nrows, d, k, n_syn):
    ...
    got = _smote_synthesize_from(X, n_synthetic=n_syn, k_neighbors=k, seed=seed)
