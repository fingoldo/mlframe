...
from mlframe.feature_engineering.windowed_shape import (
    rolling_shannon_entropy_binned, rolling_total_variation, rolling_quantile_spread,
    rolling_zero_crossings, rolling_n_peaks)
...
v,g=mk(1_000_000)
s=time.perf_counter(); rolling_shannon_entropy_binned(v,g,window_K=K); print(f"entropy_binned @1M  {time.perf_counter()-s:.3f}s (single)")

# cheap vectorized fns @10M best-of-3
v,g=mk(10_000_000)
for name,f in [("total_variation",rolling_total_variation),("quantile_spread",rolling_quantile_spread),
               ("zero_crossings",rolling_zero_crossings),("n_peaks",rolling_n_peaks)]:
    f(v,g,window_K=K); best=min(( (lambda: (time.perf_counter(), f(v,g,window_K=K), time.perf_counter()))() for _ in range(3)),key=lambda x:x[2]-x[0]); print(f"{name:18s} @10M {best[2]-best[0]:.3f}s")
