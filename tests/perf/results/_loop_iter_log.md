...
## iter140 — GPU-DISPATCHER surface (GPU ENABLED): plugin_mi_classif batch crossover miscalibrated for this GPU — RESOLVED

**Surface/scale:** GPU/cupy DISPATCHER paths + kernel_tuning_cache crossover thresholds. Target this iter: `plugin_mi_classif_dispatch` / `plugin_mi_classif_batch_dispatch` (`feature_selection/filters/_hermite_fe_mi.py`) — the canonical KTC-integrated single+batch plug-in MI dispatcher (njit-vs-cuda), the same one called out in CLAUDE.md as having had the 2026-05-20 stale-threshold incident. NVIDIA RTX 500 Ada Generation Laptop GPU (~4GB VRAM). Different dispatcher than iter139's `dispatch_batch_pair_mi`.

**Method:** warm best-of-9 (median) A/B microbench of `_plugin_mi_classif_(batch_)njit` vs `_plugin_mi_classif_(batch_)cuda` across n in {5k,10k,20k,35k,50k,75k,100k,250k,500k,1M} for k in {1,5,20}, each freeing the cupy pool between points; plus a second-seed reproduction of the 10k-50k batch band. Bench committed: `feature_selection/_benchmarks/bench_mi_classif_crossover_iter140.py`.

**Measured crossover @ this GPU (ms, median best-of-9):** k=1: cuda wins from n~=75k (n=50k njit 3.25/cuda 4.15 CPU; n=75k njit 5.33/cuda 5.27 ~1.0x; n=100k 7.26/6.28; n=500k 46.8/4.4=10.7x; n=1M 99.6/9.9). k=5: cuda from n~=35-50k (n=10k njit 1.13/cuda 3.21 CPU; n=20k njit 2.39/cuda 3.22 CPU; n=50k njit 6.65/cuda 3.18=2.1x; n=100k 14.0/4.85=2.9x). k=20: cuda from n~=50k (n=20k njit 3.07/cuda 3.82 CPU; n=35k njit 5.82/cuda 6.94 CPU; n=50k njit 11.6/cuda 9.42=1.2x). Second-seed run confirmed njit wins through ~35k for all batch k.

**Miscalibration:** the hardcoded fallback (`_fallback_mi_backend`: 75k single / **10k batch**, calibrated on a GTX 1050 Ti cc 6.1, 2026-05-20) was correct for single-column (k=1) but routed the **batch 10k-50k band to GPU** where the CPU prange kernel is faster on this host (e.g. k=5 n=20k: njit 2.39ms vs cuda 3.22ms = ~35% loss; k=20 n=35k: 5.82 vs 6.94). No persisted `plugin_mi_classif_dispatch` region existed on this host (confirmed `get_regions(...) is None`), so the dispatcher was using the stale fallback. Same miscalibration class as iter139 / the 2026-05-20 plug-in-MI incident, opposite direction (batch fallback too LOW here).

**Fix (NOT a new hardcoded constant, per CLAUDE.md):** ran the already-registered sweep `ensure_mi_classif_dispatch_tuning(force=True)` to populate the per-host kernel_tuning_cache — 22 measured regions persisted (njit through n=20k for all k; cuda from n=50k). The dispatcher already consults `lookup_mi_classif_backend` (cache) before the hardcoded fallback, so the measured map now drives routing on this host. Verified live: batch k=5/k=20 at n=20k now route to njit (was cuda under the 10k fallback). No source constant changed — the cache JSON is the artifact.

**Identity proof:** cuda vs njit MI max_abs_diff 1.3e-15 across n in {5k,50k,200k} for single + batch (k=8) — FP reduction order, far below selection-altering (~1e-3). Both backends interchangeable; routing change only trades runtime.

**Regression test:** `tests/feature_selection/test_plugin_mi_classif_dispatch.py::TestPluginMIPerHostRegionOverridesFallback::test_persisted_region_njit_at_20k_batch_beats_fallback_cuda` — env-independent (mocks `_get_cache`), asserts a persisted per-host region routing batch n=20k to njit beats the hardcoded fallback's cuda; verified FAILS on the cache-ignored (pre-fix) path which returns cuda. Full `test_plugin_mi_classif_dispatch.py`: 55 passed + the new test (56), 1 transient GPU-contention biz_value timing flake that passes in isolation. The pre-existing `test_mi_dispatch_contract.py::...[mine]` neural-MINE failure (torch CUBLAS_STATUS_INVALID_VALUE under GPU contention) is unrelated to this dispatcher.

**Verdict: RESOLVED.** Second GPU-path validation in the loop surfaced a real crossover miscalibration on `plugin_mi_classif_batch_dispatch` (hardcoded 10k batch vs real ~35-50k); fixed via the kernel_tuning_cache sweep (not a hardcode), pinned with a region-override regression test. ~20-35% recovered on the batch 10k-35k band at this GPU; bit-identical. Single-column (k=1, 75k) fallback was already correctly calibrated — documented.

Streak: RESET to 0 (RESOLVED). **Cumulative loop wave: 106 RESOLVED, 36 REJECT across 140 iterations.**

## iter141 — GPU-DISPATCHER surface (GPU ENABLED): joint_hist_batched block_size miscalibrated at large joint_size for this GPU — RESOLVED

**Surface/scale:** GPU/cupy DISPATCHER paths + kernel_tuning_cache crossover thresholds. Target this iter: `joint_hist_batched` — the `lookup_joint_hist` KTC dispatcher (`feature_selection/_benchmarks/kernel_tuning_cache/dispatch.py`) selecting `{kernel_variant, block_size}` for the batched joint-histogram CUDA kernels consumed by `mi_direct_gpu_batched` (`feature_selection/filters/gpu.py`). NVIDIA RTX 500 Ada Generation Laptop GPU (cc 8.9, sm_count=16, 48KB shared/block). Different dispatcher than iter139 (`dispatch_batch_pair_mi`) and iter140 (`plugin_mi_classif_dispatch`).

**Method:** warm best-of-20 (median) microbench of `compute_joint_hist_batched_shared_cuda` (shared-atomic) vs `compute_joint_hist_batched_cuda` (global-atomic) across block_size {256,512,1024} for n in {10k,30k,50k,100k,500k,1M} and joint_size {25,100,400}; plus a paired 30-trial A/B of shared/256 vs shared/512 at large n. Bench committed: `feature_selection/_benchmarks/bench_joint_hist_block_size_iter141.py`.

**Measured crossover @ this GPU (ms, median):** `shared` variant wins EVERYWHERE — global-atomic is 1.5-4x slower at every (n, joint) (e.g. n=500k joint=25: shared/256 0.094 vs global/512 0.410 = 4.4x; n=1M joint=400: shared/512 0.045 vs global 0.225 = 5x). Within shared, block_size: at **joint_size=400 (20x20)** bs512 wins **30/30** paired trials with a real margin (n=500k: 0.145→0.115ms = 21%; n=1M: 0.248→0.227ms = 9%; n=100k: 0.042→0.040ms = 4%); at joint_size=25/100 bs256 wins or ties (≤2/30 for bs512). bs1024 consistently worse.

**Miscalibration:** the HW-aware fallback (`_FALLBACK_BY_CC[8] = {shared, bs=256}`, applied for all joint_size≤4096) had the **variant correct (shared) and small-joint block_size correct (256), but hardcoded bs=256 for large joint_size too** — leaving a stable 9-21% win at joint_size=400 on the table. No persisted `joint_hist_batched` region existed on this host (`get_regions(...) is None`), so the dispatcher used the partial-stale fallback. Narrower miscalibration than iter139/140 (which mis-routed the whole variant/backend); here only the large-joint block_size was off.

**Fix (NOT a new hardcoded constant, per CLAUDE.md):** ran the already-registered sweep `ensure_joint_hist_tuning(force=True)` to populate the per-host kernel_tuning_cache — 7 measured regions persisted (joint=400 → shared/512 at all n; large-n joint=100 → shared/512; small joint → shared/256; catch-all shared/512). `mi_direct_gpu_batched` already consults `lookup_joint_hist` before the fallback, so the measured map now drives routing on this host. Verified live: joint=400 now routes shared/**512** (was 256) at every n; small joint unchanged. No source constant changed — the cache JSON is the artifact. (Note: this host's KTC fingerprint is CPU-named `cpu_intel-core-ultra-7-155h_no-gpu` because GPUtil import fails; the regions still persist and reload consistently for this host.)

**Identity proof:** block_size only changes grid decomposition; the int32 atomic-add joint histogram is order-independent → bs256 and bs512 produce **bit-identical** `joint_counts_batch` (verified n∈{10k,500k,1M} × joint∈{25,400}, `np.array_equal` True, sums == n). Pure-perf, zero-numeric change.

**Regression test:** `tests/feature_selection/test_gpu_kernel_equivalence.py::test_joint_hist_shared_block_size_invariant` (3 params — bs256==bs512 bit-identical) + `::test_joint_hist_lookup_routes_shared_variant` (dispatch routes `shared` across the MI size range). Full file 11 passed (GPU/importorskip-guarded).

**Verdict: RESOLVED.** Third GPU-path validation in the loop surfaced a real (narrower) block_size miscalibration on `joint_hist_batched` (cc-8 fallback hardcoded bs256 for all joint sizes; bs512 is a stable 9-21% win at joint_size=400); fixed via the kernel_tuning_cache sweep (not a hardcode), pinned with bit-identity + routing regression tests. ~9-21% recovered on the joint_size=400 GPU joint-hist band at this GPU; bit-identical. The `shared` variant choice + small-joint bs256 were already correctly calibrated — documented.

Streak: RESET to 0 (RESOLVED). **Cumulative loop wave: 107 RESOLVED, 36 REJECT across 141 iterations.**

## iter142 — GPU-DISPATCHER surface (GPU ENABLED): polyeval_dispatch cuda crossover mis-routed large-n to the slower GPU path on this GPU — RESOLVED

**Surface/scale:** GPU/cupy DISPATCHER paths + kernel_tuning_cache crossover thresholds. Target this iter: `polyeval_dispatch` (`feature_selection/filters/hermite_fe/__init__.py`) — the njit / njit_par / `_polyeval_cuda` RawKernel router for the 4-basis (hermite/legendre/chebyshev/laguerre) Horner polynomial eval, whose `cuda_threshold` crossover comes from `_lookup_polyeval_thresholds` / kernel_tuning_cache. NVIDIA RTX 500 Ada Generation Laptop GPU (cc 8.9). Different dispatcher than iter139 (`dispatch_batch_pair_mi`), iter140 (`plugin_mi_classif_dispatch`), iter141 (`joint_hist_batched` block_size). The cuda branch had been explicitly DEFERRED in `_hermite_oracle.py` ("cupy broken on dev box, CUDA crossover unbenchable") — now benched with GPU enabled.

**Method:** warm best-of-15 (median) microbench of `_NJIT_FUNCS[hermite]` (single-thread Horner), `_NJIT_PAR_FUNCS[hermite]` (prange), and `_polyeval_cuda` (RawKernel incl. full H2D+D2H) across n in {1e4,3e4,1e5,5e5,1e6}, in its own subprocess. Bench committed: `feature_selection/_benchmarks/bench_polyeval_cuda_crossover_iter142.py`.

**Measured crossover @ this GPU (ms, median, hermite):** n=1e4 njit 0.024 / par 0.099 / cuda 0.335 (njit); n=3e4 0.066 / 0.048 / 0.567 (par); n=1e5 0.448 / 0.079 / 1.574 (par); n=5e5 7.269 / **1.033** / 6.853 (par); n=1e6 12.364 / **1.515** / 12.816 (par). **cuda NEVER beats njit_par at any swept n** — the degree-5 Horner is transfer-bound: the H2D+D2H round trip for a single column dwarfs the trivial compute on this laptop GPU. njit_par wins from n=3e4 up.

**Mis-route:** source default `_CUDA_THRESHOLD=500_000` (derived on an old GTX 1050 Ti, then DEFERRED). With GPU enabled the dispatcher routed n>=500k to `_polyeval_cuda` — which is **~6.6x slower @500k (6.85 vs 1.03ms) and ~8.5x slower @1M (12.8 vs 1.5ms)** than the njit_par it should have used. Confirmed live: `_lookup_polyeval_thresholds('hermite', 1e6)` returned cuda_thr=500000 → picks_cuda=True pre-fix. Worse, the KTC sweep `_run_sweep_polyeval` had a latent bug: when cuda never wins it LEFT `cuda_threshold = 500_000` (the source fallback), so even running the sweep would have re-persisted the mis-route.

**Fix (NOT a new hardcoded constant, per CLAUDE.md):** (1) fixed `_run_sweep_polyeval` (`kernel_tuning_cache/_auto_tune_sweeps_a.py`) so the "cuda never wins" fallback is a sentinel `max(swept_n)*100` (above any realistic prod n) instead of the 500k mis-route default; (2) ran `ensure_polyeval_tuning(force=True)` to populate the per-host kernel_tuning_cache — 4 measured regions persisted (all bases: cuda_threshold=100_000_000, par_threshold 20k–50k). `polyeval_dispatch` already consults `_lookup_polyeval_thresholds` first, so the measured map now drives routing. Verified live post-fix: n∈{5e5,1e6} now route njit_par (picks_cuda=False); par_threshold also recalibrated 50k→20k (njit_par wins from 30k). No source threshold constant changed — the cache JSON + the corrected sweep-fallback are the artifacts.

**Identity proof:** all three backends evaluate the identical Horner recurrence per element with no cross-element reduction; bench measured njit_par vs cuda @1e6 `max_abs_diff = 0.000e+00` (bit-identical). Pure routing recalibration, zero numeric change.

**Regression test:** `tests/feature_selection/test_polyeval_cuda_threshold_sentinel.py::test_polyeval_sweep_disables_cuda_when_it_never_wins` — patches the backends so cuda always loses, asserts the sweep emits `cuda_threshold > 1_000_000` and `!= 500_000`. FAILS on pre-fix code (which left exactly 500_000). Hermite + KTC suites green: 122 passed (`test_hermite_fe_coverage` + `test_kernel_tuning_cross_hw` + `test_gen_default_tuning`), new test 1 passed (GPU/importorskip-guarded).

**Verdict: RESOLVED.** Fourth GPU-path validation surfaced a real large-n mis-route on `polyeval_dispatch`: the long-DEFERRED cuda crossover (500k, old GPU) sent prod-shape n>=500k to a cuda path that is ~6.6–8.5x SLOWER than njit_par on this transfer-bound RTX 500 Ada, plus a latent sweep bug that would re-persist the mis-route. Fixed via the kernel_tuning_cache sweep + sentinel-fallback correction (not a hardcode), pinned with a fail-on-regression test. ~6.6–8.5x recovered on the n>=500k polyeval band (by NOT using the GPU here); bit-identical. Note: this is a "GPU correctly loses" recalibration — the win is eliminating a wrong GPU route, consistent with the surface thinning (iters 139→142: 2.8–8.5x → 20–35% → 9–21% → here a route-elimination, not a kernel speedup).
