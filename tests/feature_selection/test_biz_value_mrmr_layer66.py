...
def _make_mrmr(**overrides):
    """Cheap-and-deterministic MRMR ctor (mirrors Layer 65 helper)."""
...
class TestMonotoneTransformInvariance:
    ...
    def test_invariant_under_exp(self, seed):
        ...
        assert mi_raw == pytest.approx(mi_exp, abs=1e-12), (
            f"seed={seed}: copula_mi not invariant under exp; "
            f"mi(x,y)={mi_raw}, mi(exp(x),y)={mi_exp}"
        )
    ...
    def test_invariant_under_log_of_positive(self, seed):
        ...
        assert mi_raw == pytest.approx(mi_log, abs=1e-12), (
            f"seed={seed}: copula_mi not invariant under log; "
            f"mi(x,y)={mi_raw}, mi(log(x),y)={mi_log}"
        )
    ...
    def test_invariant_under_target_monotone(self, seed):
        ...
        assert mi_raw == pytest.approx(mi_tanh, abs=1e-12), (
            f"seed={seed}: copula_mi not invariant under tanh(y); "
            f"mi(x,y)={mi_raw}, mi(x,tanh(y))={mi_tanh}"
        )
...
class TestHeavyTailInvariance:
        ...
        for s in seeds:
            ...
            for tag, mi in [("log", mi_log), ("z", mi_z), ("cube", mi_cube)]:
                assert mi == pytest.approx(mi_raw, abs=1e-12), (
                    f"seed={s}: copula MI not invariant under monotone "
                    f"{tag} on heavy-tail x; mi(x,y)={mi_raw}, "
                    f"mi({tag}(x),y)={mi}"
                )
    ...
    def test_copula_mi_above_zero_on_heavy_tail_threshold(self):
        ...
        for s in (1, 7, 13, 42, 101, 202, 303, 404):
            ...
            cm = float(copula_mi(
                X["x1"].to_numpy(), y.to_numpy().astype(np.int64), n_bins=20,
            ))
            assert cm > 0.3, (
                f"seed={s}: copula MI on heavy-tail x1 / log-threshold y "
                f"= {cm:.4f}; expected >= 0.3 nats (genuine dependence "
                f"floor)."
            )
...
class TestAucLiftOnHeavyTail:
    ...
    def test_copula_augmented_logreg_auc_beats_raw(self):
        ...
        assert wins >= len(aucs_raw) - 1, (
            f"copula-augmented AUC matched-or-beat raw on only "
            f"{wins}/{len(aucs_raw)} seeds; per-seed floor too soft."
        )
...
class TestDefaultDisabledByteIdentical:
    ...
    def test_default_off_no_copula_columns(self, seed):
        ...
        assert added == [], (
            f"seed={seed}: default fe_hybrid_orth_copula_enable=False "
