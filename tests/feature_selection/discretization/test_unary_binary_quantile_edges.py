...
def test_identical_row_same_value_under_drift():
    ...
    assert out_train[0] == out_test[0], f"identical (0.5, 0.5) row produced different values: train={out_train[0]}, test={out_test[0]}"
    assert np.isclose(float(out_train[0]), 0.25, atol=1e-9), f"mul(0.5, 0.5) should be 0.25 continuous; got {out_train[0]}"
