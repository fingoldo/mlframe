...
def _make_fake_self():
    ...
    # unwrapped-callable via the descriptor protocol on CLASS attribute access -- assigned
    # directly as an INSTANCE attribute (as here, on a bare SimpleNamespace) it stays a raw,
    # non-callable staticmethod object on Python <3.10 (this repo's floor is py3.9). No
    # descriptor protocol is needed for a plain function reference held on an instance.
    self.filter_data = _io_plot.filter_data
    self.check_if_which_features_is_correct = _io_plot.check_if_which_features_is_correct
    self.box_plot = types.MethodType(_io_plot.box_plot, self)
    self.create_list = _io_plot.create_list
    self.to_dictionary = _io_plot.to_dictionary
