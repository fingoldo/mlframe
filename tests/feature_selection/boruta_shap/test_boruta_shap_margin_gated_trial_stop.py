...
def _make_selector(**kw):
    ...
    base = dict(
        ...
        importance_measure="gini", classification=True, n_trials=_N_TRIALS_CAP, percentile=95,
...
def test_margin_gated_stop_is_decision_equivalent_and_saves_trials():
    ...
    assert set(on.accepted) == set(off.accepted), (
        f"margin-gated accepted set diverged: on={sorted(on.accepted)} off={sorted(off.accepted)}"
    )
    assert _jac(on.rejected, off.rejected) >= 0.84, (
        f"rejected Jaccard {_jac(on.rejected, off.rejected):.3f} below recipe bar 0.84"
    )
...
def test_shipped_rule_is_margin_gated_not_naive():
    ...
    assert _should_stop_tentative_tail(
        accepted_history, hits, tentative_idx, iteration=iteration, n_tests=n_tests,
        pvalue=pvalue, patience=patience, margin=margin,
    ) is False, "shipped rule must NOT stop while a tentative feature is near a boundary"
    ...
    assert _should_stop_tentative_tail(
        accepted_history, hits_stuck, tentative_idx, iteration=iteration, n_tests=n_tests,
        pvalue=pvalue, patience=patience, margin=margin,
    ) is True
