...
def _hierarchical_two_super_three_sub(n: int = 2000, seed: int = 0):
    ...
    y = pd.Series(
        (meta_1 + meta_2 + 0.3 * rng.standard_normal(n) > 0).astype(int)
    )
    return X, y
...
def _flat_independent_data(n: int = 1500, seed: int = 1):
    ...
    rng = np.random.default_rng(int(seed))
    X = pd.DataFrame({
        f"f_{i}": rng.standard_normal(n) for i in range(12)
    })
    y = pd.Series(
        (X["f_0"] + 0.3 * rng.standard_normal(n) > 0).astype(int)
    )
    return X, y
...
class TestLayer48_BuilderUnit:
    ...
    def test_single_anchor_returns_empty(self):
        ...
        h = build_cluster_hierarchy(
            {"cluster_anchors_names": {"s1_a1": ["s1_a2"]}}, X,
        )
        assert h == {}
    ...
    def test_pickle_returns_value_pickleable(self):
        ...
        h = build_cluster_hierarchy(
            {"cluster_anchors_names": {"s1_a1": [], "s2_a1": []}}, X,
        )
...
class TestLayer48_TwoLevelHierarchy:
    ...
    def test_C1_super_clusters_detected_on_synthetic(self):
        ...
        assert 1 in h, (
            f"Expected at least one level-1 super-cluster on hierarchical "
            f"data; got hierarchy={h}"
        )
        level1 = h[1]
        ...
        for super_anchor, sub_anchors in level1.items():
            ...
            if len(group & s2_anchors) >= 2:
                found_s2_super = True
        assert found_s1_super or found_s2_super, (
            f"Expected a super-cluster grouping >= 2 S1 anchors OR >= 2 "
            f"S2 anchors; got level1={level1}"
        )
    ...
    def test_C2_flat_data_no_super_structure(self):
        ...
        assert h == {}, (
            f"Independent data must yield empty hierarchy; got {h}"
        )
