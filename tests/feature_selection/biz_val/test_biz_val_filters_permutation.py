...
def _warmup_njit():
    ...
    parallel_mi_besag_clifford(cx, fx, cy, fy, 100, mi, np.uint64(0), dtype=np.int32)
...
def test_biz_val_permutation_besag_clifford_stops_before_full_budget():
    ...
    cx, fx, cy, fy, mi = _classes_and_mi(*_make_strong_signal(n=5000, seed=42))
    N_PERMS = 1000
    nf, nt = parallel_mi_besag_clifford(cx, fx, cy, fy, N_PERMS, mi, np.uint64(0), dtype=np.int32)
    assert nt < int(0.7 * N_PERMS), f"BC must early-stop with nchecked < {int(0.7 * N_PERMS)}; got nchecked={nt}"
    # AND it must agree with the full-budget verdict (no failures
    # for strong signal -- both should report nfailed=0).
    assert nf == 0, f"On strong-signal target every permutation should fail to recover the MI; BC reports nfailed={nf}"
...
def test_biz_val_permutation_inner_prange_reproducible_across_n_workers():
    ...
    a_failed, a_checked = parallel_mi_prange(cx, fx, cy, fy, N_PERMS, mi, np.uint64(42), dtype=np.int32)
    b_failed, b_checked = parallel_mi_prange(cx, fx, cy, fy, N_PERMS, mi, np.uint64(42), dtype=np.int32)
    assert (a_failed, a_checked) == (b_failed, b_checked), (
        f"prange must be reproducible across runs with same seed; got run1=({a_failed}, {a_checked}) vs run2=({b_failed}, {b_checked})"
    )
...
def test_biz_val_permutation_zero_permutations_returns_zero_no_crash():
    ...
    assert nf == 0 and nt == 0, f"npermutations=0 must return (0, 0); got ({nf}, {nt})"
...
def test_biz_val_null_kernels_bit_identical_to_legacy(kernel_pair):
    ...
    from mlframe.feature_selection.filters.permutation import (
        parallel_mi,
        parallel_mi_with_null,
        parallel_mi_prange,
        parallel_mi_prange_with_null,
        parallel_mi_besag_clifford,
        parallel_mi_besag_clifford_with_null,
    )
...
def test_biz_val_mi_direct_return_null_mean_backward_compatible():
    ...
    factors = np.column_stack([x_bin, y]).astype(np.int32)
    factors_nbins = np.array([10, 2], dtype=np.int64)
    legacy = mi_direct(factors, x=(0,), y=(1,), factors_nbins=factors_nbins, npermutations=16, base_seed=3, prefer_gpu=False)
    assert isinstance(legacy, tuple) and len(legacy) == 2
    quad = mi_direct(factors, x=(0,), y=(1,), factors_nbins=factors_nbins, npermutations=16, base_seed=3, prefer_gpu=False, return_null_mean=True)
    assert len(quad) == 4
...
def test_biz_val_null_mean_demotes_noise_keeps_signal():
    ...
    obs_s, _, null_s, _ = mi_direct(sig, x=(0,), y=(1,), factors_nbins=fnb, npermutations=16, base_seed=1, prefer_gpu=False, return_null_mean=True)

    xn, yn = _make_no_signal(n=2000, seed=5)
    noise = np.column_stack([xn, yn]).astype(np.int32)
    obs_n, _, null_n, _ = mi_direct(noise, x=(0,), y=(1,), factors_nbins=fnb, npermutations=16, base_seed=1, prefer_gpu=False, return_null_mean=True)
...
def test_biz_val_significance_gate_keeps_weak_signal_demotes_noise():
    ...
    obs_w, _, null_w, p_w = mi_direct(weak, x=(0,), y=(1,), factors_nbins=fnb, npermutations=64, base_seed=2, prefer_gpu=False, return_null_mean=True)

    xn, yn = _make_no_signal(n=n, seed=5)
    noise = np.column_stack([xn, yn]).astype(np.int32)
    obs_n, _, null_n, p_n = mi_direct(noise, x=(0,), y=(1,), factors_nbins=fnb, npermutations=64, base_seed=2, prefer_gpu=False, return_null_mean=True)
    ...
    assert rel_w > 5.0 * max(rel_n, 1e-9), f"significance-gated weak signal relevance ({rel_w:.5f}) must dominate gated noise ({rel_n:.5f})"
...
def test_biz_val_null_signif_alpha_default_is_005_and_decision_stable_across_band():
    ...
    sig = np.column_stack([xs, ys]).astype(np.int32)
    _, _, _, p_sig = mi_direct(sig, x=(0,), y=(1,), factors_nbins=fnb, npermutations=2, base_seed=1, prefer_gpu=False, return_null_mean=True)
    ...
    noise = np.column_stack([xn, yn]).astype(np.int32)
    _, _, _, p_noise = mi_direct(noise, x=(0,), y=(1,), factors_nbins=fnb_noise, npermutations=2, base_seed=1, prefer_gpu=False, return_null_mean=True)
...
def test_biz_val_addone_pvalue_caps_confidence_below_one(monkeypatch):
    ...
    _, conf_plain = pm.mi_direct(data, x=(0,), y=(1,), factors_nbins=fnb, npermutations=nperm, parallelism="inner", base_seed=1, prefer_gpu=False)
    monkeypatch.setenv("MLFRAME_MRMR_ADDONE_PVALUE", "1")
    _, conf_add = pm.mi_direct(data, x=(0,), y=(1,), factors_nbins=fnb, npermutations=nperm, parallelism="inner", base_seed=1, prefer_gpu=False)
...
def test_regression_addone_default_is_on_and_does_not_change_selection(monkeypatch):
    ...
    mi_default, _ = pm.mi_direct(data, x=(0,), y=(1,), factors_nbins=fnb, npermutations=64, parallelism="inner", base_seed=2, prefer_gpu=False)
    monkeypatch.setenv("MLFRAME_MRMR_ADDONE_PVALUE", "0")
    mi_plain, _ = pm.mi_direct(data, x=(0,), y=(1,), factors_nbins=fnb, npermutations=64, parallelism="inner", base_seed=2, prefer_gpu=False)
