...
def _import_bootstrap_fe():
    """Lazily import and return the bootstrap-MI scoring/FE functions."""
...
def _import_point_estimate_fe():
    """Lazily import and return the point-estimate MI scoring/FE functions."""
...
@cache
def _bootstrap_enabled_fit(seed: int):
    """Cached ``(X, y, m)`` for the default bootstrap-enabled fit
    (n_boot=10, sample_fraction=0.8, degrees=(2,3), hermite, top_k=3)
    shared between test_enable_appends_at_least_one_he2 (all SEEDS) and
    test_pickle_roundtrip_preserves_bootstrap_recipes (fixed seed=42,
    a member of SEEDS). Nothing downstream mutates X/y/m in place.
    """
...
class TestStableSignalRetained:
    """A stable high-MI signal (x1__He2) keeps a high bootstrap uplift_lcb across seeds."""

    @pytest.mark.parametrize("seed", SEEDS)
    def test_signal_uplift_lcb_above_noise(self, seed):
        """x1__He2's uplift_lcb is materially positive and above every noise column's."""
    ...
    def test_signal_selected_into_winners(self, seed):
        """x1__He2 is appended by the bootstrap-selected top-K winners."""
...
class TestNoiseSuppressed:
    """Noise basis columns get wide CIs and are suppressed from the bootstrap top-K."""
    ...
    def test_noise_lcb_smaller_than_mean(self, seed):
        ...
        noise_rows = scores[scores["engineered_col"].str.startswith("noise_")]
        assert not noise_rows.empty, f"seed={seed}: expected noise basis columns in score table"
        # At least one noise basis column must have strictly positive
        # uplift std (genuine bootstrap variance, not numeric zero).
        assert (
            noise_rows["uplift_std"] > 1e-6
        ).any(), f"seed={seed}: all noise basis columns have zero uplift_std; bootstrap did not produce a non-degenerate CI."
...
class TestBootstrapVsPointEstimateDiffer:
    """On a borderline frame, the bootstrap support is a strict subset of the point-estimate support."""
...
class TestDefaultDisabledByteIdentical:
    """With ``fe_hybrid_orth_bootstrap_enable=False`` (the default), legacy behaviour is preserved."""

    @pytest.mark.parametrize("seed", SEEDS)
    def test_default_off_no_bootstrap_columns(self, seed):
        """Default-off leaves hybrid_orth_features_ empty."""
...
class TestEnableAppendsEngineered:
    """Turning fe_hybrid_orth_bootstrap_enable on appends at least one engineered column."""
    ...
    def test_enable_appends_at_least_one_he2(self, seed):
        ...
        assert any(
            c == "x1__He2" or c.startswith("x1__") for c in added
...
class TestCategoricalDoesNotSwallowBootstrap:
    ...
    def test_categorical_frame_still_appends_engineered(self, seed, caplog):
        """A categorical-bearing frame still appends engineered columns without the warn-and-continue band-aid."""
        import logging
        ...
        assert any(c == "x1__He2" or c.startswith("x1__") for c in added), f"seed={seed}: the He_2(x1) winner must survive categorical scoping; got {added}"
...
class TestPickleAndClone:
    """clone() preserves ctor params; pickle preserves appended columns + recipes."""

    def test_clone_preserves_bootstrap_params(self):
        """clone() round-trips the 3 bootstrap ctor params."""
        m = _make_mrmr(
            fe_hybrid_orth_bootstrap_enable=True,
    ...
    def test_pickle_roundtrip_preserves_bootstrap_recipes(self):
        ...
        _X, _y, m = _bootstrap_enabled_fit(42)
        blob = pickle.dumps(m)
        ...
        # rule differs.
        recipes_before = (
            {r.name: r for r in getattr(m, "_engineered_recipes_", {}).values() if getattr(r, "kind", None) == "orth_univariate"}
            if isinstance(getattr(m, "_engineered_recipes_", None), dict)
            else {r.name: r for r in (getattr(m, "_engineered_recipes_", []) or []) if getattr(r, "kind", None) == "orth_univariate"}
        )
        recipes_after = (
            {r.name: r for r in getattr(m2, "_engineered_recipes_", {}).values() if getattr(r, "kind", None) == "orth_univariate"}
            if isinstance(getattr(m2, "_engineered_recipes_", None), dict)
...
class TestRecipeReplay:
    """Recipe replay on held-out data matches the fit-time engineered values."""
...
class TestSortedGatherGate:
    ...
    def test_all_columns_distinct_gate(self):
        """_all_columns_distinct correctly flags distinct vs duplicated column sets."""
        from mlframe.feature_selection.filters._orthogonal_bootstrap_mi_fe import (
