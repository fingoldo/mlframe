...
def test_biz_value_selection_byte_identical_on_genuine_multi_signal():
    ...
    assert sel_on == sel_off, f"early-stop changed the selection on a genuine multi-signal fixture:\n  ON={sel_on}\n OFF={sel_off}"
    ...
    v = getattr(fs_on, "sufficient_summary_", None)
    assert v is not None and not v.reached, f"early-stop fired on a fixture with unrecovered signal: {v and v.reason}"
...
def test_cprofile_sufficient_summary_cost_is_negligible():
    ...
    MRMR(verbose=0, fe_max_steps=2, random_seed=1, skip_retraining_on_same_content=False).fit(*_f1_fixture(seed=1))
    ...
    assert total > 0
    frac = check_ct / total
    assert frac < 0.02, f"sufficient-summary check is too expensive: {check_ct:.4f}s / {total:.2f}s = {100 * frac:.2f}% of fit (expected < 2%)"
