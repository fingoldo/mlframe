...
def _build_all_noise(seed: int, n: int = 1500, p: int = 20):
    ...
    X = pd.DataFrame(
        {f"n{i:02d}": rng.standard_normal(n) for i in range(p)}
    )
...
class TestAllNoiseNoSpuriousFE:
    ...
    def test_hybrid_orth_emits_no_spurious_columns(self, seed):
        ...
        assert len(m.hybrid_orth_features_) <= 3, (
            f"seed={seed}: hybrid_orth manufactured {len(m.hybrid_orth_features_)} "
            f"engineered columns on a pure-noise frame: "
    ...
    def test_mi_greedy_emits_no_spurious_columns(self, seed):
        ...
        assert len(m.mi_greedy_features_) <= 5, (
            f"seed={seed}: mi_greedy manufactured {len(m.mi_greedy_features_)} "
            f"engineered columns on a pure-noise frame: "
    ...
    def test_both_enabled_no_spurious_columns(self, seed):
        ...
        m.fit(X, y)
        total_spurious = (
            len(m.hybrid_orth_features_) + len(m.mi_greedy_features_)
        )
...
class TestAdversarialPair:
    ...
    def test_no_noise_noise_cross_in_hybrid_orth_pair(self, seed):
        ...
        for name in m.hybrid_orth_features_:
            ...
            mentioned_signal = any(
                src in name for src in ("x_real", "x_pad")
            )
            ...
            if mentioned_noise and not mentioned_signal:
                ...
                pytest.fail(
                    f"seed={seed}: hybrid_orth pair {name!r} crosses two "
                    f"noise columns; absolute-MI floor leaked. All: "
                    f"{m.hybrid_orth_features_}"
                )
...
class TestMultiCollinearSources:
    ...
    def test_hybrid_orth_does_not_emit_he2_for_every_duplicate(self, seed):
        ...
        he2_cols = [
            c for c in m.hybrid_orth_features_
            if c.endswith("__He2") or "He2" in c
        ]
        ...
        assert len(he2_cols) <= 10, (
            f"seed={seed}: hybrid_orth emitted {len(he2_cols)} He_2 columns "
            f"(> top_k cap); {m.hybrid_orth_features_}"
        )
...
class TestHybridMiGreedyConflictResolution:
    ...
    def test_each_constructor_alone_emits_winner(self, seed):
        ...
        assert hybrid_he2, (
            f"seed={seed}: hybrid_orth (alone) did not emit x__He2 for "
            f"quadratic signal; got {m_h.hybrid_orth_features_}"
        )
        # MI-greedy alone.
        m_g = _make_mrmr(
        ...
        m_g.fit(X, y)
        mig_x2 = [
            c for c in m_g.mi_greedy_features_
            if c in {"square(x)", "abs(x)", "sqrt_abs(x)", "log_abs(x)"}
        ]
        assert mig_x2, (
            f"seed={seed}: mi_greedy (alone) did not emit a |x|-family "
            f"transform for quadratic signal; got {m_g.mi_greedy_features_}"
        )
    ...
    def test_final_support_picks_at_most_one_x_quadratic(self, seed):
        ...
        x_quadratic = [
            ...
            if (
                "He2" in c
                or c in {"square(x)", "abs(x)", "sqrt_abs(x)", "log_abs(x)"}
            )
        ]
        ...
        assert len(x_quadratic) <= 1, (
            f"seed={seed}: {len(x_quadratic)} quadratic-x encodings "
            f"survived selection ({x_quadratic}); MRMR redundancy should "
            f"keep at most one"
        )
...
class TestRecipeCorruptionSurvival:
    ...
    def test_pickle_unpickle_clean_roundtrip(self, seed):
        ...
        m_rt = pickle.loads(pickle.dumps(m))
    ...
    def test_corrupted_transform_name_raises_actionable(self, seed):
        ...
        if target_idx is None:
            pytest.skip(
                f"seed={seed}: no mi_greedy_transform recipe in support "
                f"({[r.name for r in recipes]}); cannot test corruption"
            )
        ...
        assert "this_is_not_a_real_transform_xyz" in msg, (
            f"seed={seed}: corruption raise must name the bad transform "
            f"for actionability; got: {msg!r}"
        )
    ...
    def test_corrupted_src_names_raises_or_recovers(self, seed):
        ...
        if target_idx is None:
            pytest.skip(
                f"seed={seed}: no mi_greedy_transform recipe in support; "
                f"cannot test corruption"
            )
        original = recipes[target_idx]
        # Replace src_names with a column NOT in X (and not in
        # feature_names_in_ either).
        bogus_src = tuple(
            f"col_definitely_not_in_X_{i}" for i in range(len(original.src_names))
        )
        ...
        try:
            ...
            assert any(b in msg for b in bogus_src), (
                f"seed={seed}: error must name the missing column for "
                f"actionability; got: {msg!r}"
            )
            return
        # Reached only on silent success -> contract failure.
        pytest.fail(
            f"seed={seed}: transform() silently succeeded with corrupted "
            f"src_names {bogus_src!r}; expected an actionable raise."
        )
