...
class TestRankingQuality:
    ...
    def test_top_k_precision_5_signals_15_noise(self):
        ...
        assert prec_at_5 >= 0.4, f"top-5 signal-recovery precision too low: {prec_at_5:.2f} ({recovered_top5} signals folded into top-5); selected={selected}"
        assert recall >= 0.4, f"signal recovery recall too low: {recall:.2f} ({recovered_total}/5 signals recovered); selected={selected}"
        ...
        auc_base = cross_val_score(
            ...
            X[[f"sig{k}" for k in range(5)]].to_numpy(),
            y.to_numpy(),
            cv=5,
            scoring="roc_auc",
        ).mean()
    ...
    def test_no_noise_in_top_3_with_strong_signal(self):
        ...
        y_arr = ((cols["sig0"] + 0.7 * cols["sig1"] + 0.5 * cols["sig2"]) > 0).astype(np.int64)
        for k in range(10):
...
class TestClusterRecall:
    ...
    def test_three_clusters_each_represented(self):
        ...
        y = pd.Series((sum(latents) > 0).astype(np.int64))
        X = pd.DataFrame(cols)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            sel = MRMR(
        ...
        selected = list(sel.get_feature_names_out())
        clusters_represented = sum(1 for c in range(3) if any(n.startswith(f"clu{c}_") or f"clu{c}" in n for n in selected))
        # At least 2 of 3 clusters must surface (3/3 is harder for
        # finite n).
        assert clusters_represented >= 2, f"cluster recall too low: only {clusters_represented}/3 clusters; selected={selected}"
...
class TestStabilityQuant:
    ...
    def test_same_seed_overlap_100pct(self):
        ...
        # Jaccard similarity
        jaccard = len(sup1 & sup2) / max(len(sup1 | sup2), 1)
        assert jaccard == 1.0, f"same-seed reproducibility: Jaccard={jaccard:.3f}; sup1={sup1}, sup2={sup2}"
    ...
    def test_different_seed_high_overlap_on_clear_signal(self):
        ...
        assert jaccard >= 0.5, f"different-seed overlap on clear signal: Jaccard={jaccard:.3f}"
...
class TestVsBaselineRanking:
    ...
    def test_mrmr_with_dcd_avoids_redundant_copies(self):
        ...
        assert sig_a_copies < 5, f"DCD failed to prune redundant copies; sig_a_copies={sig_a_copies}/5; selected={names}"
        # And sig_b (or aggregate) must surface.
        assert sig_b_present or has_pc1, f"sig_b unique signal lost; selected={names}"
