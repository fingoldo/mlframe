...
def test_fe_runs_on_polars_input_and_matches_pandas():
    ...
    base = dict(verbose=0, random_seed=0, n_jobs=1, fe_max_steps=2,
    ...
    assert eng_pl, (
        f"polars input produced NO engineered features (FE silently skipped); "
        f"selection={list(m_pl.get_feature_names_out())}"
    )
    # And the engineered structure should match the pandas fit (same bridge -> same FE).
    assert set(eng_pl) == set(eng_pd), (
        f"polars FE diverged from pandas FE:\n  pandas={sorted(eng_pd)}\n  polars={sorted(eng_pl)}"
    )
...
def test_fe_disabled_polars_stays_native_no_error():
    ...
    df_pl = pl.DataFrame(data)
    m = MRMR(verbose=0, random_seed=0, n_jobs=1, fe_max_steps=0,
             dcd_enable=False, build_friend_graph=False, cluster_aggregate_enable=False).fit(
