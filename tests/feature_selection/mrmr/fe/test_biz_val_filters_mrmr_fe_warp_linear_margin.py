...
def test_biz_val_warp_linear_margin_low_displaces_to_linear_f():
    ...
    assert sv == [1], f"low warp_linear_margin must let f displace the exp-warp anchor g; selected_vars={sv}"
    assert bool(state.pool_pruned_mask[0]) and not bool(state.pool_pruned_mask[1]), f"g pruned, f kept; mask={state.pool_pruned_mask.tolist()}"
...
def test_biz_val_warp_linear_margin_high_suppresses_displacement():
    ...
    assert sv == [0], f"high warp_linear_margin must suppress the displacement (gap < margin); selected_vars={sv}"
    assert not bool(state.pool_pruned_mask[0]) and bool(state.pool_pruned_mask[1]), f"g kept, f pruned; mask={state.pool_pruned_mask.tolist()}"
...
def test_biz_val_warp_linear_margin_delta_flips_survivor():
    ...
    assert sv_low == [1] and sv_high == [0], f"margin alone must flip the survivor: low->f (sv={sv_low}), high->g (sv={sv_high})"
