...
np.random.seed(0); n=60000
a=np.random.rand(n);b=np.random.rand(n);c=np.random.rand(n);d=np.random.rand(n);e=np.random.rand(n);f=np.random.rand(n)
y=0.2*a**2/b + f/5 + np.log(c*2)*np.sin(d/3)
rng=np.random.default_rng(0); idx=rng.permutation(n); tr=idx[:int(.7*n)]; va=idx[int(.7*n):int(.85*n)]; te=idx[int(.85*n):]
raw={"a":a,"b":b,"c":c,"d":d,"e":e}
UN={"id":lambda x:x,"sqr":lambda x:x**2,"log":smart_log,"sin":np.sin,"exp":np.exp,"cbrt":np.cbrt,"neg":np.negative,"abs":np.abs,"rec":lambda x:np.power(x,-1.)}
BI={"add":lambda u,v:u+v,"sub":lambda u,v:u-v,"mul":lambda u,v:u*v,"div":lambda u,v:u/v}
def scrub(v): return np.nan_to_num(np.asarray(v,float),nan=0,posinf=0,neginf=0)
def mi(v): return float(_cmi_from_binned(_quantile_bin(scrub(v),12),_quantile_bin(y,12),None))
pool={}
for k,v in raw.items(): pool[k]=scrub(v)
for (n1,v1),(n2,v2) in itertools.combinations(raw.items(),2):
    for ua,ub in itertools.product(UN,UN):
        for bn,bf in BI.items():
            ...
            if np.std(val)<1e-9: continue
            if mi(val)<0.05: continue
            pool[f"{bn}({ua}({n1}),{ub}({n2}))"]=val
names=list(pool); V=np.column_stack([pool[k] for k in names]); MIs=np.array([mi(pool[k]) for k in names])
print(f"pool={len(names)}")
def lin_fit_resid(cols,split):
    # residual of y after a linear fit on cols (fit on tr), evaluated on split
    if not cols: return y[split]-y[tr].mean()
    m=make_pipeline(StandardScaler(),LinearRegression()).fit(V[:,cols][tr],y[tr]); return y[split]-m.predict(V[:,cols][split])
def lin_mae(cols):
    m=make_pipeline(StandardScaler(),LinearRegression()).fit(V[:,cols][tr],y[tr]); return MAE(y[te],m.predict(V[:,cols][te]))
def abscorr(u,v):
    if np.std(u)<1e-12 or np.std(v)<1e-12: return 0.0
    r=np.corrcoef(u,v)[0,1]; return abs(r) if np.isfinite(r) else 0.0
def greedy(K=6,w=0.0):
    ...
    for _ in range(K):
        ...
        for i in range(len(names)):
            ...
            if w>0: score=(1-w)*(MIs[i]-0.5*red)+w*use
            else:   score=MIs[i]-0.7*red
            if score>best: best,bi=score,i
        if bi<0: break
        sel.append(bi)
    return sel
for w in (0.0,0.5,0.8):
    sel=greedy(6,w); print(f"w={w}: linMAE={lin_mae(sel):.4f} sel={[names[i] for i in sel]}")
