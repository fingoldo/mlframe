...
class TestValidatePairFeCV:
    ...
    def test_return_keys(self):
        ...
        out = validate_pair_fe_cv(
            x_a, x_b, y, n_splits=3, n_trials=6, max_degree=2, seed=0,
        )
        expected = {
            "in_sample_mi", "oos_mean", "oos_std", "oos_per_fold",
            "optimism_ratio", "trivial_oos_mean", "honest_uplift_vs_trivial",
    ...
    def test_pure_noise_no_spurious_uplift(self):
        ...
        assert out["oos_mean"] <= 0.1, (
            f"spurious oos uplift on pure-noise target: {out['oos_mean']:.3f}"
        )
    ...
    def test_determinism_same_seed(self):
        ...
        out_a = validate_pair_fe_cv(
            x_a, x_b, y, n_splits=3, n_trials=6, max_degree=2, seed=123,
        )
...
def test_biz_xor_pair_uplifts_over_parents():
    ...
    out = validate_pair_fe_cv(
        x_a, x_b, y, n_splits=3, n_trials=8, max_degree=2, seed=0,
    )
    uplift = out["oos_mean"] / best_parent
    assert uplift >= 2.0, (
        f"XOR pair uplift {uplift:.2f}x over best parent (mi_a={mi_a:.4f}, mi_b={mi_b:.4f}, "
        f"oos_mean={out['oos_mean']:.4f}) - expected >= 2.0"
    )
