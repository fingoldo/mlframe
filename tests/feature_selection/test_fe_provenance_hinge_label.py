...
def test_no_real_recipe_kind_falls_to_engineered_unknown():
    ...
    for kind in (
        "hinge_basis", "numeric_rounding", "digit_extract", "modular",
        "group_distance", "temporal_expanding", "temporal_rolling",
        "temporal_lag", "orth_wavelet", "rare_category", "conditional_residual",
        "conditional_dispersion", "rankgauss", "div", "log_div",
        "grouped_agg", "composite_group_agg", "grouped_quantile",
        "target_aware_group_bin", "cat_pair_cross", "cat_triple_cross",
        "orth_diff_basis", "orth_cluster_basis", "orth_triplet_cross",
        "orth_quadruplet_cross",
    ):
        assert kind in mapped and kind not in INTENTIONAL_UNKNOWN, (
            f"recipe kind {kind!r} not mapped -> would render as engineered_unknown"
        )
...
def _fe_on():
    ...
    return MRMR(
        verbose=0, random_seed=0, dcd_enable=False,
        cluster_aggregate_enable=False, build_friend_graph=False,
        stability_selection_method="classic", retain_artifacts=False,
        n_jobs=1, fe_hybrid_orth_enable=True, fe_auto=True,
    )
...
def test_canonical_hinge_legs_labeled_not_unknown():
    ...
    assert origins == {"hinge_basis"}, (
        f"hinge legs carry wrong origin(s): {origins}"
    )
