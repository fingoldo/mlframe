...
def test_rfecv_cv_shuffle_true_overrides_temporal_pandas(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="mlframe.feature_selection.wrappers.rfecv"):
        cv, _val, _ = _resolve_cv_and_val_cv(
            ...
            early_stopping_rounds=0, _polars_time_series_hint=False, verbose=1,
        )
    ...
    assert any("cv_shuffle=True" in m and "TimeSeriesSplit" in m for m in warn_messages), \
        f"expected explicit cv_shuffle override warning; got {warn_messages}"
...
def test_rfecv_cv_shuffle_false_still_swaps_to_tss_on_temporal_pandas(caplog):
    ...
    est = LogisticRegression(max_iter=50)
    cv, _val, _ = _resolve_cv_and_val_cv(
        cv=3, X=X, y=y, groups=None, estimator=est, cv_shuffle=False,
        random_state=None, fit_params={}, early_stopping_val_nsplits=0,
        early_stopping_rounds=0, _polars_time_series_hint=False, verbose=0,
    )
...
def test_rfecv_cv_shuffle_true_overrides_polars_hint(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="mlframe.feature_selection.wrappers.rfecv"):
        cv, _val, _ = _resolve_cv_and_val_cv(
            cv=3, X=X, y=y, groups=None, estimator=est, cv_shuffle=True,
            random_state=42, fit_params={}, early_stopping_val_nsplits=0,
            early_stopping_rounds=0, _polars_time_series_hint=True, verbose=0,
        )
