...
def test_selection_identical_numpy_vs_pandas(seed):
    ...
    assert out_np == out_pd, f"get_feature_names_out differs between numpy fast path and pandas path (seed={seed}):\n  numpy : {out_np}\n  pandas: {out_pd}"
    ...
    np.testing.assert_array_equal(
        np.asarray(cv_np["cv_mean_perf"]),
        np.asarray(cv_pd["cv_mean_perf"]),
