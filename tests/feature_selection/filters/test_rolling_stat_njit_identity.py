...
def _sorted_by_entity_time(n, n_ent, seed):
    ...
    for g in np.unique(gc):
        m = gc == g; t[m] = np.sort(rng.uniform(0, 1000, int(m.sum())))
    vals = rng.standard_normal(n); vals[::19] = np.nan
    return t, vals, gc
