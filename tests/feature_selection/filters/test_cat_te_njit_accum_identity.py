...
def test_cell_sum_cnt_njit_bit_identical():
    ...
    cs = np.zeros(nu); cc = np.zeros(nu)
    for row in range(n):
        c = int(classes[row]); cs[c] += y[row]; cc[c] += 1.0
...
def test_kfold_te_fit_runs_and_deterministic():
    ...
    def fit():
        ...
        m = MRMR(fe_ntop_features=4, n_jobs=1, verbose=0, random_seed=3,
                 fe_kfold_te_enable=True, fe_kfold_te_cols=["cat0", "cat1", "cat2"],
