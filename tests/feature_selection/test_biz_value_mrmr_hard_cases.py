...
class TestSignalDetection:
    def test_xor_synergy_both_features_selected(self):
        ...
        carries_signal = (
            ...
            or any("x1" in n and "x2" in n for n in names)
        )
        assert carries_signal, (
            f"XOR synergy not detected; support={names}"
        )
    ...
    def test_signal_under_heavy_collinearity(self):
        ...
        has_signal = (
            "signal" in names
            or any(n.startswith("sig_copy") for n in names)
            or any("dcd_pc1" in n or "_pc1_" in n for n in names)
        )
        assert has_signal, (
            f"signal lost under collinearity; support={names}"
        )
        # Pure noise must NOT be the top pick.
        if names:
            top = names[0]
            assert not top.startswith("noise"), (
                f"pure noise feature ranked first: top={top}"
            )
    ...
    def test_rare_positive_class_1pct(self):
        ...
        assert "signal" in names, (
            f"rare-positive signal missed; support={names}, pos_rate=1%"
        )

    @pytest.mark.parametrize("nbins_strategy", [
...
class TestClusterDiscovery:
    def test_three_latent_factors_with_copies(self):
        ...
        f0_present = any(
            n.startswith("f0_") or "f0_anchor" in n or "_pc1_" in n
            for n in names
        )
        assert f0_present, (
            f"f0 cluster missed entirely; support={names}"
        )
    ...
    def test_anti_correlated_cluster_members(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            sel = MRMR(verbose=0, dcd_enable=True,
                       dcd_tau_cluster=0.5,
                       dcd_cluster_size_threshold=2).fit(X, y)
        names = list(sel.get_feature_names_out())
        ...
        assert has_only_one_sig, (
            f"redundant anti-correlated pair both kept; support={names}"
        )
...
class TestRobustness:
    def test_single_unique_value_column_ignored(self):
        ...
        assert "const" not in names, (
            f"constant column selected; support={names}"
        )
    ...
    def test_categorical_with_rare_levels(self):
        ...
        names = list(sel.get_feature_names_out())
        assert "cat" in names, (
            f"categorical with rare levels missed; support={names}"
        )
    ...
    def test_target_string_labels_3_class(self):
        ...
        names = list(sel.get_feature_names_out())
        assert "signal" in names, (
            f"multi-class string target: signal missed; support={names}"
        )
...
class TestNaNAsSignal:
    def test_nan_pattern_perfectly_predicts_y(self):
        ...
        with warnings.catch_warnings():
            ...
            sel = MRMR(verbose=0).fit(X, y)
        names = list(sel.get_feature_names_out())
        assert "nan_signal" in names, (
            f"NaN-as-signal missed; support={names}"
        )
