...
def cat_frame():
    ...
    cats = rng.choice(
        ...
        p=[0.4, 0.3, 0.25, 0.03, 0.02], size=n,
    )
...
def test_rare_category_fit_equals_replay(cat_frame):
    ...
    for name, payload in raw.items():
        rec = build_rare_category_recipe(
            name=name, src_col=payload["src_col"], kind=payload["kind"],
...
def test_rare_category_unseen_maps_to_rare(cat_frame):
    ...
    rec = build_rare_category_recipe(
        name=name, src_col=payload["src_col"], kind=payload["kind"],
        freq_lookup=payload["freq_lookup"], rare_threshold=payload["rare_threshold"],
        dominant_cut=payload["dominant_cut"],
    )
...
def test_conditional_residual_fit_equals_replay(num_frame):
    ...
    for name, payload in raw.items():
        rec = build_conditional_residual_recipe(
            name=name, x_i=payload["x_i"], x_j=payload["x_j"],
            edges=payload["edges"], bin_mean=payload["bin_mean"],
...
def test_conditional_dispersion_fit_equals_replay(num_frame):
    enc_df, raw = generate_conditional_dispersion_features(
        num_frame, ["x0", "x2"], n_bins=8, kinds=("absz", "z2"),
    )
    assert len(raw) > 0
    for name, payload in raw.items():
        rec = build_conditional_dispersion_recipe(
            name=name, x_i=payload["x_i"], x_j=payload["x_j"],
            edges=payload["edges"], bin_mean=payload["bin_mean"],
            bin_std=payload["bin_std"], global_mean=payload["global_mean"],
            global_std=payload["global_std"], kind=payload["kind"],
        )
        replay = apply_recipe(rec, num_frame)
        np.testing.assert_allclose(replay, enc_df[name].to_numpy(), rtol=0, atol=1e-9)
...
def test_conditional_dispersion_kind_is_frozen(num_frame):
    ...
    rec_abz = build_conditional_dispersion_recipe(
        name=abz_name, x_i=abz_p["x_i"], x_j=abz_p["x_j"], edges=abz_p["edges"],
        bin_mean=abz_p["bin_mean"], bin_std=abz_p["bin_std"],
        global_mean=abz_p["global_mean"], global_std=abz_p["global_std"], kind="absz",
    )
    rec_z2 = build_conditional_dispersion_recipe(
        name=z2_name, x_i=z2_p["x_i"], x_j=z2_p["x_j"], edges=z2_p["edges"],
        bin_mean=z2_p["bin_mean"], bin_std=z2_p["bin_std"],
        global_mean=z2_p["global_mean"], global_std=z2_p["global_std"], kind="z2",
    )
...
def test_rankgauss_fit_equals_replay(num_frame):
    ...
    for name, payload in raw.items():
        rec = build_rankgauss_recipe(
            name=name, src_col=payload["src_col"],
            fit_sorted=payload["fit_sorted"], n_fit=payload["n_fit"],
        )
        replay = apply_recipe(rec, num_frame)
