...
def test_maximal_preset_excludes_non_rowwise_pure_and_replays_slice_consistent():
    ...
    with warnings.catch_warnings():
        ...
        fs = MRMR(verbose=0, random_seed=0, fe_unary_preset="maximal", fe_binary_preset="maximal").fit(X=df, y=pd.Series(y, name="y"))
    ...
    for recipe in getattr(fs, "_engineered_recipes_", []):
        full = np.nan_to_num(np.asarray(apply_recipe(recipe, df), dtype=np.float64), nan=0.0, posinf=0.0, neginf=0.0)
        sl = df.iloc[500:900].reset_index(drop=True)
        part = np.nan_to_num(np.asarray(apply_recipe(recipe, sl), dtype=np.float64), nan=0.0, posinf=0.0, neginf=0.0)
        assert np.allclose(full[500:900], part, atol=1e-6, rtol=1e-5), f"recipe {recipe.name!r} not slice-replay-consistent (row-wise-purity violation)"
