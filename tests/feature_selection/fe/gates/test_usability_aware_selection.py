...
def test_usability_aware_reaches_linear_floor_with_replayable_recipes():
    ...
    def _matrix(selected, Xframe):
        ...
        for s in selected:
            if s.recipe is None:
                ...
                rep = np.nan_to_num(np.asarray(apply_recipe(s.recipe, Xframe), dtype=float),
    ...
    with warnings.catch_warnings():
        ...
        sel_lin = select_usability_aware_features(Xtr, ytr, list(df.columns), w=0.8, K=6, seed=0,
    ...
    assert mae_lin <= 0.07, (
        f"usability-aware linear MAE {mae_lin:.4f} did not reach the f/5 floor (~0.05); "
        f"selected={[s.name for s in sel_lin]}"
    )
