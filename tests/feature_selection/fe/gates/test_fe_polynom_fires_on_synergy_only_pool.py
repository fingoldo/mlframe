...
def _interaction_only_frame(n=3000, seed=42):
    ...
    cols = {k: rng.normal(size=n) for k in
            ["x_a", "x_b", "x_c", "x_d", "x_e", "x_f", "noise1", "noise2"]}
    yc = (1.0 * cols["x_a"] * cols["x_b"]
          + 0.5 * cols["x_c"] * cols["x_d"]
          + 0.3 * cols["x_e"] * cols["x_f"]
...
def test_smart_polynom_fires_when_pool_is_all_synergy():
    ...
    m = MRMR(
        ...
        fe_smart_polynom_iters=2, fe_smart_polynom_optimization_steps=30,
    ).fit(df, y)
