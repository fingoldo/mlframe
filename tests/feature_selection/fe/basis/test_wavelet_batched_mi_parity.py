...
def test_wavelet_batched_mi_matches_per_column_cpu():
    ...
    mi_cpu = np.array([_binned_mi(cols[:, k].astype(np.float64), y, nbins=int(kx[k]) + 1) for k in range(K)])
    ...
    assert np.allclose(mi_batched, mi_cpu, atol=1e-9), f"max|d|={np.max(np.abs(mi_batched - mi_cpu)):.2e}"
