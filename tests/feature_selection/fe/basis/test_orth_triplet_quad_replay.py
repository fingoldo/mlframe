...
def test_triplet_recipe_frozen_params_replay_slice_consistent():
    ...
    rec = build_orth_triplet_cross_recipe(
        ...
        deg_b=1,
        deg_c=2,
        preprocess_params_i=pps[0],
        preprocess_params_j=pps[1],
        preprocess_params_k=pps[2],
    )
...
def test_triplet_recipe_unfrozen_DRIFTS_on_slice_proving_the_bug():
    ...
    part = np.asarray(apply_recipe(rec, sl), dtype=np.float64)
    assert not np.allclose(full[1000:1700], part, atol=1e-6), "expected the UNFROZEN recipe to drift on a slice (bug not reproduced -- test would be vacuous)"
...
def test_diff_basis_recipe_frozen_params_replay_slice_consistent():
    ...
    rec = build_orth_diff_basis_recipe(name="diff", col_a="a", col_b="b", basis="hermite", degree=2, preprocess_params=pp)
    full = np.asarray(apply_recipe(rec, df), dtype=np.float64)
    sl = df.iloc[1000:1700].reset_index(drop=True)
    part = np.asarray(apply_recipe(rec, sl), dtype=np.float64)
...
def test_diff_basis_recipe_unfrozen_DRIFTS_on_slice_proving_the_bug():
    ...
    sl = df.iloc[1000:1700].reset_index(drop=True)
    part = np.asarray(apply_recipe(rec, sl), dtype=np.float64)
    assert not np.allclose(full[1000:1700], part, atol=1e-6), "expected the UNFROZEN diff-basis recipe to drift on a slice (bug not reproduced -- test vacuous)"
