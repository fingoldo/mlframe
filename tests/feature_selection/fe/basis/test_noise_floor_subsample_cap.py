...
def _mk(rng, n, kind):
    ...
    if kind == "prod":
        ...
        yc = 0.03 * a * b + rng.standard_normal(n)
...
def test_noise_floor_cap_is_selection_equivalent_to_full_n(kind, monkeypatch):
    ...
    assert (r_full is None) == (r_cap is None), f"noise-floor cap flipped the reject decision on '{kind}': full={r_full is None} cap={r_cap is None}"
