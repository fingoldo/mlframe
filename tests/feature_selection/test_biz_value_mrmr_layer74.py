...
def _make_mrmr(**overrides):
    """Cheap-and-deterministic MRMR ctor (mirrors Layer 72 / 73)."""
...
class TestCmimRanksRedundantLow:
    ...
    def test_cmim_redundant_far_below_novel(self, seed):
        ...
        assert eng_dup, (
            f"seed={seed}: no x_dup_*__He columns ranked: {list(s_map)}"
        )
...
class TestCmimVsJmimAgreement:
    ...
    def test_cmim_jmim_rank_agreement_clean_fixture(self):
        ...
        for s in SEEDS:
            ...
            assert len(common) >= 4, (
                f"seed={s}: too few common engineered cols "
                f"({len(common)}) for the rank agreement test."
            )
            ...
            assert np.isfinite(rho), (
                f"seed={s}: Spearman rho is NaN; ranking variance "
                f"degenerated."
            )
...
class TestAucLiftViaCmim:
    ...
    def test_cmim_augmented_logreg_auc_beats_marginal_mi(self):
        ...
        for s in (1, 7, 13, 42, 101, 202, 303, 404):
            ...
            cmim_added = [
                c for c in X_cmim_tr.columns if c not in X_tr.columns
            ]
            X_cmim_te = (
                pd.concat([X_te, eng_te_all[cmim_added]], axis=1)
...
class TestDefaultDisabledByteIdentical:
    ...
    def test_default_off_no_cmim_columns(self, seed):
        ...
        assert added == [], (
            f"seed={seed}: default fe_hybrid_orth_cmim_enable=False "
