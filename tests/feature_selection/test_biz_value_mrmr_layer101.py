...
2. Composite mega-fixture, ALL-ON via the two default-flip auto knobs:
   ...
   columns from >= 4 distinct origins, downstream LogReg AUC >= 0.85, and
   no engineered name collisions appear.
...
class TestMegaFixtureAllOn:
    ...
    def test_downstream_auc_at_least_085(self):
        aucs = []
        for s in (1, 7, 42):
        ...
        assert mean_auc >= 0.85, (
            f"downstream LogReg AUC {mean_auc:.4f} < 0.85 on the mega-fixture "
            f"(per-seed {[round(a, 4) for a in aucs]}); the all-on FE pipeline "
            f"failed to recover the planted signal."
        )
...
class TestParamOracleCoexistence:
    ...
    def test_three_consumers_distinct_buckets(self, tmp_path):
        ...
        from mlframe.feature_selection.filters._oracle_scorer_select import (
            ORACLE_FN_NAME as SCORER_FN, OracleScorerSelector,
        )
        ...
        sel.benchmark_all_scorers(
            _clean_numeric(X), y, degrees=(2,), basis="hermite", n_boot=3,
            ts=FIXED_TS,
        )
        ...
        gen.record(
            default_fingerprint((X, y), {}), {"alpha": 0.5},
            {"quality": 0.7}, ts=FIXED_TS, fn_name=gen_fn,
        )
