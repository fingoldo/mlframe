...
from mlframe.feature_selection._shap_proxy_cluster import build_unit_matrix
...
def test_signed_l2_sum_not_collapsed_to_median():
    ...
    assert not np.allclose(units_l2[:, 0], units_med[:, 0]), (
        "signed_l2_sum aggregate is identical to median -- the non-linear "
        "dispatch is not being applied (gap-04 regression)."
    )
...
def test_signed_l2_sum_matches_canonical_reducer():
    ...
    assert np.allclose(units_l2[:, 0], expected), (
        "build_unit_matrix signed_l2_sum aggregate must match the canonical "
        "_apply_method_nonlinear reducer."
    )
