...
def test_biz_val_interaction_coalition_sees_xor_pair():
    ...
    phi, base, y_phi = compute_shap_matrix(model, X, y, classification=True, out_of_fold=False, n_models=1, rng=np.random.default_rng(0))
    ...
    int_01, int_0 = (interaction_subset_loss(Phi, ibase, y, [0, 1], "brier"), interaction_subset_loss(Phi, ibase, y, [0], "brier"))
    ...
    assert (int_0 - int_01) > (main_0 - main_01), f"interaction gap {(int_0 - int_01):.3f} not above main gap {(main_0 - main_01):.3f}"
