...
def test_resolve_shap_prefilter_top_uses_safety_factor():
    ...
    assert resolve_shap_prefilter_top(brute_force_max_features=22, safety_factor=4, min_features=40) == 88
...
def test_resolve_shap_prefilter_top_respects_min_features_floor():
    ...
    assert resolve_shap_prefilter_top(brute_force_max_features=5, safety_factor=4, min_features=40) == 40
...
def test_resolve_shap_prefilter_top_safety_factor_is_int_multiplied():
    ...
    assert resolve_shap_prefilter_top(brute_force_max_features=22, safety_factor=1, min_features=40) == 40
    # factor 10 -> 220.
    assert resolve_shap_prefilter_top(brute_force_max_features=22, safety_factor=10, min_features=40) == 220
...
def test_shap_prefilter_preserves_informative_recall_synthetic():
    ...
    X, y, roles = make_regime_dataset(n_samples=5000, n_informative=12, n_redundant=0, redundancy_rho=0.0, n_noise=988, snr=8.0, task="binary", seed=0)
    sel = ShapProxiedFS(
        classification=True,
        metric="brier",
        optimizer="auto",
    ...
    assert len(informative & kept) == len(informative), f"informative recall dropped: {sorted(informative - kept)} lost, kept={sorted(informative & kept)}"
...
def test_resolve_shap_aware_stage1_keep_tightens_to_cushion_floor():
    ...
    assert resolve_shap_aware_stage1_keep(effective_prefilter_top=88, stage1_cushion=8, stage1_floor=200, default_stage1_keep=2000) == 704
...
def test_resolve_shap_aware_stage1_keep_floor_dominates_tiny_cap():
    ...
    assert resolve_shap_aware_stage1_keep(effective_prefilter_top=10, stage1_cushion=8, stage1_floor=200, default_stage1_keep=2000) == 200
...
def test_resolve_shap_aware_stage1_keep_never_expands_default():
    ...
    assert resolve_shap_aware_stage1_keep(effective_prefilter_top=200, stage1_cushion=8, stage1_floor=200, default_stage1_keep=500) == 500
...
def test_resolve_shap_aware_stage1_keep_never_below_effective_prefilter_top():
    ...
    assert resolve_shap_aware_stage1_keep(effective_prefilter_top=300, stage1_cushion=1, stage1_floor=10, default_stage1_keep=2000) == 300
...
def test_resolve_shap_aware_stage1_keep_none_eff_top_returns_default():
    ...
    assert resolve_shap_aware_stage1_keep(effective_prefilter_top=None, stage1_cushion=8, stage1_floor=200, default_stage1_keep=2000) == 2000
...
def test_selector_records_stage1_keep_tightened_when_lever_active():
    ...
    X, y, _ = make_regime_dataset(n_samples=600, n_informative=5, n_redundant=0, redundancy_rho=0.0, n_noise=1295, snr=8.0, task="binary", seed=0)  # 1300 cols
    sel = ShapProxiedFS(
        classification=True,
        metric="brier",
        optimizer="auto",
...
def test_selector_user_pinned_stage1_keep_overrides_lever():
    ...
    X, y, _ = make_regime_dataset(n_samples=600, n_informative=5, n_redundant=0, redundancy_rho=0.0, n_noise=1295, snr=8.0, task="binary", seed=0)
    sel = ShapProxiedFS(
        classification=True,
        metric="brier",
        optimizer="auto",
...
def test_biz_val_shap_aware_stage1_keep_e2e_speedup_at_target_regime():
    ...
    def run(enabled, seed):
        ...
        sel = ShapProxiedFS(
            ...
            within_cluster_refine=True,
            shap_prefilter_enabled=True,
            shap_aware_stage1_keep=enabled,
            random_state=seed,
            verbose=False,
        )
        t0 = time.perf_counter()
    ...
    assert r_new >= r_base - 1, f"new recall dropped vs baseline: base={r_base}/20 new={r_new}/20"
