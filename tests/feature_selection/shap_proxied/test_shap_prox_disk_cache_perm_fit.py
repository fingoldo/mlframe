...
def test_permutation_importance_default_no_cache_identical():
    ...
    base_a, imps_a = _permutation_importance_ranking(
        tpl, X_search, y_search, X_hold, y_hold, cols, True, "brier",
        n_estimators_cap=20, seed=0,
    )
...
def test_permutation_importance_cache_hit_identical_and_writes_entry(tmp_path: Path):
    ...
    base_cold, imps_cold = _permutation_importance_ranking(
        tpl, X_search, y_search, X_hold, y_hold, cols, True, "brier",
        n_estimators_cap=20, seed=0, disk_cache=dc, template_id="tpl_v1",
    )
    files_after_cold = [f.name for f in cache_dir.iterdir()]
    assert any(n.startswith("perm_imp_fit_") for n in files_after_cold), (
        f"cold call should have written a perm_imp_fit_ entry; got {files_after_cold}"
    )
    ...
    assert dc.misses == misses_after_cold, (
        f"warm call should not miss the fit cache; misses cold={misses_after_cold} warm={dc.misses}"
    )
...
def test_within_cluster_refine_two_fit_cache_writes_perm_fit_and_honest_loss(tmp_path: Path):
    ...
    assert any(n.startswith("honest_loss_") for n in files_cold), (
        f"refine should write honest_loss_ entries (iter81); got {files_cold}"
    )
    assert any(n.startswith("perm_imp_fit_") for n in files_cold), (
        f"refine stage-2a should write perm_imp_fit_ entries (iter82); got {files_cold}"
    )
    ...
    assert tuple(refined_cold) == tuple(refined_warm), (
        f"warm refine subset must match cold (cache served same booster + same losses); "
        f"cold={refined_cold} warm={refined_warm}"
    )
