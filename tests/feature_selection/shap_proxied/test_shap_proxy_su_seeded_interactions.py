...
def test_su_synergy_screen_ranks_true_pair_first():
    ...
    kept, info = su_synergy_screen(
        X, y, n_bins=8, top_k=20, max_screen_cols=120,
        snr_z=3.0, snr_null_quantile=0.99, n_permutations=3,
    ...
    assert {top_a, top_b} == {"op_a", "op_b"}, (
        f"true operand pair not ranked #0; got {top_a} x {top_b} (info={info})")
    ...
    assert info["best_synergy"] >= 5 * info["gate"], (
        f"true synergy {info['best_synergy']} not well above the noise floor {info['gate']}")
...
def test_su_synergy_screen_snr_gate_no_ops_on_noise():
    ...
    assert info["best_synergy"] < info["gate"], (
        f"best noise synergy {info['best_synergy']} should sit below the gate {info['gate']}")
...
def _shap_sel(su_seeded, n_features):
    ...
    return ShapProxiedFS(
        ...
        prefilter_top=min(40, n_features), prefilter_n_estimators=60,
        ...
        su_seeded_interactions=su_seeded, random_state=0, verbose=False)
...
def test_su_seeded_facade_recovers_pure_interaction_operands():
    ...
    assert on_recall > off_recall, (
        f"su_seeded recall {on_recall} did not beat additive-default recall {off_recall} "
        f"(off={sorted(off_set)}, on={sorted(on_set)})")
...
def test_su_seeded_no_op_leaves_additive_default_byte_identical():
    ...
    assert rep.get("n_kept_pairs", 0) == 0, (
        f"expected SNR-gate no-op but kept {rep.get('kept_pairs')}; tune the bed if the floor moved")
    ...
    assert sel_on.selected_features_ == sel_off.selected_features_, (
        ...
        f"on ={sel_on.selected_features_}")
