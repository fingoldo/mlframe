...
def test_fe_max_polynoms_fit_transform_no_keyerror():
    ...
    with warnings.catch_warnings():
        ...
        m = MRMR(interactions_max_order=2, fe_max_polynoms=4, full_npermutations=1, cv=2,
...
def test_poly_recipe_persists_coef_and_replays_via_hermval():
    ...
    r = build_unary_binary_recipe(
        name="poly_test", src_a_name="a", src_b_name="b",
        unary_a_name="poly_" + str(coef), unary_b_name="identity", binary_name="mul",
        unary_preset="medium", binary_preset="medium",
        quantization_nbins=None, quantization_method=None, quantization_dtype=np.int8,
        poly_a_coef=coef,
    )
