...
def test_unit_rescue_fires_for_weak_smooth_interaction_inert_on_noise():
    ...
    assert ("a", "b") in eligible_pairs or ("b", "a") in eligible_pairs, (
        f"rescue must route the prevalence-failed (a,b) synergy pair into escalation; "
        f"eligible={eligible_pairs}"
    )
    ab = _ab_from_recipes(sel)
    names = list(sel.get_feature_names_out())
    assert ab, (
        f"an (a,b) engineered feature must be admitted by the rescue path; "
        f"feature_names_out={names}; admitted="
        f"{[h.get('admitted') for h in hist]}"
    )
    ...
    assert out_cols, (
        f"an (a,b) escalation column must reach the output; names={names}"
    )
    Xt = sel.transform(df.iloc[:2000])
    truth = (df["a"].values[:2000] ** 2) / df["b"].values[:2000]
    best = max(
        abs(float(np.corrcoef(np.nan_to_num(np.asarray(Xt[c], dtype=np.float64)),
                              truth)[0, 1]))
        for c in out_cols if c in Xt.columns
    )
...
def test_unit_rescue_can_be_disabled():
    ...
    df, y = _make_f2(seed=11, n=12000)
    MRMR.clear_fit_cache()
    sel = MRMR(verbose=0, n_jobs=1, random_seed=0,
               fe_synergy_prevalence_rescue_enable=False, **_LEAN)
    sel.fit(df, y)
    assert not _ab_from_recipes(sel), (
        "rescue OFF must NOT produce an (a,b) feature (true-negative control)"
    )
...
def test_biz_value_rescue_improves_f2_downstream_r2():
    ...
    def _r2(sel):
        Xt = sel.transform(df)
        X = np.column_stack([np.nan_to_num(np.asarray(Xt[c], dtype=np.float64))
                             for c in Xt.columns])
        m = HistGradientBoostingRegressor(random_state=0, max_iter=150)
        return cross_val_score(m, X, y.values, cv=KFold(5, shuffle=True, random_state=0),
                               scoring="r2").mean()

    MRMR.clear_fit_cache()
    off = MRMR(verbose=0, n_jobs=1, random_seed=0,
               fe_synergy_prevalence_rescue_enable=False, **_LEAN)
    off.fit(df, y)
    r2_off = _r2(off)
