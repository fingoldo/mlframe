...
def test_cv_skill_logs_warning_on_inner_failure(caplog):
    ...
    assert any("_cv_skill" in m and "CV scoring failed" in m for m in msgs), (
        f"inner CV failure must be logged, not silently swallowed; got {msgs!r}")
...
def test_cv_skill_happy_path_no_warning(caplog):
    ...
    y = (X[:, 0] > 0).astype(int)
    with caplog.at_level(logging.WARNING, logger="mlframe.feature_selection.hetero_vote"):
        r = _cv_skill(DecisionTreeClassifier(random_state=0), X, y,
                      classification=True, folds=3, random_state=0)
    assert r >= 0.0
