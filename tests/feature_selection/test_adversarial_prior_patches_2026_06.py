...
def test_synergy_does_not_admit_pure_noise_pair():
    ...
    out = detect_synergy_combos([cx, cy], yt, [0, 1], max_order=2, min_order=2,
                                synergy_ratio=1.5, min_joint_mi=0.05)
...
def test_synergy_recovers_2way_xor():
    ...
    out = detect_synergy_combos([a, b, c], yt, [0, 1, 2], max_order=2, min_order=2,
                                synergy_ratio=1.5, min_joint_mi=0.05)
    combos = {frozenset(t[0]) for t in out}
    assert frozenset((0, 1)) in combos, f"2-way XOR signal pair missed: {out}"
    assert frozenset((0, 2)) not in combos and frozenset((1, 2)) not in combos, \
        f"noise-bearing pair admitted: {out}"
...
def test_synergy_recovers_3way_xor_and_rejects_noise_triple():
    ...
    cols = [a, b, c, n1, n2, n3]
    out = detect_synergy_combos(cols, yt, list(range(6)), max_order=3, min_order=3,
                                synergy_ratio=1.5, min_joint_mi=0.05)
    combos = {frozenset(t[0]) for t in out}
...
def test_synergy_high_cardinality_hits_max_cells_cap():
    ...
    out = detect_synergy_combos([a, b], yt, [0, 1], max_order=2, min_order=2,
                                synergy_ratio=1.5, min_joint_mi=0.05)
...
def test_synergy_many_distinct_codes_noise_not_admitted():
    ...
    assert jmi < 0.05, (
        f"MM-corrected joint MI of high-card noise pair too high: {jmi} "
        f"(detect_synergy_combos admitted: {out})")
...
def test_two_seeded_fits_identical_support():
    ...
    kw = dict(random_seed=7, verbose=0, n_jobs=1, full_npermutations=2,
              baseline_npermutations=2, fe_max_steps=0,
              skip_retraining_on_same_shape=False, min_features_fallback=2)
