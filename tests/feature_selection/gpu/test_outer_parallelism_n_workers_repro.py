...
def test_outer_bit_exact_across_n_workers_ge2(n_workers_pair):
    ...
    assert conf_a == conf_b, f"outer-mode n_workers={nw_a} -> conf={conf_a}, n_workers={nw_b} -> conf={conf_b}; same seed must give same output."
    assert mi_a == mi_b
...
def test_inner_still_bit_exact_across_n_workers():
    ...
    for nw in (1, 2, 4, 8):
        ...
        confs.append(conf)
    assert len(set(confs)) == 1, f"inner mode regressed - n_workers in {{1,2,4,8}} gave {confs}"
...
def test_outer_reduces_to_full_inner_across_workers():
    ...
    assert len(set(out_values)) == 1, f"outer-mode reproducibility broken: {out_values}"
